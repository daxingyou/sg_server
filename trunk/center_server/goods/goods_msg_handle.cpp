...
bool goods_msg_handle_t::init_game_msg_handler()
{
	...
	bresult &= game_msg_handler_t::m_game_msg_handle.register_func(op_cmd::ge_destine_item_request, handle_ge_destine_item_request);
	bresult &= game_msg_handler_t::m_game_msg_handle.register_func(op_cmd::ge_undestine_item_request, handle_ge_undestine_item_request);
...
bool goods_msg_handle_t::handle_ge_goods_buy_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	...
	if (msg.has_reason()) 
	{
		auto reason = reply.mutable_reason();
		reason->CopyFrom(msg.reason());
	}
...
bool goods_msg_handle_t::handle_ge_destine_item_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::ge_destine_item_request);
	...
	proto::server::eg_destine_item_reply reply;
	uint32_t reply_code = goods_manager_t::destine_goods(uid, msg.search_id(),reply);
	reply.set_role_id(uid);
	reply.set_num(msg.num());
	reply.set_reply_code(reply_code);
	if (msg.has_reason())
	{
		auto p_reason = reply.mutable_reason();
		p_reason->CopyFrom(msg.reason());
	}
	p_user->send_msg_to_game(op_cmd::eg_destine_item_reply, reply);
	return true;
}

bool goods_msg_handle_t::handle_ge_undestine_item_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::ge_undestine_item_request);
	...
	goods_manager_t::undestine_goods(uid,msg.goods_uid());
	return true;
}
