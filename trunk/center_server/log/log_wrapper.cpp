...
void log_wrapper_t::send_offline_arena_rank_log(uint64_t role_uid, uint32_t op_type, uint32_t old_class, uint32_t old_rank, uint32_t new_class, uint32_t new_rank)
{
	if (!env::xml->log_sql.offline_arena)
	{
		return;
	}
	proto::log::eo_offline_arena_rank_log_notify msg;
	msg.set_role_uid(role_uid);
	msg.set_role_name(global_user_data_mgr_t::get_name(role_uid));
	msg.set_role_level(global_user_data_mgr_t::get_user_level(role_uid));
	msg.set_op_type(op_type);
	msg.set_old_class(old_class);
	msg.set_old_rank(old_rank);
	msg.set_new_class(new_class);
	msg.set_new_rank(new_rank);
	msg.set_log_time(common::time_util_t::now_time());
	write_new_log(op_cmd::eo_offline_arena_rank_log_notify, msg);
}
