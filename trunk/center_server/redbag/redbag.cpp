...
void redbag_t::peek_data(proto::common::redbag_data* p_data)
{
	...
	p_data->set_sender_name(global_user_data_mgr_t::get_name(m_sender_uid));
...
uint32_t redbag_t::draw(global_user_ptr p_role, proto::common::redbag_data* p_data)
{
	...
	int32_t draw_money = 0;
	if (1 == m_rest_count)
	{
		//只有一个包了
		draw_money = (int32_t)m_rest_money;
	}
	else
	{
		//还有超过一个包 最大值最小值算法比较绕
        ...
        if (draw_money < 0)
        {
            log_error("redbag[%lu] draw_min[%d] draw_max[%d] error!", get_uid(), draw_min, draw_max);
            return errcode_enum::notice_unknown;
        }
	}
