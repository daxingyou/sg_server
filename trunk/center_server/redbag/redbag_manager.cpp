...
void redbag_manager_t::load_data()
{
	...
	for (auto redbag_uid : redbag_uids)
	{
		...
		std::string sender_name = global_user_data_mgr_t::get_name(sender_uid);
...
void redbag_manager_t::redbag_timeout(uint64_t redbag_uid, const boost::system::error_code& ec)
{
	...
	if (p_redbag->get_rest_money() > 0)
	{
		...
		std::string sender_name = global_user_data_mgr_t::get_name(p_redbag->get_sender_uid());
		log_wrapper_t::send_redbag_log(p_redbag->get_sender_uid(), sender_name, redbag_uid, p_redbag->get_count(),
...
void redbag_manager_t::send(global_user_ptr p_role, uint32_t count, uint32_t money)
{
	...
	std::string sender_name = global_user_data_mgr_t::get_name(p_redbag->get_sender_uid());
	log_wrapper_t::send_redbag_log(p_redbag->get_sender_uid(), sender_name, p_redbag->get_uid(), p_redbag->get_count(),
