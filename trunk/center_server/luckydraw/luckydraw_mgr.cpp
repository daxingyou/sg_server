...
bool luckydraw_mgr_t::init() 
{
	...
	//初始化系统类型的祈福礼包
	LuckydrawType* p_conf = GET_CONF(LuckydrawType, proto::common::luckydraw_type_wish);
	if (NULL == p_conf)
	{
		log_error("NULL == p_conf LuckydrawType[%d]", proto::common::luckydraw_type_wish);
		return false;
	}
	
	for (auto card : p_conf->card())
	{
		LuckydrawWish* p_conf_wish = GET_CONF(LuckydrawWish, card);
		if (NULL == p_conf_wish)
		{
			log_error("NULL == p_conf_wish LuckydrawWish[%d]", card);
			return false;
		}

		if (p_conf_wish->open_time() > 0)
			continue;
		
		proto::common::wish_summon_info wish_info;
		wish_info.set_wish_summon_id(card);
		wish_info.set_open_flag(0);
		wish_info.set_time_left(0);
		wish_info.set_buy_count(0);
		luckydraw_mgr_t::sys_wish_summon_map.insert(std::make_pair(card, wish_info));
	}
	...
	for (auto it : country_map)
	{
		...
		for (auto card : p_conf->card())
		{
			...
			wish_info.set_time_left(0);
			wish_info.set_buy_count(0);
			wish_summon_list.insert(std::make_pair(card, wish_info));
		}
...
uint32_t luckydraw_mgr_t::on_buy_wish_summon(const global_user_ptr &p_user, uint32_t summon_id)
{
	...
	LuckydrawType* p_conf_type = GET_CONF(LuckydrawType, proto::common::luckydraw_type_wish);
	if (NULL == p_conf_type) {
		log_error("role[%lu] LuckydrawType[%u] not find", p_user->get_uid(), proto::common::luckydraw_type_wish);
		return errcode_enum::notice_unknown;
	}
	...
	else
	{
		...
		uint32_t money = p_conf_type->money().second * (money_per / 10000.0f);
