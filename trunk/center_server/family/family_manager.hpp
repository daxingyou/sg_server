...
public:
	...
	// 禅让族长
	static uint32_t family_demise(const global_user_ptr& p_user, uint64_t uid);
	...
	// 得到满足条件的助战家族列表
	static void get_help_family_list(uint32_t page, const std::set<uint64_t>& invited_list, const std::set<uint64_t>& redused_list, proto::client::ec_king_war_help_family_list_reply& reply);

	// 模糊查询
	static void get_family_fuzzy_query(const std::string& name, proto::client::ec_family_fuzzy_query_reply& reply);

	// 得到满足条件的助战家族列表
	static void get_help_family_fuzzy_list(const std::string& name, const std::set<uint64_t>& invited_list, const std::set<uint64_t>& redused_list, proto::client::ec_king_war_help_family_fuzzy_reply& reply);
...
public:
	...
	struct family_against_data_t
	{
		...
		proto::common::family_war_state m_state = proto::common::family_war_state_init;
...
public:
	...
	// 是否在家族战期间，并且是参战家族
	static bool is_family_war_member(uint64_t family_id);

	// 家族战因为对方家族不在无法开启，结束处理
	static void family_war_early_end(uint64_t win_family_id);
	...
	// 请求家族战信息
	static void world_trend_info_family_war(const global_user_ptr& p_user, proto::client::ec_world_trend_info_reply& reply);

	// 家族战开战通告
	static void user_online_family_war_notify(const global_user_ptr& p_user);
