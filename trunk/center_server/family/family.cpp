...
void family::update_prestige_daily()
{
	...
	// 排行榜更新
	rank_manager_t::update_rank_family(m_family_id, newprestige);
...
uint32_t family::family_demise(const global_user_ptr& p_master, uint64_t uid)
{
	if (NULL == p_master) {
		log_error("p_master null error!");
		return common::errcode_enum::notice_unknown;
	}

	const family_member_ptr& p_master_member = get_family_member(p_master->get_uid());
	if (NULL == p_master_member) {
		log_error("family[%lu] have no member[%lu]", m_family_id, p_master->get_uid());
		return common::errcode_enum::notice_unknown;
	}
	...
	// 把族长删除
	m_officer_map[family_officer_type_general].erase(p_master->get_uid());
	// 加入普通成员管理器中
	m_officer_map[family_officer_type_member].insert(p_master->get_uid());
	// 更新职位
	p_master_member->update_officer(family_officer_type_member);

	// 设置成员为族长
	m_officer_map[p_family_member->get_officer()].erase(uid);
	// 加入新的管理器中
	m_officer_map[family_officer_type_general].insert(uid);
	// 更新职位
	p_family_member->update_officer(family_officer_type_general);

	// 更新和族长相关的信息
	m_data.set_patriarch(common::string_util_t::uint64_to_string(uid));

	if (get_country_id() != 0 && is_king_family()) {
		country_ptr p_country = country_mgr_t::get_country(get_country_id());
		if (p_country && p_country->get_king_family() == m_family_id) { // 国王家族，重新设置国王
			p_country->demise_king(p_master, uid);
		}
	}

	// 职位变动，通知家族
	proto::client::ec_family_officer_changed_notify ntf;
	ntf.set_role_id(common::string_util_t::uint64_to_string(p_master->get_uid()));
	ntf.set_officer(family_officer_type_member);
	send_msg_to_family(op_cmd::ec_family_officer_changed_notify, ntf);

	// 职位变动，通知家族
	ntf.set_role_id(common::string_util_t::uint64_to_string(uid));
	ntf.set_officer(family_officer_type_general);
	send_msg_to_family(op_cmd::ec_family_officer_changed_notify, ntf);
	...
	MailTable* p_conf = GET_CONF(MailTable, common::mailcode_enum::sys_mail_family_demise_leader);
	if (p_conf) {
		std::string content_text;
		StringTool::format(content_text, common::string_util_t::convert_to_utf8(p_conf->content()).c_str(), p_master->get_name().c_str(), global_user_data_mgr_t::get_name(uid).c_str());
	...
	FAMILY_LOG("family[%lu] user[%lu] new office[%u]", m_family_id, uid, family_officer_type_general);

	WRITE_FAMILY_LOG_TO_DB(m_family_id, p_master->get_uid(), uid, common::log_enum::source_type_family_assign, family_officer_type_general);
...
void family::set_level(uint32_t val)
{
	...
	general_info_manager_t::add_num(proto::common::ACHIEVE_FAMILY_LEVEL, val);

	// 通知场景中玩家家族等级改变
	proto::server::eg_family_level_change_notify notify;
	notify.set_family_level(val);
	for (family_member_map::const_iterator citr = m_member_map.begin();
		citr != m_member_map.end(); ++citr) {
	...
	env::server->send_msg_to_all_games(op_cmd::eg_family_level_change_notify, 0, notify);
...
void family::tower_trigger_notify(uint64_t role_uid, const proto::server::ge_tower_trigger_notify& msg)
{
	...
	if (!p_notify_str) {
		log_error("Comprehensive[%u] not find", comprehensive_common::dreamland_hide_find);
		return;
	}
...
void family::set_king_war_apply_country(uint32_t country_id)
{
	if (country_id != m_king_war_apply_country) {
		m_king_war_apply_country = country_id;
	}
}

void family::set_king_war_fight_country(uint32_t country_id)
{
	if (country_id != m_king_war_fight_country) {
		m_king_war_fight_country = country_id;
	}
}

void family::send_sys_notice_to_member(uint32_t sys_tid)
{
	std::vector<uint64_t> to_uids;
	for (family_member_map::const_iterator citr = m_member_map.begin();
		citr != m_member_map.end(); ++citr)
	{
		if (citr->second != NULL && citr->second->is_online())
		{
			to_uids.push_back(citr->first);
		}
	}

	BROADCAST_SYS_NOTICE_TO_UIDS(sys_tid, to_uids, 0, m_family_id, 0);
}

void family::send_sys_mail_to_member(uint32_t mail_tid)
{
	MailTable* p_conf = GET_CONF(MailTable, mail_tid);
	if (!p_conf) {
		log_error("mail[%u] config not find", mail_tid);
		return;
	}
	...
	mail_manager_t::send_mail(members, proto::common::MAIL_TYPE_SYS, mail_tid);
}
