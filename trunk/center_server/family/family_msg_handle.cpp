...
bool family_msg_handle::handle_ce_family_assign_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	...
	if (msg.office() == family_officer_type_general) {
		back_code = family_manager_t::family_demise(p_user, common::string_util_t::string_to_uint64(msg.role_id()));
	} else {
		back_code = family_manager_t::assign_member(p_user, common::string_util_t::string_to_uint64(msg.role_id()), msg.office());
	}
	...
	network::msg_buf_ptr& msg_buf)
{
	...
	if (msg.type() == 0)
	{
		p_family->add_money(msg.num(), msg.source_type(), uid);
	}
	else
	{
		p_family->cost_money(msg.num(), msg.source_type(), uid);
	}
	return true;
}
