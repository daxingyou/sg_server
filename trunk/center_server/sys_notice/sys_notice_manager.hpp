...
public:
	...
	static void broadcast_system_notice(SysNoticeTable* p_table, const std::vector<uint64_t>& to_uids, uint64_t uid, uint64_t family_id, uint64_t country_id, const std::string& text);
	...
	static void broadcast_system_notice_to_family(uint64_t uid, const std::string& text, uint32_t label = 0);
...
//GM命令相关
public:	
	static void add_gm_system_notice(const proto::gm_common::gm_notice_single &single);
	//gm_消息
	static void broadcast_gm_system_notice( uint32_t type, const std::string &text );

	static void on_send_notice_callback(const boost::system::error_code& ec);

	static std::map<uint32_t, proto::gm_common::gm_notice_single> s_notice_map;

	static boost::shared_ptr<boost::asio::deadline_timer> s_send_notice_timer;	//扫荡结束定时器
};
...
#define BROADCAST_SYS_NOTICE_TO_UIDS(id, to_uids, uid, family_id, country_id, args...) \
	SysNoticeTable* p_notice = GET_CONF(SysNoticeTable, id); \
	if (NULL != p_notice && !p_notice->text().empty()) \
	{ \
		std::string notice_text; \
		StringTool::format(notice_text, common::string_util_t::convert_to_utf8(p_notice->text()).c_str(), ##args); \
		sys_notice_manager_t::broadcast_system_notice(p_notice, to_uids, uid, family_id, country_id, notice_text); \
	} 
...
#define BROADCAST_SYS_NOTICE_TO_FAMILY(id, uid,label,type,param_id ,args...) \
	LanguageTable* p_notice = GET_CONF(LanguageTable, id); \
	if (NULL != p_notice && !p_notice->str().empty()) \
	{ \
		std::string notice_text; \
		StringTool::format(notice_text, common::string_util_t::convert_to_utf8(p_notice->str()).c_str(), ##args); \
		sys_notice_manager_t::broadcast_system_notice_to_family(uid, notice_text,label); \
	}
...
#define BROADCAST_SYS_NOTICE_SYS_CHANNEL(uid,type,param_id, args...) \
	...
	{ \
		...
		if (IS_NOTIFY_TYPE_3 == p_config->is_notify()) \
		{ \
			BROADCAST_SYS_NOTICE_TO_FAMILY(p_config->language_id(), uid, p_config->label(), type, param_id, ##args); \
		} \
