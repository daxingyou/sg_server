...
bool family_data_mgr_t::load_all_datas(family_manager_t::family_map& familys)
{
	...
	FAMILY_LOG("family data load end, success[%u] failed[%u]", success_num, (int32_t)all_keys.size() - success_num);
...
bool family_data_mgr_t::load_all_war_history_data(uint32_t cond, proto::common::family_war_history_data& history_data)
{
	...
	for (const auto& key : keys)
	{
		uint32_t rank = redis_client_t::get_uint32("family_war", "family_rank", key);
		if (rank != 0) {
...
{
	return redis_client_t::is_hash_exist("family", family_id);
}
...
{
	return redis_client_t::is_hash_exist("family_member", member_id);
}

bool family_data_mgr_t::is_build_key_exsit(const std::string& key)
{
	return redis_client_t::is_hash_exist("family_build", key);
}
...
bool family_data_mgr_t::load_all_member(const family_ptr& p_family) 
{
	...
	for (const auto& member_id : all_members) {
		...
		// 对于删除的家族成员的处理
		if (redis_client_t::get_uint64("family_member", "family_id", member_id) != p_family->m_family_id) {
			continue;
		}
...
void family_data_mgr_t::del_family(uint64_t family_id)
{
	...
	{
		redis_client_t::remove_key("family_build", build_key);
	}

	// 删除家族信息
	redis_client_t::remove_key("family", family_id);
}
...
{
	if (p_family_member == NULL) {
		log_error("p_family_member null error");
		return;
	}
