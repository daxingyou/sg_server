...
bool country_data_mgr_t::load_all_datas(country_mgr_t::country_map& countries)
{
	...
	for (std::map<uint32_t, Country*>::iterator itr = all_Country.begin();
		...
		if (p_country->m_country_info.m_king_family_id != 0 && !family_data_mgr_t::is_family_key_exsit(p_country->m_country_info.m_king_family_id)) {
			log_error("country[%u] king_family[%lu] not exsit", itr->first, p_country->m_country_info.m_king_family_id);
			p_country->m_country_info.m_king_family_id = 0;
			update_country_king_family_id(itr->first, 0);
		}
		...
		for (std::map<uint32_t, CountryOfficial*>::iterator official_itr = all_Officials.begin();
			...
			if (!redis_client_t::is_hash_exist("country_office", key)) { // 对于新的官职初始化
				redis_client_t::set_uint32("country_office", "country_id", key, itr->first);
				redis_client_t::set_uint32("country_office", "country_officer_type", key, official_itr->first);
				redis_client_t::set_uint64("country_office", "role_uid", key, 0);
				redis_client_t::set_uint32("country_office", "reappoint_time", key, 0);
			...
			if (officer_uid != 0 && global_user_data_mgr_t::is_user_key_exsit(officer_uid)) {
				...
				p_country->m_country_info.officers[official_itr->first].set_hero_plugin(global_user_data_mgr_t::get_user_plugin(officer_uid));
			}
		...
		p_country->m_country_info.m_king_war_info.m_defensive_help_family = redis_client_t::get_uint64("country", "family_defensive_helper", itr->first);
		p_country->m_country_info.m_king_war_info.m_defensive_cost = redis_client_t::get_uint32("country", "defensive_cost", itr->first);
		p_country->m_country_info.m_king_war_info.m_defensive_notice_user = redis_client_t::get_string("country", "defensive_notice_user", itr->first);
		p_country->m_country_info.m_king_war_info.m_defensive_notice_officer = redis_client_t::get_uint32("country", "defensive_notice_officer", itr->first);
		p_country->m_country_info.m_king_war_info.m_defensive_notice = redis_client_t::get_string("country", "defensive_notice", itr->first);
		p_country->m_country_info.m_king_war_info.m_defensive_like_num = redis_client_t::get_uint32("country", "defensive_like_num", itr->first);
		p_country->m_country_info.m_king_war_info.m_defensive_dislike_num = redis_client_t::get_uint32("country", "defensive_dislike_num", itr->first);

		p_country->m_country_info.m_king_war_info.m_family_attack = redis_client_t::get_uint64("country", "family_attack", itr->first);
		p_country->m_country_info.m_king_war_info.m_attack_help_family = redis_client_t::get_uint64("country", "family_attack_helper", itr->first);
		p_country->m_country_info.m_king_war_info.m_attack_cost = redis_client_t::get_uint32("country", "attack_cost", itr->first);
		p_country->m_country_info.m_king_war_info.m_attack_notice_user = redis_client_t::get_string("country", "attack_notice_user", itr->first);
		p_country->m_country_info.m_king_war_info.m_attack_notice_officer = redis_client_t::get_uint32("country", "attack_notice_officer", itr->first);
		p_country->m_country_info.m_king_war_info.m_attack_notice = redis_client_t::get_string("country", "attack_notice", itr->first);
		p_country->m_country_info.m_king_war_info.m_attack_like_num = redis_client_t::get_uint32("country", "attack_like_num", itr->first);
		p_country->m_country_info.m_king_war_info.m_attack_dislike_num = redis_client_t::get_uint32("country", "attack_dislike_num", itr->first);
...
{
	return redis_client_t::is_hash_exist("country", country_id);
}

bool country_data_mgr_t::is_king_war_apply_key_exsit(uint64_t family_id)
{
	return redis_client_t::is_hash_exist("king_war_apply", family_id);
}
...
void country_data_mgr_t::init_king_war_offer_info(uint32_t king_war_id, country_t::king_war_apply_map& apply_map)
{
	...
	for (auto family_id : all_familys) {
		...
		country_t::king_war_apply_ptr p_apply_family(new country_t::king_war_apply());
		...
		p_apply_family->m_king_war_id = king_war_id;
		p_apply_family->m_bid_state = state;
		p_apply_family->m_bid_cost = redis_client_t::get_uint32("king_war_apply", "bid_cost", family_id);
		p_apply_family->m_time = redis_client_t::get_uint32("king_war_apply", "bid_time", family_id);

		apply_map.insert(std::make_pair(family_id, p_apply_family));

		family_ptr p_family = family_manager_t::get_family(family_id);
		if (p_family) {
			p_family->set_king_war_apply_country(king_war_id);
		}
	}
}
...
void country_data_mgr_t::update_country_king_war_info(uint32_t country_id, const country_t::king_war_info& info)
{
	...
	redis_client_t::set_uint64("country", "family_defensive_helper", country_id, info.m_defensive_help_family);
	redis_client_t::set_uint32("country", "defensive_cost", country_id, info.m_defensive_cost);
	redis_client_t::set_string("country", "defensive_notice_user", country_id, info.m_defensive_notice_user);
	redis_client_t::set_uint32("country", "defensive_notice_officer", country_id, info.m_defensive_notice_officer);
	redis_client_t::set_string("country", "defensive_notice", country_id, info.m_defensive_notice);
	redis_client_t::set_uint32("country", "defensive_like_num", country_id, info.m_defensive_like_num);
	redis_client_t::set_uint32("country", "defensive_dislike_num", country_id, info.m_defensive_dislike_num);

	redis_client_t::set_uint64("country", "family_attack", country_id, info.m_family_attack);
	redis_client_t::set_uint64("country", "family_attack_helper", country_id, info.m_attack_help_family);
	redis_client_t::set_uint32("country", "attack_cost", country_id, info.m_attack_cost);
	redis_client_t::set_string("country", "attack_notice_user", country_id, info.m_attack_notice_user);
	redis_client_t::set_uint32("country", "attack_notice_officer", country_id, info.m_attack_notice_officer);
	redis_client_t::set_string("country", "attack_notice", country_id, info.m_attack_notice);
	redis_client_t::set_uint32("country", "attack_like_num", country_id, info.m_attack_like_num);
	redis_client_t::set_uint32("country", "attack_dislike_num", country_id, info.m_attack_dislike_num);
...
void country_data_mgr_t::update_country_notice_info(uint32_t country_id, uint32_t type, const country_t::king_war_info& info)
{
	// 不存在直接return
	if (!is_country_key_exsit(country_id)) {
		log_error("country[%u] is not exsit", country_id);
	...
	if (type == country_t::king_war_defensive) {
		redis_client_t::set_string("country", "defensive_notice_user", country_id, info.m_defensive_notice_user);
		redis_client_t::set_uint32("country", "defensive_notice_officer", country_id, info.m_defensive_notice_officer);
		redis_client_t::set_string("country", "defensive_notice", country_id, info.m_defensive_notice);
	} else if (type == country_t::king_war_attack) {
		redis_client_t::set_string("country", "attack_notice_user", country_id, info.m_attack_notice_user);
		redis_client_t::set_uint32("country", "attack_notice_officer", country_id, info.m_attack_notice_officer);
		redis_client_t::set_string("country", "attack_notice", country_id, info.m_attack_notice);
	}
...
void country_data_mgr_t::update_country_like_info(uint32_t country_id, uint32_t type, const country_t::king_war_info& info)
{
	// 不存在直接return
	if (!is_country_key_exsit(country_id)) {
		log_error("country[%u] is not exsit", country_id);
...
void country_data_mgr_t::update_country_help_family(uint32_t country_id, uint32_t type, uint64_t help_family_uid)
{
	// 不存在直接return
	if (!is_country_key_exsit(country_id)) {
		log_error("country[%u] is not exsit", country_id);
	...
	if (type == country_t::king_war_defensive) {
		redis_client_t::set_uint64("country", "family_defensive_helper", country_id, help_family_uid);
	} else if (type == country_t::king_war_attack) {
		redis_client_t::set_uint64("country", "family_attack_helper", country_id, help_family_uid);
	}
}
...
void country_data_mgr_t::clear_king_war_info(uint32_t country_id)
{
	...
	redis_client_t::set_uint64("country", "family_defensive_helper", country_id, 0);
	redis_client_t::set_uint32("country", "defensive_cost", country_id, 0);
	redis_client_t::set_string("country", "defensive_notice_user", country_id, "");
	redis_client_t::set_uint32("country", "defensive_notice_officer", country_id, 0);
	redis_client_t::set_string("country", "defensive_notice", country_id, "");
	redis_client_t::set_uint32("country", "defensive_like_num", country_id, 0);
	redis_client_t::set_uint32("country", "defensive_dislike_num", country_id, 0);
	redis_client_t::set_uint64("country", "family_attack", country_id, 0);
	redis_client_t::set_uint64("country", "family_attack_helper", country_id, 0);
	redis_client_t::set_uint32("country", "attack_cost", country_id, 0);
	redis_client_t::set_string("country", "attack_notice_user", country_id, "");
	redis_client_t::set_uint32("country", "attack_notice_officer", country_id, 0);
	redis_client_t::set_string("country", "attack_notice", country_id, "");
	redis_client_t::set_uint32("country", "attack_like_num", country_id, 0);
	redis_client_t::set_uint32("country", "attack_dislike_num", country_id, 0);
	...
	// 清理报名
	std::vector<uint64_t> all_apply_familys;
	redis_client_t::get_keys_by_field_condition<uint64_t, uint64_t, std::vector<uint64_t>>("king_war_apply", "king_war_id", country_id, all_apply_familys);
	if (!all_apply_familys.empty()) {
		for (const auto& family_id : all_apply_familys) {
			redis_client_t::remove_key("king_war_apply", family_id);
