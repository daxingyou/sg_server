...
void challenge_t::peek_data(proto::common::challenge_info* p_info)
{
	...
	std::string challenger_name = global_user_data_mgr_t::get_name(m_challenger_uid);
	p_info->set_challenger_name(challenger_name);
	std::string target_name = global_user_data_mgr_t::get_name(m_target_uid);
	p_info->set_target_name(target_name);
...
void challenge_manager_t::load_data()
{
	...
	PK_LOG("today[%d] m_today_challenge_map size[%d]", today, (int32_t)m_today_challenge_map.size());

	uint32_t tomorrow = time_util_t::get_tomorrow();
	challenge_data_mgr_t::load_challenge_by_day(tomorrow, m_tomorrow_challenge_map);
	PK_LOG("tomorrow[%d] m_tomorrow_challenge_map size[%d]", tomorrow, (int32_t)m_tomorrow_challenge_map.size());
	...
	PK_LOG("day_after_tomorrow[%d] m_day_after_tomorrow_challenge_map size[%d]", day_after_tomorrow, (int32_t)m_day_after_tomorrow_challenge_map.size());
...
void challenge_manager_t::close_timer()
{
	if (NULL != m_timer)
	{
		m_timer->cancel();
...
uint32_t challenge_manager_t::challenge_request(global_user_ptr p_role, uint64_t target_uid)
{
	...
	if (p_role->get_uid() == target_uid)
	{
		...
		return errcode_enum::date_war_oneself;
	}
	...
	PK_LOG("add_challenge[%lu] challenger[%lu] target[%lu]", p_new_challenge->get_uid(), p_role->get_uid(), p_target->get_uid());
	//通知约战目标
...
uint32_t challenge_manager_t::challenge_process_request(global_user_ptr p_role, uint32_t process)
{
	...
	p_challenge->stop_process_timer();
	std::string challenger_name = global_user_data_mgr_t::get_name(p_challenge->get_challenger_uid());
	std::string target_name = global_user_data_mgr_t::get_name(p_challenge->get_target_uid());
	if (0 == process)
	{
		PK_LOG("role[%lu] accepted challenge[%lu]", p_role->get_uid(), p_challenge->get_uid());
		//应战
		...
		//接受约战的公告
		BROADCAST_SYS_NOTICE(sys_notice_enum::sys_notice_12, 0, 0, 0, challenger_name.c_str(), target_name.c_str());
		p_challenge->set_can_notice(true);
	}
	else
	{
		PK_LOG("role[%lu] refused challenge[%lu]", p_role->get_uid(), p_challenge->get_uid());
		...
		//拒绝约战的公告
		BROADCAST_SYS_NOTICE(sys_notice_enum::sys_notice_11, 0, 0, 0, challenger_name.c_str(), target_name.c_str(), challenger_name.c_str(), target_name.c_str())
	}
	//通知双方结果
	proto::client::ec_challenge_result_notify ntf;
	ntf.set_challenger_name(challenger_name);
	ntf.set_target_name(target_name);
...
void challenge_manager_t::challenge_timeout(uint64_t challenger_uid, const boost::system::error_code& ec)
{
	...
	p_challenge->stop_process_timer();
	PK_LOG("challenge[%lu] refused, respond time_out", p_challenge->get_uid());
	p_challenge->set_state(proto::common::challenge_state_refuse);
	challenge_data_mgr_t::update_challenge_state(p_challenge->get_uid(), p_challenge->get_state());
	std::string challenger_name = global_user_data_mgr_t::get_name(p_challenge->get_challenger_uid());
	std::string target_name = global_user_data_mgr_t::get_name(p_challenge->get_target_uid());
...
void challenge_manager_t::challenge_fight_result(uint64_t challenger_uid, uint32_t result)
{
	...
	PK_LOG("challenge[%lu] fight result[%d]", p_challenge->get_uid(), result);
	p_challenge->set_state(result);
	challenge_award(p_challenge);
	...
	{
		BROADCAST_SYS_NOTICE(sys_notice_enum::sys_notice_16, 0, 0, 0, challenger_name.c_str(), target_name.c_str());
	}
	else if (result == proto::common::challenge_state_lose)
	{
		BROADCAST_SYS_NOTICE(sys_notice_enum::sys_notice_17, 0, 0, 0, challenger_name.c_str(), target_name.c_str());
	}
	else if (result == proto::common::challenge_state_draw)
	{
		BROADCAST_SYS_NOTICE(sys_notice_enum::sys_notice_18, 0, 0, 0, challenger_name.c_str(), target_name.c_str());
	}
	else if (result == proto::common::challenge_state_win_without_fight)
	{
		BROADCAST_SYS_NOTICE(sys_notice_enum::sys_notice_15, 0, 0, 0, challenger_name.c_str(), target_name.c_str(), target_name.c_str(), challenger_name.c_str());
	}
	else if (result == proto::common::challenge_state_lose_without_fight)
	{
		BROADCAST_SYS_NOTICE(sys_notice_enum::sys_notice_15, 0, 0, 0, challenger_name.c_str(), target_name.c_str(), challenger_name.c_str(), target_name.c_str());
	}
}
...
void challenge_manager_t::one_day()
{
	...
	PK_LOG("m_today_challenge_map size[%d]", (int32_t)m_today_challenge_map.size());
	m_tomorrow_challenge_map.clear();
	m_tomorrow_challenge_map = m_day_after_tomorrow_challenge_map;
	PK_LOG("m_tomorrow_challenge_map size[%d]", (int32_t)m_tomorrow_challenge_map.size());
...
void challenge_manager_t::notice_challenge(uint32_t id, bool is_in_time)
{
	...
	for (auto it : m_today_challenge_map)
	{
		...
		std::string challenger_name = global_user_data_mgr_t::get_name(p_challenge->get_challenger_uid());
		std::string target_name = global_user_data_mgr_t::get_name(p_challenge->get_target_uid());
		BROADCAST_SYS_NOTICE(sys_notice_enum::sys_notice_14, 0, 0, 0, challenger_name.c_str(), target_name.c_str());
	}
}
...
void challenge_manager_t::notice_single_challenge(uint64_t role_uid)
{
	...
	std::string target_name = global_user_data_mgr_t::get_name(p_challenge->get_target_uid());
	//成功找到 开始装逼
	BROADCAST_SYS_NOTICE(sys_notice_enum::sys_notice_13, 0, 0, 0, challenger_name.c_str(), target_name.c_str(), time_util_t::get_day_str(p_challenge->get_challenge_time()).c_str());
...
void challenge_manager_t::ready_challenge(uint32_t id, bool is_in_time)
{
	if (is_in_time)
	{
		PK_LOG("ready_challenge size[%d]", (int32_t)m_today_challenge_map.size());
		//进入战前倒计时
		for (auto it : m_today_challenge_map)
		{
			challenge_ptr p_challenge = it.second;
			...
			if (NULL != p_challenger && p_challenger->is_in_king_city())
			{
				...
				std::string target_name = global_user_data_mgr_t::get_name(p_challenge->get_target_uid());
				ntf.set_challenger_name(target_name);
	...
	{
		PK_LOG("begin_challenge ready size[%d]", (int32_t)m_today_challenge_map.size());
		for (auto it : m_today_challenge_map)
		{
			challenge_ptr p_challenge = it.second;
			if (NULL == p_challenge)
			...
			if (NULL == p_challenger)
			{
				PK_LOG("NULL == p_challenger[%lu]", p_challenge->get_challenger_uid());
				challenger_exist = false;
			}
			else if (!p_challenger->is_in_king_city())
			{
				PK_LOG("p_challenger[%lu] is not in king_city", p_challenge->get_challenger_uid());
				challenger_exist = false;
			}
			...
			if (NULL == p_target)
			{
				PK_LOG("NULL == p_target[%lu]", p_challenge->get_target_uid());
				target_exist = false;
			}
			else if (!p_target->is_in_king_city())
			{
				PK_LOG("p_target[%lu] is not in king_city", p_challenge->get_target_uid());
				target_exist = false;
			}
			if (challenger_exist)
			{
...
void challenge_manager_t::begin_challenge(const boost::system::error_code& ec)
{
	...
	PK_LOG("begin_challenge size[%d]", (int32_t)m_today_challenge_map.size());
	if (NULL != m_timer)
	{
		m_timer->cancel();
	}
	...
	for (auto it : m_today_challenge_map)
	{
		...
		global_user_ptr p_challenger = global_user_manager_t::get_global_user_by_uid(p_challenge->get_challenger_uid());
		if (NULL != p_challenger && p_challenger->is_in_king_city() && 0 == p_challenger->get_troop_id())
		{
			challenger_exist = true;
		}
		bool target_exist = false;
		global_user_ptr p_target = global_user_manager_t::get_global_user_by_uid(p_challenge->get_target_uid());
		if (NULL != p_target && p_target->is_in_king_city() && 0 == p_target->get_troop_id())
		{
			target_exist = true;
		}
		if (challenger_exist)
...
void challenge_manager_t::challenge_award(challenge_ptr p_challenge)
{
	if (NULL == p_challenge)
	{
	...
	uint32_t result = p_challenge->get_state();
	uint64_t winner_uid = 0;
	if (result == proto::common::challenge_state_win)
	{
		winner_uid = p_challenge->get_challenger_uid();
	}
	else if (result == proto::common::challenge_state_lose)
	{
		winner_uid = p_challenge->get_target_uid();
	}
	else if (result == proto::common::challenge_state_draw)
	{
	...
	{
		//发送奖励
		auto p_award = GET_CONF(Comprehensive, comprehensive_common::pk_battle_fight_winreward);
		uint32_t mail_id = GET_COMPREHENSIVE_VALUE_1(p_award);
		mail_manager_t::send_mail(winner_uid, proto::common::MAIL_TYPE_SYS, mail_id);
	}
}
