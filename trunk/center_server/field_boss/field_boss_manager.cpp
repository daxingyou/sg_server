...
void field_boss_manager_t::clear_all()
{
	...
	for (filed_boss_id_map::const_iterator citr = m_filed_boss_id_map.begin();
		...
		for (const auto& p_filed_boss : citr->second) {
			if (!p_filed_boss) {
				log_error("p_filed_boss null error");
				continue;
			}

			if (p_filed_boss->get_state() == proto::common::FIELD_BOSS_STATE_START) {
	...
	if (msg.object_ids_size() != 0)
		env::server->send_msg_to_all_games(op_cmd::eg_clear_all_boss_notify, 0, msg);
...
void field_boss_manager_t::load_day(uint32_t cur_time)
{
	...
	uint32_t tm_wday = day_time.tm_wday > 0 ? day_time.tm_wday : 7;
...
void field_boss_manager_t::field_boss_rank_reward(const proto::server::ge_field_boss_rank_notify& ntf)
{
	...
	if (!persional_other_mail) {
		log_error("p_mail[%u] error null", persional_other_mail_id);
		return;
	}
	...
	if (!family_first_mail) {
		log_error("p_mail[%u] error null", family_first_mail_id);
		return;
	}
	...
	if (!family_other_mail) {
		log_error("p_mail[%u] error null", family_other_mail_id);
		return;
	}
