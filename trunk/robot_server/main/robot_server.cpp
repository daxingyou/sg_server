...
bool robot_server_t::load_ini(const std::string& ini_path)
{
    try
    {
        ...
        m_account_ip_list.clear();
        m_account_port_list.clear();
        std::string str_ip_list = tree.get<std::string>("robot.account_server_ip");
        string_util_t::split(str_ip_list, m_account_ip_list, ";");
        if (m_account_ip_list.empty())
        {
        ...
        string_util_t::split(str_port_list, m_account_port_list, ";");
        if (m_account_port_list.empty())
        {
            printf("\033[31mconnect to account: port_list is empty!\033[0m\n");
            return false;
        }
        if (m_account_ip_list.size() != m_account_port_list.size())
        {
            printf("\033[31mconnect to account: ip_size[%d] != port_size[%d]\033[0m\n", uint32_t(m_account_ip_list.size()), uint32_t(m_account_port_list.size()));
            return false;
        }
