...
function state_machine:add_transition(infrom,inevt,into,inguard,inaction)
	--print("infrom=", infrom, "inevt=", inevt)
...
-- 如果存在evt事件，则跳转到evt事件，如果没有evt事件，则跳转过所有事件后再添加evt事件
...
function state_machine:do_task()
	while self.taskfront <= self.taskback do
		...
		if self.task[self.taskfront].no_state == true then self:done_current_task() end -- no_state：无状态任务，once task
		return
	end
end
--------------------AI状态机_over-------------------------
