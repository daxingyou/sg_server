...
struct random_util_t
{
    ...
    template<typename T>
    static bool random_from_vec(const std::vector<T>& from_vec, uint32_t random_num, std::vector<T>& to_vec)
    {
        std::vector<T> tmp_from_vec(from_vec.begin(), from_vec.end());
        to_vec.clear();
		uint32_t from_vec_size = tmp_from_vec.size();
        if (from_vec_size <= 0 || random_num <= 0)
        {
            return false;
        }
		uint32_t tmp_num = random_num;
