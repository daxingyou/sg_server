...
struct bit_util_t
{
	...
	template <typename T>
	static void move_left_filling_1(T& src_val, uint32_t movement)
	{
		while (movement--)
			(src_val <<= 1) += 1;
	}
...
struct string_util_t
{
    ...
    {
		return std::move(boost::lexical_cast<std::string>(int_value));
    };
	...
	//将一个64位的数拆成 "32;32"
	static std::string uint64_to_2_string(uint64_t int_value)
	{
		uint32_t low = (int_value << 32) >> 32;
		uint32_t hight = int_value >> 32;
		std::string low_string = uint32_to_string(low);
		std::string hi_string = uint32_to_string(hight);
		return low_string + ";" + hi_string;
	}

	//讲一个"32:32"转成一个uint64;
	static uint64_t string_2_to_uint64(const std::string &str)
	{
		std::vector<std::string> tmp_vec;
		split(str, tmp_vec, ";");
		if (tmp_vec.size() != 2)
			return 0;
		uint32_t low = string_to_uint32(tmp_vec[0]);
		uint64_t hight = string_to_uint32(tmp_vec[1]);
		return (hight << 32) + low;
	}
};
...
public:
	...
	//获取间隔日切的时间戳(当日)
	static uint32_t get_zero_time()
	{
		time_t cur_time = now_time();
		time_t sp_time = get_zero_time(cur_time);
		if (cur_time > sp_time )
			return sp_time;
		return get_zero_time( cur_time - DAY_SECOND );
	}

	//获取间隔日切的时间戳
	static uint32_t get_zero_time(uint32_t time)
	{
		struct tm tm;
		get_local_time(tm, time);
		tm.tm_hour = 5;
		tm.tm_min = 0;
		tm.tm_sec = 0;
		return mktime(&tm);
	...
	static uint32_t get_next_special_time(uint32_t hour)
	{
		time_t cur_time = now_time();
		time_t sp_time = get_next_special_time( cur_time, hour);
		if (sp_time > cur_time)
			return sp_time;
		
		return get_next_special_time(cur_time + DAY_SECOND, hour);
	}

	//获取整点的时间戳
	static uint32_t get_next_special_time(uint32_t  time, uint32_t hour)
	{
		if (hour > 24)
			return 0;
