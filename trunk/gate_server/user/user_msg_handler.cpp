...
bool user_msg_handler_t::init_msg_handler()
{
	...
	m_user_msg_handle.register_func(op_cmd::ca_frames_check_request, handle_ca_frames_check_request);
...
bool user_msg_handler_t::handle_ca_pingpong_request(const tcp_socket_ptr& s, const msg_buf_ptr& msg_buf)
{
	...
	switch (p_user->get_game_state()) { 
		case common::ROLE_GAME_STATE_GATE:
		case common::ROLE_GAME_STATE_REGATE:
		case common::ROLE_GAME_STATE_LOGIN:
		case common::ROLE_GAME_STATE_SCENE:
		case common::ROLE_GAME_STATE_WAIT: { // 连接到gate就需要发pingpang
			p_user->start_pingpong_timer();
			p_user->send_msg_to_client(op_cmd::ac_pingpong_reply);
			break;
		}
		default: {
			log_error("user[%lu] state[%u] error, when rev ca_pingpong_request", p_user->get_uid(), p_user->get_game_state());
			break;
		}
	}
...
bool user_msg_handler_t::handle_ca_connect_request(const tcp_socket_ptr& s, const msg_buf_ptr& msg_buf)
{
    ...
    do 
    {
		...
		if (p_user->is_gate_state()) {
			...
			return false;
		} else if (p_user->is_login_game()) {
			log_error("user_id[%lu] state[%u] is already login gate", user_uid, p_user->get_game_state());
			break;
		}
		...
		//设置登陆状态
		switch (p_user->get_game_state()) {
			case common::ROLE_GAME_STATE_INIT: {
				p_user->set_login_gate();
				break;
			}
			case common::ROLE_GAME_STATE_OFFLINE: {
				p_user->set_login_regate();
				break;
			}
			default: {
				log_error("user[%lu] state[%u] error, when rev ca_connect_request", p_user->get_uid(), p_user->get_game_state());
				break;
			}
		}
		rlt = true;
...
bool user_msg_handler_t::handle_ca_frames_check_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	if (NULL == s)
	{
		log_error("s == NULL!");
	...
	PRE_C2S_MSG(proto::client::ca_frames_check_request);

	proto::client::ac_frames_check_reply reply;
	reply.set_high(msg.high());
	reply.set_low(msg.low());
	p_user->send_msg_to_client(op_cmd::ac_frames_check_reply, reply);
