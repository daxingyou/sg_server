...
bool user_manager_t::socket_bind_user(const tcp_socket_ptr& s, const user_ptr& p_user)
{
	...
	{
		// 绑定的时候不应该出现有老的socket
		log_error("user[%lu] NULL != old_socket", p_user->get_uid());
...
bool user_manager_t::del_socket(const tcp_socket_ptr& s, bool is_del_user)
{
    ...
    if (!find_socket(s))
    {
        log_error("not find socket[%d:%d], when del_socket!", s->socket().native(), s->get_socket_id());
        return false;
    }
    ...
    {
        log_info("gate no client connection!");
    }
...
void user_manager_t::socket_session_timeout_callback(const boost::system::error_code& ec, const tcp_socket_ptr& s)
{
	...
	ROLE_LOG("client[%d] session timeout!", s->get_socket_id());
...
void user_manager_t::clear_all_users_and_socket(bool is_notify_login, uint32_t reason)
{
	for (auto itr = m_client_socket_list.begin(); itr != m_client_socket_list.end(); ++itr)
	{
	...
	if (is_notify_login) { // 批量通知login
		proto::server::al_kick_gate_all_user_notify al_kick;
		al_kick.set_gate_id(env::server->get_id());
		env::server->send_msg_to_login(op_cmd::al_kick_gate_all_user_notify, 0, al_kick);
	}
...
void user_manager_t::pingpong_timeout_callback(const boost::system::error_code& ec, uint64_t uid)
{
	...
	case common::ROLE_GAME_STATE_GATE:		// 对于在登陆到gate，还没进入游戏的直接删除
		...
		break;
		case common::ROLE_GAME_STATE_REGATE:		// 对于重登玩家的处理
		{
			log_warn("user[%lu] long time no re-login game, so delete!", uid);
	...
	default:	// 对于离线和注销状态不用处理
		{
			log_error("user[%lu] pingpong_timeout_callback error state[%u]", uid, p_user->get_game_state());
		}
		break;
	}
}
    ...
    case KICK_ROLE_REASON_MSG_BOMB:
        {
			...
			ac_kick.set_kick_reason(errcode_enum::accounts_error_unusual);
