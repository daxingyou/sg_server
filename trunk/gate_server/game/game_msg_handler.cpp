...
bool game_msg_handler_t::init_cross_msg_handler()
{
	...
	m_cross_msg_register.insert(op_cmd::cg_ignore_lineup_flag_request);
	...
	m_cross_msg_register.insert(op_cmd::gc_combat_loading_end_reply);
	m_cross_msg_register.insert(op_cmd::gc_combat_play_end_reply);
	...
	m_cross_msg_register.insert(op_cmd::gc_combat_form_reply);
...
bool game_msg_handler_t::handle_ga_broadcast_notify(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	...
	case op_cmd::gc_family_war_fight_info_notify:
		...
		break;
	case op_cmd::gc_family_long_info_notify:
		{
			BROADCAST_UID_MSG(proto::client::gc_family_long_info_notify);
		}
		break;
	case op_cmd::gc_family_platform_info_notify:
		{
			BROADCAST_UID_MSG(proto::client::gc_family_platform_info_notify);
		}
		break;
	case op_cmd::gc_attack_icon_notify:
		{
			BROADCAST_UID_MSG(proto::client::gc_attack_icon_notify);
		}
		break;
	default:
		{
