...
// 			{
// 				log_error("uid[%lu] cross[%d] cmd[%s:%d] does not belong to cross_msg_list!", uid, p_user->get_cross_id(), CMD_DESC(cmd), cmd);
// 				return;
// 			}
// 		}
...
// 		}
		...
		log_trace("send user[%lu] cmd[%s:%d] size[%d] to client", uid, CMD_DESC(cmd), cmd, (uint32_t)send_buf->size());
    }
