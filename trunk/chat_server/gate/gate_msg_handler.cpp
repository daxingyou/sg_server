...
bool gate_msg_handler_t::handle_ch_channel_chat_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	...
	uint32_t silence_time = p_user->get_silence_time();
	if (time_util_t::now_time() < silence_time)
	{
		log_error("p_user[%lu] silence", uid);
		return false;
	}
...
int32_t gate_msg_handler_t::check_chat_info_valid(std::string& chat_info)
{
	...
	{
		log_error("chat_info[%s] length[%d] error!", chat_info.c_str(), (uint32_t)chat_info.length());
