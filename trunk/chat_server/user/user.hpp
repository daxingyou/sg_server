...
public:
	...
	const std::string& get_nickname() { return m_nickname; }
	void set_name(const std::string& name) { m_nickname = name; }
	...
	void set_silence_time(uint32_t val) { m_silence_time = val; }
	uint32_t get_silence_time() { return m_silence_time; }
private:
    uint64_t m_uid = 0;
