...
bool active_reward_data_manager::save_active_reward_data_to_redis(uint64_t uid, const proto::common::active_reward_data& data)
{
	redis_client_t::set_protobuf("active_reward", "active_reward_data", uid, data);
	return true;
}

bool active_reward_data_manager::load_active_reward_data_from_redis(uint64_t uid, proto::common::active_reward_data* p_data)
{
	if (p_data == NULL)
	{
		log_error("load_active_reward_data_from_redis [%lu] data == null", uid);
		return false;
	}
	proto::common::active_reward_data data_info;
	redis_client_t::get_protobuf("active_reward", "active_reward_data", uid, data_info);

	p_data->CopyFrom(data_info);
	return true;
}
