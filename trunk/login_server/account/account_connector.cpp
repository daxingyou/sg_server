...
void account_connector_t::on_new_msg(const tcp_socket_ptr& s, const message_t& msg)
{
	if (msg.has_uid())
	{
		env::server->get_ios().post(boost::bind(&login_server_t::on_account_msg, env::server, s, msg.get_uid() ,msg.head.cmd, msg.body));
	} 
	else
	{
		env::server->get_ios().post(boost::bind(&login_server_t::on_account_msg, env::server, s, 0, msg.head.cmd, msg.body));
	}
