...
public:
    ...
    */
    ...
    template<typename T_MSG>
    void send_msg_to_account(const network::tcp_socket_ptr& s, uint16_t cmd, const T_MSG& msg)
    {
        network::msg_buf_ptr send_buf = network::wrap_msg<T_MSG>(cmd, msg);
        if (NULL == send_buf)
        {
        ...
        account_ptr p_account = account_manager_t::get_account_by_socket(s);
        if (NULL == p_account)
        {
    ...
    template<typename T_MSG>
    void send_msg_to_all_accounts(uint16_t cmd, const T_MSG& msg)
    {
        network::msg_buf_ptr send_buf = network::wrap_msg<T_MSG>(cmd, msg);
        if (NULL == send_buf)
        {
