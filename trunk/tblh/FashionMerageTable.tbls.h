...
/// @brief    表格结构:FashionMerageTableBase
/// @remark   记录大小:12 X32
///           记录大小:20 X64
///           字段数量:3
///           数据文件:h合成表.xlsx 时装
...
struct FashionMerageTableBase
{
	...
	//------------------------
	uint32_t getUniqueID() const { return id; }; // TODO: 请用 时装ID 生成唯一键
...
struct FashionMerageTable : public Entry
{
...
public:
	void reset();
	void fill( const FashionMerageTableBase& base );
	const char* getClassName() const { return "h合成表"; }

	static const std::string getBinaryName();
};

typedef DataTableManager< FashionMerageTable, FashionMerageTableBase > FashionMerageTableManager;

inline void FashionMerageTable::reset()
{
	this->t_id = 0;
...
inline void FashionMerageTable::fill( const FashionMerageTableBase& base )
{
	this->reset();
...
inline const std::string FashionMerageTable::getBinaryName()
{
#ifdef _PLATFORM_X64
	return "FashionMerageTable.tbls64";
#else
	return "FashionMerageTable.tbls32";
#endif
};
