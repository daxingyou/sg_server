...
{
#ifdef _PLATFORM_X64
	static const int          FormatCheckCode = 0x09E504F9; ///< II|IIIIIIIVVVIIVIIIIIIIIIIVVVVVVVVIIII
#else
	static const int          FormatCheckCode = 0x023D04A9; ///< II|IIIIIIIvvvIIvIIIIIIIIIIvvvvvvvvIIII
#endif
	...
	uint32_t                  special_sign;                 ///< 技能特殊标记
	uint32_t                  max_cd;                       ///< 冷却时间最大值
};
#pragma pack()
...
struct SkillEffectTable : public Entry
{
	...
	uint32_t                  hp_condtion()                 { return t_hp_condtion; }
	...
	uint32_t                  special_sign()                { return t_special_sign; }
	uint32_t                  max_cd()                      { return t_max_cd; }
...
private:
	...
	uint32_t                  t_special_sign;               ///< 技能特殊标记
	uint32_t                  t_max_cd;                     ///< 冷却时间最大值
...
inline void SkillEffectTable::reset()
{
	...
	this->t_special_sign = 0;
		
	t_add_buff_vec.clear();
	t_dispel_buff_vec.clear();
	t_remove_buffs.clear();       
	t_remove_self_buffs.clear();  
	t_remove_friend_buffs.clear();
	t_add_gx_buff.clear();
	t_add_bd_buff.clear();
	this->t_max_cd = 0;
};
...
inline void SkillEffectTable::fill( const SkillEffectTableBase& base )
{
	...
	this->t_hp_condtion = base.hp_condtion;
	this->t_cd = base.cd;
	...
	this->t_special_sign = base.special_sign;
	this->t_max_cd = base.max_cd;
