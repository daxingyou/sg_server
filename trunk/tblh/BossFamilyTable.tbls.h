...
/// @brief    表格结构:BossFamilyTableBase
/// @remark   记录大小:20 X32
///           记录大小:28 X64
///           字段数量:5
///           数据文件:s首领刷新表.xlsx boss家族排名奖励
///           排序主键:id
#pragma pack(1)
struct BossFamilyTableBase
{
#ifdef _PLATFORM_X64
...
public:
	void reset();
	void fill( const BossFamilyTableBase& base );
	const char* getClassName() const { return "s首领刷新表"; }

	static const std::string getBinaryName();
};

typedef DataTableManager< BossFamilyTable, BossFamilyTableBase > BossFamilyTableManager;

inline void BossFamilyTable::reset()
{
	this->t_id = 0;
...
extern void addBossFamilyReward(uint32_t, uint32_t);

inline void BossFamilyTable::fill( const BossFamilyTableBase& base )
{
	this->reset();
	...
	addBossFamilyReward(base.monster_tid, base.id);
};

inline const std::string BossFamilyTable::getBinaryName()
{
#ifdef _PLATFORM_X64
	return "BossFamilyTable.tbls64";
#else
	return "BossFamilyTable.tbls32";
#endif
};
