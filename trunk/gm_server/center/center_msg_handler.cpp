...
bool center_msg_handler_t::init_center_msg_handler()
{
	...
	m_center_msg_handler.register_func(op_cmd::em_manage_role_ntf, handle_em_manage_role_ntf);
...
bool center_msg_handler_t::handle_em_manage_role_ntf(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::em_manage_role_ntf);

	admin_func_manager_t::send_manage_role_to_tools(msg);
