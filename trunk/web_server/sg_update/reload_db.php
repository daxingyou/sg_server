...
{
	$sql="select mark_type, version from platform_info";
	...
	if ($result != false )
	{
		while( $row=$result->fetch_array() ){
			...
			#查询战斗服地址
			$server_info->combat_url=load_combat_data($conn, $mark_type);
			#查询客户端加密信息
			load_encrypt_info($conn, $server_info );	
...
function load_encrypt_info($conn, &$server_info )
{
	$sql = "select `key`, `iv` from encrypt"; 
	$result=$conn->query($sql);
	if ($result != false ){
		while( $row=$result->fetch_array() ){
			$key=$row['key']; 
			...
			{
				$ch=chr(ord(substr($tmp_key,$i,1))-5);
				$key=$key.$ch;
			}	
...
{
	$sql = "select compatible_version from platform_info where mark_type = '$mark_type'";
   	$result=$conn->query($sql);
   	if ($result != false ){
		$version_list = array();
		$count = 0;
       	while( $row=$result->fetch_array() ){
       		$version_list[$count]=$row['compatible_version'];
           	$count = $count + 1;
      	}
       	return $version_list;
   	}else{
...
function load_combat_data($conn, $mark_type)
{   
    $sql = "select combat_url from combat_view where mark_type = '$mark_type'";
    $result=$conn->query($sql);
    if ($result != false ){
       $count = 0;
       while( $row=$result->fetch_array() ){
           $combat_data=$row['combat_url'];
       }
       return $combat_data;
   }else {
        write_log("load_combat_data $sql error:$result->error $result->errno");     
    }
}
