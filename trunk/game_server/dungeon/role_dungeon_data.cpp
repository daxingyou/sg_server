...
role_dungeon_data_t::role_dungeon_data_t(uint64_t uid)
	: m_role_uid(uid)
{
	m_times_key = cache_key_t::create(m_role_uid, cache_name::dungeon_times);
}
...
void role_dungeon_data_t::peek_data(const role_ptr& p_role, proto::common::role_all_client_dungeon_data* p_data)
{
	...
	for (auto it : m_dungeon_times_data_map)
	{
		dungeon_times_data_ptr p_dungeon_times_data = it.second;
		if (NULL == p_dungeon_times_data)
		{
			log_error("NULL == p_dungeon_times_data");
			continue;
		}
		proto::common::role_dungeon_times_data* p_single = p_data->add_times_datas();
		p_single->set_dungeon_type(p_dungeon_times_data->dungeon_type);
		p_single->set_remain_fight_time(p_dungeon_times_data->remain_fight_time);
		p_single->set_today_buy_time(p_dungeon_times_data->today_buy_time);
	}
...
void role_dungeon_data_t::save_data(dungeon_data_ptr p_dungeon_data, bool is_right_now /*= false*/)
{
	if (p_dungeon_data == NULL)
	{
		log_error("p_dungeon_data null error");
	...
	role_unify_save::add_task(m_role_uid, cache_key_t::create(m_role_uid, cache_name::dungeon, p_dungeon_data->dungeon_id), data, is_right_now);
}

void role_dungeon_data_t::load_times_data(const proto::common::role_all_dungeon_times_data& data)
{
	m_dungeon_times_data_map.clear();
	for (int32_t i = 0; i < data.datas_size(); ++i)
	{
		const proto::common::role_dungeon_times_data& dungeon_times_data = data.datas(i);

		dungeon_times_data_ptr p_dungeon_times_data(new dungeon_times_data_t());
		if (p_dungeon_times_data == NULL)
		{
			log_error("p_dungeon_times_data null error");
			continue;
		}

		p_dungeon_times_data->dungeon_type = dungeon_times_data.dungeon_type();
		p_dungeon_times_data->remain_fight_time = dungeon_times_data.remain_fight_time();
		p_dungeon_times_data->today_buy_time = dungeon_times_data.today_buy_time();
		m_dungeon_times_data_map.insert(std::make_pair(p_dungeon_times_data->dungeon_type, p_dungeon_times_data));
	}
}

void role_dungeon_data_t::save_times_data()
{
	proto::common::role_all_dungeon_times_data data;
	for (auto it : m_dungeon_times_data_map)
	{
		dungeon_times_data_ptr p_dungeon_times_data = it.second;
		if (NULL == p_dungeon_times_data)
		...
		proto::common::role_dungeon_times_data* p_single = data.add_datas();
		p_single->set_dungeon_type(p_dungeon_times_data->dungeon_type);
		p_single->set_remain_fight_time(p_dungeon_times_data->remain_fight_time);
		p_single->set_today_buy_time(p_dungeon_times_data->today_buy_time);
	}

	role_unify_save::add_task(m_role_uid, m_times_key, data);
}

void role_dungeon_data_t::one_day()
{
	m_dungeon_times_data_map.clear();
	save_times_data();
}
...
dungeon_times_data_ptr role_dungeon_data_t::get_dungeon_times_data(uint32_t dungeon_id)
{
	dungeon_times_data_map::iterator itr = m_dungeon_times_data_map.find(dungeon_id);
	if (itr != m_dungeon_times_data_map.end())
	{
		return itr->second;
	}
...
dungeon_times_data_ptr role_dungeon_data_t::add_dungeon_times_data(uint32_t dungeon_id)
{
	dungeon_times_data_ptr p_dungeon_times_data = get_dungeon_times_data(dungeon_id);
	if (NULL != p_dungeon_times_data)
	{
		return p_dungeon_times_data;
	}
	DungeonType* p_conf = GET_CONF(DungeonType, dungeon_id);
	if (NULL == p_conf)
	{
		log_error("DungeonType NULL == p_conf[%d]", dungeon_id);
		return p_dungeon_times_data;
	}
	p_dungeon_times_data = dungeon_times_data_ptr(new dungeon_times_data_t());
	if (NULL == p_dungeon_times_data)
	{
		log_error("p_dungeon_times_data null error");
		return p_dungeon_times_data;
	...
	p_dungeon_times_data->dungeon_type = dungeon_id;
	p_dungeon_times_data->remain_fight_time = p_conf->fight_times();
	p_dungeon_times_data->today_buy_time = 0;
	m_dungeon_times_data_map.insert(std::make_pair(p_dungeon_times_data->dungeon_type, p_dungeon_times_data));
	return p_dungeon_times_data;
}

void role_dungeon_data_t::add_fight_times(uint32_t dungeon_type)
{
	dungeon_times_data_ptr p_dungeon_times_data = get_dungeon_times_data(dungeon_type);
	if (NULL == p_dungeon_times_data)
	{
		p_dungeon_times_data = add_dungeon_times_data(dungeon_type);
	}
	if (NULL != p_dungeon_times_data && p_dungeon_times_data->remain_fight_time > 0)
	{
...
void role_dungeon_data_t::gm_pass_all_dungeon()
{
	std::map<uint32_t, DungeonTable*> all_dungeons;
	GET_ALL_CONF(DungeonTable, all_dungeons);
	...
	for (std::map<uint32_t, DungeonTable*>::iterator itr = all_dungeons.begin();
		...
		if (!p_dungeon_data) {
			...
			p_dungeon_data->dungeon_id = itr->first;

			m_dungeon_data_map.insert(std::make_pair(itr->first, p_dungeon_data));

			p_dungeon_data->total_pass_time += 1;

			save_data(p_dungeon_data);

			DUNGEON_LOG("role[%lu] add dungeon[%u] pass time", m_role_uid, itr->first);
		}
	}
}
