...
void role_buff_t::do_effect(uint32_t remain_time, bool notify)
{
	...
	case role_buff_type_addspeed:
		...
		break;
	case role_buff_type_add_percent_speed:
		{
			p_role->set_buff_prec_speed(p_role->get_buff_prec_speed() + p_conf->get_default_param());
			p_role->change_real_speed(notify ? role_t::speed_forced_update : role_t::speed_only_set);
			ROLE_BUFF_LOG("role[%lu] buff[%d] add_prec_speed[%d]", m_owner, m_tid, p_conf->get_default_param());
		}
		break;
	case role_buff_type_addattr:
		...
		break;
	case role_buff_type_addexp:
	{
		p_role->set_buff_addexp(p_role->get_buff_addexp() + p_conf->get_default_param());
		ROLE_BUFF_LOG("role[%lu] buff[%d] addexp[%d]", m_owner, m_tid, p_conf->get_default_param());
	}
	break;
	case role_buff_type_change_plugin:
	{
		p_role->set_change_plugin(p_conf->get_default_param());
		ROLE_BUFF_LOG("role[%lu] buff[%d] change_plugin[%d]", m_owner, m_tid, p_conf->get_default_param());
	}
	break;
	case role_buff_type_task_plugin:
	{
		p_role->set_task_plugin(p_conf->get_default_param());
		ROLE_BUFF_LOG("role[%lu] buff[%d] task_plugin[%d]", m_owner, m_tid, p_conf->get_default_param());
	}
	break;
	default:
		break;
...
void role_buff_t::lose_effect(bool notify)
{
	...
	case role_buff_type_add_percent_speed:
		{
			if (p_role->get_buff_prec_speed() > p_conf->get_default_param())
			{
				p_role->set_buff_prec_speed(p_role->get_buff_prec_speed() - p_conf->get_default_param());
			} 
			else
			{
				p_role->set_buff_prec_speed(0);
			}
			p_role->change_real_speed(notify ? role_t::speed_forced_update : role_t::speed_only_set);
			ROLE_BUFF_LOG("role[%lu] buff[%d] del_prec_speed[%d]", m_owner, m_tid, p_conf->get_default_param());
		}
		break;
	case role_buff_type_addattr:
		{
	...
	case role_buff_type_addexp:
	{
		if (p_role->get_buff_addexp() > (uint32_t)p_conf->get_default_param())
		{
			p_role->set_buff_addexp(p_role->get_buff_addexp() - p_conf->get_default_param());
		}
		else
		{
			p_role->set_buff_addexp(0);
		}
		ROLE_BUFF_LOG("role[%lu] buff[%d] add_addexp[%d]", m_owner, m_tid, p_conf->get_default_param());
	}
	break;
	case role_buff_type_change_plugin:
	{
		p_role->set_change_plugin(0);
		ROLE_BUFF_LOG("role[%lu] buff[%d] change_plugin[%d]", m_owner, m_tid, 0);
	}
	break;
	case role_buff_type_task_plugin:
	{
		p_role->set_task_plugin(0);
		ROLE_BUFF_LOG("role[%lu] buff[%d] task_plugin[%d]", m_owner, m_tid, 0);
	}
	break;
	default:
		break;
