...
void role_buff_manager_t::del_role_buff_by_type(uint32_t buff_type, uint32_t source_type, uint32_t source_param, bool notify /*= false*/)
{
	role_buff_ptr p_buff = NULL;
	for (auto& it : m_buff_map)
	{
		role_buff_ptr p_tmp = it.second;
		if (NULL == p_tmp)
		{
			continue;
		}
		auto p_conf = GET_CONF(RoleBuff, p_tmp->get_tid());
		if (NULL == p_conf)
		{
			log_error("NULL == p_conf[%d]", p_tmp->get_tid());
			continue;
		}
		if (p_conf->type() == buff_type)
		{
			p_buff = p_tmp;
			break;
		}
	...
	if (NULL == p_buff)
	{
		//log_error("role[%lu] del_role_buff_by_type NULL == p_buff[type:%d]", m_role_uid, buff_type);
		return;
	}
	uint32_t buff_id = p_buff->get_tid();
	ROLE_BUFF_LOG("role[%lu] del_role_buff_by_type[type:%d id:%d]", m_role_uid, buff_type, buff_id);
	del_role_buff(buff_id, source_type, source_param, notify);
}

void role_buff_manager_t::del_timer()
{
	for (auto& it : m_buff_map)
	{
		role_buff_ptr p_tmp = it.second;
...
void role_buff_manager_t::notify_to_client()
{
	role_ptr p_role = role_manager_t::find_role(m_role_uid);
	if (NULL != p_role)
	{
