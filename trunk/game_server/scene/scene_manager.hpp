...
public:
	...
	// 生成一个副本id
	static uint64_t get_new_dungeon_id(uint32_t map_tid);

	// 生成一个静态地图id
	static uint64_t get_static_scene_id(uint32_t map_tid, uint32_t index);
...
public:
	...
	{
		jump_result_failed = 0,			// 失败
		jump_result_success = 1,		// 同场景跳转成功
		jump_result_cross_server = 2,	// 跨场景跳转
		jump_result_center_check = 3,	// 中心服确认
	};

	static uint32_t get_pk_jump_time(uint32_t mode);
