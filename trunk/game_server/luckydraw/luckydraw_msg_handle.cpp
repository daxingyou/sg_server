...
bool luckydraw_msg_handle_t::init_msg_handler()
{
	...
	bresult &= gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_luckydraw_data_request, handle_cg_luckydraw_data_request);
	return bresult;
}
...
bool luckydraw_msg_handle_t::handle_cg_luckydraw_data_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_luckydraw_data_request);
	...
	proto::client::gc_luckydraw_data_reply reply;
	luckydraw_manager_t::get_luckydraw_data(p_role, reply.mutable_luckydraw());
	p_role->send_msg_to_client(op_cmd::gc_luckydraw_data_reply, reply);
	return true;
}
