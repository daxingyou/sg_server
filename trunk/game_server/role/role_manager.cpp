...
void role_manager_t::del_all_roles(uint32_t kick_reason)
{
    for (role_map::iterator citr = m_role_list.begin(); citr != m_role_list.end(); ++citr)
    {
        ...
        if (NULL != p_role)
        {
			if (env::server->get_type() == game_server_type_normal && ROLE_STATE_OK == p_role->get_state())
			{
				...
				// 通知其他服务器自己登出
				if (KICK_ROLE_REASON_DB_SERVER_CLOSED == kick_reason) {
					p_role->notify_the_other_servers_del_role();
				}
			}
			p_role->del_clear();
...
void role_manager_t::scene_reg_user(uint64_t uid, const proto::server::eg_reg_user_scene_request& request)
{
	do 
	{
        ...
        udr.set_time(time_util_t::now_msecs());
...
void role_manager_t::on_del_buff(uint64_t role_uid, uint32_t buff_id, const boost::system::error_code& ec)
{
	...
	p_role_buf_mgr->notify_to_client();
}

void role_manager_t::exec_all_user(void(*func_callback)(role_ptr p_role, uint32_t ex_data ),  uint32_t  ex_data )
{
	...
	for (auto it : m_role_list)
	{
		func_callback(it.second, ex_data);
	}
}
