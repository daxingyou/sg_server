...
uint32_t shop_manager_t::check_user_buy_goods(const role_ptr& p_role, const proto::common::user_buy_goods& info, em_goods_check_type type, uint32_t buy_count, proto::common::role_change_data* p_data)
{
	if (p_role == NULL)
	{
	...
	for (const auto& item : conds) {
		switch (item.first) {
			...
			case proto::common::shop_buy_condition_task: {
				...
				{
					SHOP_LOG("user[%lu] ShopTable id[%u] item[%d] task not done", p_role->get_object_id(), goods_id, item.second);
	...
	uint32_t cost = p_conf->price(goods_num, buy_count);
	if (money < cost)
	{
		SHOP_LOG("user[%lu] money_type[%u] money_value[%u] ShopTable id[%u] money[%u] count[%u] insufficient",
			p_role->get_object_id(), p_conf->money_type(), money, goods_id, cost, goods_num);
...
bool shop_manager_t::back_role_money(role_ptr p_role, const proto::common::user_buy_goods& info, uint32_t buy_count)
{
	if (p_role == NULL)
	{
		log_error("p_role null error");
...
uint32_t shop_manager_t::give_role_item(role_ptr p_role, const proto::common::user_buy_goods& info, uint32_t buy_count, proto::common::role_change_data* p_data)
{
	if (p_role == NULL)
	{
		log_error("p_role null error");
...
uint32_t shop_manager_t::goods_buy_request( role_ptr p_role, uint64_t goods_uid, uint32_t item_tid, uint32_t buy_count, uint32_t price, const proto::common::ex_reason *p_reason )
{
	if (NULL == p_role)
	{
	...
	if (NULL != p_reason) 
	{
		auto reason = msg.mutable_reason();
		reason->CopyFrom(*p_reason);
	}
...
void shop_manager_t::goods_buy_reply( role_ptr p_role, const proto::server::eg_goods_buy_reply& msg, proto::client::gc_goods_buy_reply& reply )
{
	...
	if (p_item->is_equip())
	{
		...
		p_item->set_remain_remake_count(es.remain_remake_count());
		p_item->set_add_remake_count(es.add_remake_count());
		...
		if (NULL != p_task_mgr)
		{
			...
			p_task_mgr->try_change_task_state(TASK_TARGET_TYPE_COMMIT_PROP, *p_data, p_item_conf->search_id(), 1, false);
		}
	}
	...
	//处理购买回调, 如果以后有多可以考虑封成函数
	if (msg.has_reason())
	{
		const proto::common::ex_reason &reason = msg.reason();
		switch (reason.type())
		{
			//处理自动任务回调
		case proto::common::EX_REASON_TYPE::EX_REASON_TASK:
			{
				proto::client::gc_auto_task_buy_item_reply reply;
				reply.set_item_uid(string_util_t::uint64_to_string( p_item->get_uid() ));
				reply.set_task_id(reason.ex_id());
				reply.set_reply_code( errcode_enum::error_ok );
				p_role->send_msg_to_client(op_cmd::gc_auto_task_buy_item_reply, reply);
				break;
			}
		}
	}
...
// 根据查询ID ，预定某个道具
void shop_manager_t::destine_buy_request(role_ptr p_role, uint32_t search_id, uint32_t buy_count, uint32_t reason, uint32_t ex_id)
{
	if ( NULL == p_role )
	{ 
		log_info("p_role is NULL");
		return ;
	}

	//去center预定某个道具
	proto::server::ge_destine_item_request request;
	request.set_role_id( p_role->get_uid() );
	request.set_search_id( search_id );
	request.set_num(buy_count);
	proto::common::ex_reason *p_reason = request.mutable_reason();
	p_reason->set_type( reason );
	p_reason->set_ex_id( ex_id );
	env::server->send_msg_to_center( op_cmd::ge_destine_item_request, p_role->get_uid(), request );
}

//预定道具返回	
uint32_t shop_manager_t::destine_buy_reply( role_ptr p_role, uint64_t goods_uid, uint32_t item_tid, uint32_t buy_count, uint32_t price, const proto::common::ex_reason &reason) 
{
	if (NULL == p_role)
	{
		log_info("p_role is NULL");
	...
	//如果找到道具就走购买逻辑
	uint32_t ret_val = common::errcode_enum::notice_unknown;
	if (goods_uid > 0) 
	{
		ret_val = shop_manager_t::goods_buy_request(p_role, goods_uid, item_tid, buy_count, price, &reason);
	}
	
	//交易失败,解锁
	if (ret_val != common::errcode_enum::error_ok) 
	{
		proto::server::ge_undestine_item_request request;
		request.set_goods_uid( goods_uid );
		env::server->send_msg_to_center( op_cmd::ge_undestine_item_request, p_role->get_uid(), request );
		ret_val = errcode_enum::exchange_no_sell;
	}

	return ret_val;
}

uint32_t shop_manager_t::auto_buy_task_item(role_ptr p_role, uint32_t task_id)
{
	if (NULL == p_role)
	{
	...
	if (NULL == p_task_mgr)
	{
		log_error("user:[%lu] p_task_mgr is NULL", p_role->get_uid());
		return errcode_enum::notice_unknown;
	}

	auto p_task = p_task_mgr->get_task(task_id);
	if (NULL == p_task)
	{
		log_error("user[%lu], task[%d] == NULL", p_role->get_uid(), task_id);
		return errcode_enum::notice_unknown;
	}

	Task *p_confg = GET_CONF(Task, task_id );
	if (!p_confg)
	{
		log_error("user[%lu], task config[%d] == NULL", p_role->get_uid(), task_id);
		return errcode_enum::notice_unknown;
	}

	if (p_confg->get_target_type() != TASK_TARGET_TYPE_COMMIT_PROP)
	{
		log_error("user[%lu], task[%d] is not commit_item task", p_role->get_uid(), task_id);
		return errcode_enum::notice_unknown;
	}

	uint32_t search_id = p_confg->get_target_cond(TASK_TARGET_TYPE_COMMIT_PROP);
	uint32_t num = p_confg->get_target_cnt(TASK_TARGET_TYPE_COMMIT_PROP, search_id);
	...
	request.set_num(num);
	proto::common::ex_reason *p_reason = request.mutable_reason();
	p_reason->set_ex_id(task_id);
	p_reason->set_type( proto::common::EX_REASON_TYPE::EX_REASON_TASK ); 
	env::server->send_msg_to_center( op_cmd::ge_destine_item_request, p_role->get_uid(), request );
