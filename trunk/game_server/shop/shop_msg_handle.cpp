...
bool shop_msg_handle_t::init_center_msg_handler()
{
	...
	bresult &= center_msg_handler_t::m_center_msg_handle.register_func(op_cmd::eg_destine_item_reply, handle_eg_destine_item_reply);
...
bool shop_msg_handle_t::init_gate_msg_handler()
{
	...
	bresult &= gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_auto_buy_task_item_request, handle_cg_auto_buy_task_item_request);
...
bool shop_msg_handle_t::handle_eg_check_user_buy_goods_request(const network::msg_buf_ptr& msg_buf)
{
	...
	uint32_t back_code = shop_manager_t::check_user_buy_goods(p_role, msg.info(), em_goods_check_type::goods_check_type_center, msg.purchased(), center_reply.mutable_change_data());
	...
	center_reply.set_purchased(msg.purchased());
...
bool shop_msg_handle_t::handle_eg_give_user_goods_notify(const network::msg_buf_ptr& msg_buf)
{
	...
	if (msg.reply_code() != common::errcode_enum::error_ok)
	{
		...
		shop_manager_t::back_role_money(p_role, msg.info(), msg.purchased());
	}
	else
	{
		...
		if (data != NULL)
		{
			...
			// 把道具发给玩家
			uint32_t back_code = shop_manager_t::give_role_item(p_role, msg.info(), msg.purchased(), data);
			reply.set_reply_code(back_code);
...
bool shop_msg_handle_t::handle_cg_buy_goods_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	...
	proto::client::gc_buy_goods_reply reply;
	uint32_t back_code = shop_manager_t::check_user_buy_goods(p_role, msg.info(), em_goods_check_type::goods_check_type_game, 0, reply.mutable_change_data());
	reply.set_reply_code(back_code);
...
bool shop_msg_handle_t::handle_eg_destine_item_reply(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eg_destine_item_reply);
	...
	uint32_t reply_code = shop_manager_t::destine_buy_reply(p_role, msg.goods_uid(), msg.item_id(), msg.num(), msg.price(), msg.reason());
...
bool shop_msg_handle_t::handle_cg_auto_buy_task_item_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_auto_buy_task_item_request);
	...
	uint32_t reply_code =shop_manager_t::auto_buy_task_item(p_role, msg.task_id());
	if (reply_code != errcode_enum::error_ok)
	{
		proto::client::gc_auto_task_buy_item_reply reply;
		reply.set_reply_code(reply_code);
