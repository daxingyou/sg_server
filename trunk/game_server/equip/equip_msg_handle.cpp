...
bool equip_msg_handle_t::init_msg_handler()
{
	...
	gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_equip_soul_request, handle_cg_equip_soul_request);
	gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_equip_add_remake_count_request, handle_cg_equip_add_remake_count_request);

	gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_equip_plan_data_request, handle_cg_equip_plan_data_request);
	gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_equip_plan_create_request, handle_cg_equip_plan_create_request);
	gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_equip_plan_modify_request, handle_cg_equip_plan_modify_request);
	gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_equip_plan_delete_request, handle_cg_equip_plan_delete_request);
	gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_equip_plan_use_request, handle_cg_equip_plan_use_request);
	return true;
}
...
bool equip_msg_handle_t::handle_cg_equip_soul_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_equip_soul_request);
	...
	proto::client::gc_equip_soul_reply reply;
	uint32_t reply_code = equip_manager_t::equip_soul(p_role, equip_uid, msg.soul_id(), reply.mutable_rcd());
	reply.set_reply_code(reply_code);
	p_role->send_msg_to_client(op_cmd::gc_equip_soul_reply, reply);
	return true;
}


bool equip_msg_handle_t::handle_cg_equip_add_remake_count_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_equip_add_remake_count_request);
	...
	proto::client::gc_equip_add_remake_count_reply reply;
	uint32_t reply_code = equip_manager_t::equip_add_remake_count(p_role, equip_uid, reply.mutable_rcd());
	reply.set_reply_code(reply_code);
	p_role->send_msg_to_client(op_cmd::gc_equip_add_remake_count_reply, reply);
	return true;
}

bool equip_msg_handle_t::handle_cg_equip_plan_data_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_equip_plan_data_request);
	...
	auto p_equip_plan_mgr = p_role->get_equip_plan_mgr();
	...
	proto::client::gc_equip_plan_data_reply reply;
	p_equip_plan_mgr->peek_data(reply.mutable_equip_plan_data());
	p_role->send_msg_to_client(op_cmd::gc_equip_plan_data_reply, reply);
	return true;
}

bool equip_msg_handle_t::handle_cg_equip_plan_create_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_equip_plan_create_request);
	...
	uint64_t weapon = string_util_t::string_to_uint64(msg.weapon());
	uint64_t armor = string_util_t::string_to_uint64(msg.armor());
	uint64_t talisman = string_util_t::string_to_uint64(msg.talisman());
	proto::client::gc_equip_plan_create_reply reply;
	uint32_t reply_code = p_equip_plan_mgr->create_plan(msg.name(), weapon, armor, talisman);
	if (errcode_enum::error_ok == reply_code)
	{
		p_equip_plan_mgr->peek_data(reply.mutable_equip_plan_data());
	}
	reply.set_reply_code(reply_code);
	p_role->send_msg_to_client(op_cmd::gc_equip_plan_create_reply, reply);
	return true;
}

bool equip_msg_handle_t::handle_cg_equip_plan_modify_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_equip_plan_modify_request);
	...
	uint64_t plan_uid = string_util_t::string_to_uint64(msg.uid());
	uint64_t weapon = string_util_t::string_to_uint64(msg.weapon());
	uint64_t armor = string_util_t::string_to_uint64(msg.armor());
	uint64_t talisman = string_util_t::string_to_uint64(msg.talisman());
	proto::client::gc_equip_plan_modify_reply reply;
	uint32_t reply_code = p_equip_plan_mgr->modify_plan(plan_uid, msg.name(), weapon, armor, talisman);
	if (errcode_enum::error_ok == reply_code)
	{
		p_equip_plan_mgr->peek_data(reply.mutable_equip_plan_data());
	}
	reply.set_reply_code(reply_code);
	p_role->send_msg_to_client(op_cmd::gc_equip_plan_modify_reply, reply);
	return true;
}

bool equip_msg_handle_t::handle_cg_equip_plan_delete_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_equip_plan_delete_request);
	...
	uint64_t plan_uid = string_util_t::string_to_uint64(msg.uid());
	proto::client::gc_equip_plan_delete_reply reply;
	uint32_t reply_code = p_equip_plan_mgr->delete_plan(plan_uid);
	if (errcode_enum::error_ok == reply_code)
	{
		p_equip_plan_mgr->peek_data(reply.mutable_equip_plan_data());
	}
	reply.set_reply_code(reply_code);
	p_role->send_msg_to_client(op_cmd::gc_equip_plan_delete_reply, reply);
	return true;
}

bool equip_msg_handle_t::handle_cg_equip_plan_use_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_equip_plan_use_request);
	...
	uint64_t hero_uid = string_util_t::string_to_uint64(msg.hero_uid());
	uint64_t equip_plan_uid = string_util_t::string_to_uint64(msg.equip_plan_uid());
	proto::client::gc_equip_plan_use_reply reply;
	uint32_t reply_code = p_equip_plan_mgr->use_plan(equip_plan_uid, hero_uid, reply.mutable_rcd());
	if (errcode_enum::error_ok == reply_code)
	{
		p_equip_plan_mgr->peek_data(reply.mutable_equip_plan_data());
	}
	reply.set_reply_code(reply_code);
	p_role->send_msg_to_client(op_cmd::gc_equip_plan_use_reply, reply);
	return true;
}
