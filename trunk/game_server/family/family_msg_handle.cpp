...
bool family_msg_handle_t::init_gate_msg_handler()
{
	...
	ret &= gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_formations_hero_update_request, handle_cg_formations_hero_update_request);
...
bool family_msg_handle_t::init_center_msg_handler()
{
	...
	ret &= center_msg_handler_t::m_center_msg_handle.register_func(op_cmd::eg_family_level_change_notify, handle_eg_family_level_change_notify);
...
bool family_msg_handle_t::handle_cg_impeach_family_general_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
    ...
    {
        log_error("role [%lu] has [%u] gold while [%u] required money not enough can't impeach family general", p_role->get_uid(), money, impeach_money_limit);
...
bool family_msg_handle_t::handle_eg_create_family_reply( const network::msg_buf_ptr& msg_buf)
{
	...
	{
		p_role->join_family(msg.info().id(), msg.info().name(), msg.info().officer(), msg.info().country_id(), msg.info().family_lv());
...
bool family_msg_handle_t::handle_eg_update_family_info_notify(const network::msg_buf_ptr& msg_buf)
{
	...
	if (msg.id() != 0)
	{
		p_role->join_family(msg.id(), msg.name(), msg.officer(), msg.country_id(), msg.family_lv());
	}
	else
	{
		p_role->leave_family();
...
bool family_msg_handle_t::handle_eg_family_level_change_notify(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eg_family_level_change_notify);

	for (int32_t i = 0; i < msg.role_list_size(); ++i) {
		role_ptr p_role = role_manager_t::find_role(msg.role_list(i));
		if (NULL != p_role) {
			p_role->set_family_level(msg.family_level());
		}
	}
...
bool family_msg_handle_t::handle_cg_formations_hero_update_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_formations_hero_update_request);
	...
	proto::client::gc_formations_hero_update_reply reply;
	uint32_t back_code = family_manager_t::ask_formations_hero_update(p_role, msg, reply);
	reply.set_reply_code(back_code);
	p_role->send_msg_to_client(op_cmd::gc_formations_hero_update_reply, reply);
