...
void buff_element_manager_t::get_all_element_attr_id(std::set<uint32_t>& ids)
{
	for (attr_map::iterator it = m_element_attr.begin(); it != m_element_attr.end(); ++it)
	{
		ids.insert(it->first);
	}
	for (attr_map::iterator it = m_element_attr_percent.begin(); it != m_element_attr_percent.end(); ++it)
	{
		ids.insert(it->first);
	}
}
