...
#include "friend/friend_msg_handler.hpp"
...
bool center_msg_handler_t::init_msg_handler()
{
	...
	ret &= m_center_msg_handle.register_func(op_cmd::eg_offline_arena_score_award_notify, handle_eg_offline_arena_score_award_notify);
	ret &= m_center_msg_handle.register_func(op_cmd::eg_offline_arena_sweep_award_notify, handle_eg_offline_arena_sweep_award_notify);
	ret &= m_center_msg_handle.register_func(op_cmd::eg_offline_arena_buy_reply, handle_eg_offline_arena_buy_reply);
	ret &= m_center_msg_handle.register_func(op_cmd::eg_offline_arena_join_notify, handle_eg_offline_arena_join_notify);
	ret &= m_center_msg_handle.register_func(op_cmd::eg_gm_transport_ntf, handle_eg_gm_transport_ntf);
	ret &= m_center_msg_handle.register_func(op_cmd::eg_reload_config_notify, handle_eg_reload_config_notify);
	ret &= m_center_msg_handle.register_func(op_cmd::eg_kick_all_user_notify, handle_eg_kick_all_user_notify);
	ret &= m_center_msg_handle.register_func(op_cmd::eg_gm_need_reload_xml_request, handle_eg_gm_need_reload_xml_request);
	...
	if (!general_event_msg_handler_t::init_center_msg_handler())
	{
		log_error("init general_event_msg_handler msg handler failed!");
		return false;
	}

	if (!friend_msg_handle_t::init_center_msg_handler())
	{
		log_error("init friend_msg_handle_t msg handler failed!");
		return false;
	}
...
bool center_msg_handler_t::init_data_syncers()
{
	...
	ret &= m_data_syncer.register_func(proto::server::data_sync_type_charm, on_sync_charm);
...
bool center_msg_handler_t::handle_eg_offline_arena_fight_request(const network::msg_buf_ptr& msg_buf)
{
	...
	if (os.object_type() == proto::common::SCENEOBJECT_MONSTER)
	{
		...
		fight_manager_t::fill_hero_data(
			...
			0, 
			1,
			p_against_conf->get_against_ids(), 
			msg.mutable_enemy_data()->mutable_heros(), 
			0);
	}
	...
	if (NULL != p_task_mgr) {
		...
		p_task_mgr->try_change_task_state(TASK_TARGET_TYPE_OFFLIEN_ARENA, rcd, 1, 1);
	}
	return true;
}
...
bool center_msg_handler_t::handle_eg_offline_arena_join_notify(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eg_offline_arena_buy_reply);
	role_ptr p_role = role_manager_t::find_role(uid);
	if (NULL == p_role)
	...
	p_role->m_formation_mgr.update_offline_arena_form();
	return true;
}


bool center_msg_handler_t::handle_eg_offline_arena_score_award_notify(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eg_offline_arena_score_award_notify);
	role_ptr p_role = role_manager_t::find_role(uid);
	if (NULL == p_role)
	{
		log_error("NULL == p_role role[%lu]", uid);
	...
	proto::client::gc_offline_arena_score_award_reply ntf;
	drop_manager_t::drop(p_role, msg.drop_id(), log_enum::source_type_offline_arena_score_award, proto::common::drop_sys_type_offline_arena, 0, ntf.mutable_rcd());
	p_role->send_msg_to_client(op_cmd::gc_offline_arena_score_award_reply, ntf);
	return true;
}

bool center_msg_handler_t::handle_eg_offline_arena_sweep_award_notify(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eg_offline_arena_sweep_award_notify);
	role_ptr p_role = role_manager_t::find_role(uid);
	if (NULL == p_role)
	{
		log_error("NULL == p_role role[%lu]", uid);
	...
	proto::client::gc_offline_arena_sweep_award_notify ntf;
	drop_manager_t::drop(p_role, msg.drop_id(), log_enum::source_type_offline_arena_win, proto::common::drop_sys_type_offline_arena, 0, ntf.mutable_rcd(), true, msg.count());
	p_role->send_msg_to_client(op_cmd::gc_offline_arena_sweep_award_notify, ntf);
	return true;
}

bool center_msg_handler_t::handle_eg_gm_transport_ntf(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eg_gm_transport_ntf);
	role_ptr p_role = role_manager_t::find_role(msg.role_uid());
	if (NULL == p_role)
	{
		log_error("NULL == p_role role[%lu]", uid);
		return false;
	...
	scene_manager_t::role_jump_scene(p_role, proto::server::user_change_scene, msg.scene_id());
	return true;
}


bool center_msg_handler_t::handle_eg_reload_config_notify(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eg_reload_config_notify);
	if (0 == msg.load_type())
	{
...
bool center_msg_handler_t::handle_eg_kick_all_user_notify(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eg_kick_all_user_notify);
	
	role_manager_t::del_all_roles(msg.reason());

	game_troop_mgr_t::del_all_troop();

	log_error("game_server[%u] del all user, reason:%u", env::server->get_id(), msg.reason());

	return true;
}

bool center_msg_handler_t::handle_eg_gm_need_reload_xml_request(const network::msg_buf_ptr& msg_buf)
{
...
bool center_msg_handler_t::on_sync_charm(uint64_t uid, std::string val)
{
	role_ptr p_role = role_manager_t::find_role(uid);
	if (NULL == p_role)
	{
	...
	uint32_t charm = string_util_t::string_to_uint32(val);
	p_role->set_charm(charm);
