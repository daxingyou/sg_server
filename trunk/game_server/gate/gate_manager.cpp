...
void gate_manager_t::add_gate(const tcp_socket_ptr& s)
{
    ...
    if (NULL == p_gate)
    {
		...
		log_debug("new gate socket[%s: %d] connect success!!!", s->addr().address().to_string().c_str(), s->get_socket_id());
        return;
    }
    else
    {
...
void gate_manager_t::set_gate(const tcp_socket_ptr& s, uint32_t gate_id, uint32_t server_id)
{
    ...
    log_info("add gate[server_id=%d: gate_id=%d], has %d gates", server_id, gate_id, (uint32_t)m_gate_list.size());
}
...
void gate_manager_t::del_gate(const tcp_socket_ptr& s)
{
    ...
    for (; it != it_end; ++it)
    {
        ...
        {
            log_info("del gate[server_id=%d: gate_id=%d], left %d gates", p_gate->get_server_id(), p_gate->get_id(), (uint32_t)m_gate_list.size()-1);
