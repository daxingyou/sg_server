	...
	proto::client::gc_trade_buy_item_reply &msg_reply)
{
	...
	if (has_buy_success)
	{
		...
		role->save_self();
		role->get_trade_info().save_trade_item_data(role->get_uid());
	...
	proto::client::gc_trade_sell_item_reply &msg_reply)
{
	...
	/// 存盘 组包
	if (has_sell_success && total_gain > 0)
	{
	...
	role->get_trade_info().notify_trade_item_info(msg, true);
	role->send_msg_to_client(op_cmd::gc_trade_info_notify, msg);
	achieve_common_t::notify_progress_state(role->get_uid(), proto::common::Achieve_Event_Type::ACHIEVE_TRADE_NUMBER, 0);
	...
	proto::common::city_trade_shop_info *p_data)
{
	...
	do 
	{
		...
		if (NULL == p_trade)
		{
			log_error("NULL == p_trade[%d]", trade_id);
			reply_code = errcode_enum::notice_city_not_find;
			break;
		}
		/// 查找背包中的道具
		...
		if (0 != role->get_troop_id())
		{
			...
			if (NULL == p_troop)
			{
				log_error("NULL == p_troop [%lu]",role->get_troop_id());
		...
		if (p_troop)
		{
			...
			for (const auto& member : member_list)
			{
                if (NULL == member)
                {
                    log_error("NULL == member");
                    continue;
                }
				if (member->get_object_type() != proto::common::SCENEOBJECT_USER) continue;

				if (member->get_state() != proto::common::troop_state_normal) continue;

				role_ptr p_role = role_manager_t::find_role(member->get_uid());
				if (NULL == p_role)
				{
					log_error("NULL == p_role troop role[%lu]", member->get_uid());
					continue;
				}
				bool is_own = false;
				...
				p_role->get_trade_info().save_trade_item_data(role->get_uid());
