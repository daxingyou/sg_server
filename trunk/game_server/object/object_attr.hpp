...
public:
	...
	void set_talent_degree(uint32_t val) { m_talent_degree = val; }
	uint32_t get_talent_degree() { return m_talent_degree; }
	...
	uint64_t get_last_equip_plan() { return m_last_equip_plan; }
	void set_last_equip_plan(uint64_t val) { m_last_equip_plan = val; }

	void set_nick_name(std::string val) { m_nick_name = val; }
	const std::string& get_nick_name() { return m_nick_name; }
	...
	void add_famous(uint32_t famous);
	const std::vector<uint32_t>& get_famous_list() { return m_famous; }

	const HeroSkillMap& get_skill_map() { return m_hero_skills; }
...
protected:
	...
	uint64_t m_last_equip_plan = 0;
	std::string m_nick_name = "";
	std::vector<uint32_t>	m_famous;
};
