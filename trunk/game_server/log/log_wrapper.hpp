...
public:
	...
	// 角色ID 任务ID 任务类型 任务状态 任务进度 接取时间 操作类型
	static void send_task_log(uint64_t role_uid, uint32_t task_id, uint32_t task_type, uint32_t task_state, const std::string& progress, 
	...
	*/
	...
	static void send_recharge_log(uint64_t role_uid, const std::string& order_id,uint32_t recharge_tid,uint32_t recharge_rmb, uint32_t channel_id);

	// 角色ID 角色等级 旧数量 变化数量 新数量 操作类型 消耗源 消耗源参数
	static void send_rmb_log(uint64_t role_uid, uint32_t role_level, uint32_t old_num, uint32_t up_num, uint32_t new_num,
	...
	// 角色ID 开始时间 结束时间 巡逻次数 累计获得经验 使用的多倍点数 巡逻场景ID 结束原因
	static void send_patrol_hire_log(uint64_t role_uid, uint32_t begin_time, uint32_t end_time, uint32_t count, uint32_t total_exp, uint32_t use_multi_point, uint32_t map_id, uint32_t source_type);
};
