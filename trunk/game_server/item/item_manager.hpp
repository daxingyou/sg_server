...
public:
	static item_ptr get_item_by_uid(object_base_ptr p_role, uint64_t uid,
...
public:
	...
	//合成时装 add by hy
	static uint32_t merage_fashion(role_ptr p_role, uint32_t item_tid, uint32_t merage_num, proto::common::role_change_data* p_data);

	//分解道具
	static uint32_t decompose_item(role_ptr p_role, uint64_t item_uid, uint32_t item_num, proto::common::role_change_data* p_data);

	//多选道具礼包使用
	static uint32_t use_multiple_chose_item(role_ptr p_role, uint64_t item_uid, const std::set<uint32_t> chosen_list, proto::common::role_change_data *p_data,
		proto::common::package_type package = proto::common::package_type::package_type_main);
public:
	// 物品发放校验
...
private:
	...
	static uint32_t use_item_change_plugin(role_ptr p_role, item_ptr use_item, Item *config, uint32_t item_num, proto::common::role_change_data *p_data);
	...
	static uint32_t use_item_add_title(role_ptr p_role, item_ptr use_item, Item *config, uint32_t item_num, proto::common::role_change_data *p_data);
	
	static uint32_t use_item_key_gift(role_ptr p_role, item_ptr use_item, Item *config, proto::common::role_change_data *p_data);
	
	static uint32_t use_item_add_activity(role_ptr p_role, item_ptr use_item, Item *config, proto::common::role_change_data *p_data);
