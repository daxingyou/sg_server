...
bool item_msg_handle_t::init_msg_handler()
{
	...
	gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_use_multiple_chosen_item_request, handle_cg_use_multiple_chosen_item_request);
    return true;
}
...
bool item_msg_handle_t::handle_cg_merage_item_request(const network::tcp_socket_ptr& s, const msg_buf_ptr& msg_buf)
{
	...
	if (msg.type() == 0) {
		reply_code = item_manager_t::merage_item(p_role, item_tid, merage_num, p_data);
	}else{
		reply_code = item_manager_t::merage_fashion(p_role, item_tid, merage_num, p_data);
	}
...
bool item_msg_handle_t::handle_cg_use_multiple_chosen_item_request(const network::tcp_socket_ptr& s, const msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_use_multiple_chosen_item_request);
	auto p_role = role_manager_t::find_role(uid);
	if (NULL == p_role)
	{
		log_error("[wys] role[%lu] NULL", uid);
	...
	proto::client::gc_use_multiple_chosen_item_reply reply;
	for (int32_t i = 0; i < msg.chosen_list_size(); ++i)
	{
		chosen_list.insert(msg.chosen_list(i));
	}
	uint64_t item_uid = string_util_t::string_to_uint64(msg.item_uid());
	uint32_t reply_code = item_manager_t::use_multiple_chose_item(p_role, item_uid, chosen_list, reply.mutable_rcd());
	reply.set_reply_code(reply_code);

	p_role->send_msg_to_client(op_cmd::gc_use_multiple_chosen_item_reply, reply);
	return true;
}
