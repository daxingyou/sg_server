...
uint32_t shake_money_tree_t::shake_money_tree(proto::client::gc_shake_money_tree_reply &msg)
{
	...
	if (NULL != p_act_mgr)
	{
		p_act_mgr->activity_done(activity_shake_money_tree);	//加次数即可
	}
...
bool shake_money_tree_t::rand_weighting(std::map<uint32_t, uint32_t> &weighting_map, uint32_t &key)
{
	...
	int32_t rand_value = random_util_t::randBetween(1, max_weight);
    if (rand_value < 0)
    {
        log_error("1 max_weight[%d] error!", max_weight);
        return false;
    }
	uint32_t rand = 0;
	for (auto it : weighting_map)
