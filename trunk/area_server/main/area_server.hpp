...
public:
    /*
    ****transfer 的消息处理(area --> transfer --> center --> game)
    */
    ...
    void send_msg_to_transfer(uint16_t cmd, uint64_t uid, const T_MSG& msg)
    {
        ...
        {
            log_error("send uid[%lu] cmd[%s:%d] to transfer[%d] failed: NULL == p_transfer->get_socket()", uid, CMD_DESC(cmd), cmd, p_transfer->get_id());
            return;
        }
    ...
    {
        const network::tcp_socket_ptr& s = transfer_manager_t::get_transfer_socket_by_id(server_id);
        if (NULL == s)
        {
        ...
        post_network_send_msg(s, send_buf);
        log_trace("send uid[%lu] cmd[%s:%d] to transfer[%d]", uid, CMD_DESC(cmd), cmd, server_id);
    }
    ...
    void send_msg_to_all_transfer(uint16_t cmd, uint64_t uid, const T_MSG& msg)
    {
        ...
        for (auto p_transfer : transfer_manager_t::get_transfer_list())
        {
            if (NULL == p_transfer)
            {
