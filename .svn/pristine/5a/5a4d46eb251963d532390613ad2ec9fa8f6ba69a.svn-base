...
public:
    ...
    {
		std::set<uint64_t>::iterator result = m_role_list.find(uid);
    ...
    void add_role(uint64_t uid)
    {
        ...
        m_role_list.insert(uid);
        //log_warn("add user[%lu] to the game[%d] list, has %d users in the game[%d]", uid, m_id, (uint32_t)m_role_list.size(), m_id);
    }
    ...
    void del_role(uint64_t uid)
    {
        ...
        //log_warn("delete user[%lu] from the game[%d] list, left %d users in the game[%d]", uid, m_id, (uint32_t)m_role_list.size(), m_id);
    }
...
public:
	...
	static bool is_all_game_close() { return m_game_list.empty(); }
