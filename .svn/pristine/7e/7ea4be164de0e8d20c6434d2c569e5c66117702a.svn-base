    ...
    for (size_t idx = 0; idx < m_account_list.size(); ++idx)
    {
        ...
        if (NULL != p_account)
        {
            ...
            if (NULL != s && p_account->get_socket() == s)
            {
                ...
                log_info("add account[%d], has %d accounts", account_id, get_account_size());
                return;
            }
        }
    }
...
void account_manager_t::del_account(const tcp_socket_ptr& s)
{
    ...
    account_vec::iterator it = m_account_list.begin();
    account_vec::iterator it_end = m_account_list.end();
    for (; it != it_end; ++it)
    {
        account_ptr p_account = (*it);
        if (NULL != p_account && p_account->get_socket() == s)
        {
            uint32_t account_id = p_account->get_id();
			p_account->reset();
            m_account_list.erase(it);
            log_info("del account[%d], left %d accounts", account_id, get_account_size());
            return;
        }
    }
}
...
account_ptr account_manager_t::get_account_by_rand()
{
    if (0 == get_account_size())
    {
        return account_ptr();
    }

    int32_t rand_index = rand() % get_account_size();
    return m_account_list[rand_index];
}
