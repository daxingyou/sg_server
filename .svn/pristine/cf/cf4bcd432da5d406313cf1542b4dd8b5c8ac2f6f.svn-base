...
bool drop_manager_t::drop(role_ptr p_role, uint32_t drop_bag_id, uint32_t source_type, proto::common::drop_sys_type drop_sys_type,
						  uint32_t source_param, proto::common::role_change_data* rcd)
{
	if (NULL == p_role)
	{
	...
	std::vector<DropTable*> drop_list;
	...
	for (uint32_t i = 0; i < p_drop_bag->drop_ids_size(); ++i)
	{
		...
		{
			log_error("drop get_drop_list error drop_ids[%s] drop_result_size[%d]", p_drop_bag->base_drop_ids().c_str(), drop_list.size());
			return false;
		}
	...
	for (uint32_t i = 0; i < drop_list.size(); ++i)
	{
		...
		{
            if (!item_manager_t::check_limit_item(p_drop->drop_item_tid(), p_drop->drop_item_num()))
            {
				ITEM_LOG("drop check_limit_item fail drop_item_tid[%d], drop_item_num[%d]", p_drop->drop_item_tid(), p_drop->drop_item_num());
                continue;
            }
		...
		case drop_item_type_item:// 添加道具
            ...
            if (money_type > proto::common::MONEY_TYPE_NONE && money_type < proto::common::MONEY_TYPE_END)
            {
                ...
                {
                    //经验道具
					is_process_success = p_role->add_exp(p_drop->drop_item_num(), source_type, source_param, rcd);
                }
				else if (money_type == proto::common::MONEY_TYPE_LIVENESS)
				{
					activity_mgr_ptr p_activity_mgr = p_role->get_activity_mgr();
					is_process_success = p_activity_mgr->add_liveness_count(p_drop->drop_item_num());
				}
                else
                {
                    //货币道具
					is_process_success = money_manager_t::add_money(p_role, money_type, p_drop->drop_item_num(), source_type, source_param, false, rcd);
                }
				...
				if (rcd && is_process_success)
				{
					...
					if (p_item_data)
					{
						...
						if (p_item_single)
						{
							...
							p_item_single->set_up_num(p_drop->drop_item_num());
						}
					}
				}
            }
            ...
            {
                //真正的道具
                if (item_manager_t::check_add_item(p_role, p_drop->drop_item_tid(), p_drop->drop_item_num(), proto::common::package_type_main))
                {
                    item_manager_t::add_item(p_role, p_drop->drop_item_tid(), p_drop->drop_item_num(),
                        source_type, source_param, rcd);
                }
                // 组织邮件内容
                else
                {
                    mail_item_info.insert(std::make_pair(p_drop->drop_item_tid(), p_drop->drop_item_num()));
		...
		if (is_need_syn_item_limit)
		{
			...
			if (NULL == p_item_data)
			{
				log_error("drop ge_check_drop_items_request add_item_list error pos[%d]", i);
				return false;
			}
			p_item_data->set_item_tid(p_drop->drop_item_tid());
			p_item_data->set_item_num(p_drop->drop_item_num());
		}
	}
								   ...
								   std::vector<DropTable*> &drop_all_result)
{
	...
	for (uint32_t i = 0; i < drop_result.size(); ++i)
	{
		...
		drop_all_result.push_back(p_drop_table);
...
bool drop_manager_t::check_drop_cond(role_ptr p_role, DropTable *p_drop)
{
	...
	case drop_cond_type_task:
		{	// 检查任务状态 未接取和已完成两种状态认为不满足条件
			...
			{
				ITEM_LOG("drop check_drop_cond fail role_uid[%lu] task[%d] state[%d] not finish", p_role->get_uid(), p_drop->cond_param1(), p_task->get_state());
				return false;
			}
		}
		break;
										 ...
										 const std::set<uint32_t> &drop_list, std::vector<DropTable*> &drop_result)
{
	...
	for (uint32_t roll_count = 0; roll_count < roll_limit; ++roll_count)
	{
		...
		if (!b_roll_result)
		{
			log_error("drop roll error drop_result[%d] roll_count[%d] roll_pos[%d]", drop_result.size(), roll_count, roll_pos);
			return false;
		}
		// 够数了 结束
										...
										uint32_t total_prob, int32_t &roll_pos)
{
	...
	// 随机
	uint32_t rand_value = random_util_t::randBetween(1, total_prob);
										 ...
										 const std::set<uint32_t> &drop_list, std::vector<DropTable*> &drop_result)
{
	...
	for (uint32_t roll_count = 0; roll_count < roll_limit; ++roll_count)
	{
		...
		if (!b_roll_result)
		{
			log_error("drop roll error drop_result[%d] roll_count[%d]", drop_result.size(), roll_count);
			return false;
		}
		// 够数了 结束
		if (drop_result.size() >= drop_num)
	...
	uint32_t source_param, std::vector<proto::common::item_num_info> &item_vec, proto::common::role_change_data* rcd)
{
	...
	for (uint32_t i = 0; i < drop_list.size(); ++i)
	{
		...
		item_info.set_num(p_drop->drop_item_num());
