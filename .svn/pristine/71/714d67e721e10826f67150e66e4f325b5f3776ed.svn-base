...
public:
    ...
    {
        std::set<uint64_t>::iterator itr = m_user_list.find(uid);
    ...
    void add_user(uint64_t uid)
    {
        ...
        ROLE_LOG("add user[%lu] to the game[%d] list, has %d users in the game[%d]", uid, m_id, (uint32_t)m_user_list.size(), m_id);
    }
    ...
    bool del_user(uint64_t uid)
    {
        ...
        ROLE_LOG("delete user[%lu] from the game[%d] list, left %d users in the game[%d]", uid, m_id, (uint32_t)m_user_list.size(), m_id);
        return true;
    }
...
private:
    ...
    uint32_t                    m_id = 0;
	uint32_t					m_type = 0;
    std::set<uint64_t>			m_user_list; // 包含在线和不在线的是所有该game上的users
};
