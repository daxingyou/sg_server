...
public:
	static item_ptr get_equip_by_uid(object_base_ptr p_role, uint64_t uid);
...
public:
	...
	static uint32_t equip_soul(role_ptr p_role, uint64_t equip_uid, uint32_t soul_id, proto::common::role_change_data* p_data);
	static uint32_t equip_add_remake_count(role_ptr p_role, uint64_t equip_uid, proto::common::role_change_data* p_data);
private:
	...
	static uint32_t get_equip_remake_count();
public:
...
private:
	...
	static void make_add_remake_info();
private:
	...
	static std::vector<Dawn::triple<uint32_t, uint32_t, uint32_t>> s_add_remake_info; //增加改造次数
};
