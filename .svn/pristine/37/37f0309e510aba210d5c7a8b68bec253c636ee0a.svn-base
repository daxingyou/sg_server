...
public:
	...
	// 设置所有战斗失败
	void set_all_fight_failed();

	// 更新血量
	uint32_t update_hero_hp(family_war_side_type side_type, uint64_t fight_id, const proto::common::fight_camp& enemy_camp, const combat_ptr& p_combat);
	...
	// 设置占领的家族
	void set_occupy_side(family_war_side_type side_type);

	// 得到伤害百分百
	float get_hurt_pre(family_war_side_type side_type);

	// 设置复活时间
	void set_long_revive(uint32_t count_down);
