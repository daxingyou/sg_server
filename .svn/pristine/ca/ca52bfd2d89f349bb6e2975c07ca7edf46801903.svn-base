...
// TODO：消息打包还可以优化，把uid移到包头，减少内存new的次数
...
static msg_buf_ptr wrap_msg(uint16_t cmd, uint64_t uid, const T_MSG& msg)
{
	...
	if (PH_LEN + UID_MASK_LEN + body_sz <= MAX_TCP_PACKET_SIZE) {
		...
		*((uint64_t*)unBuf) = uid;
		...
		msg_head_t* head = new (zlibBuf) msg_head_t();//placement new;
		head->cmd = cmd;
		head->flags = msg_head_t::flag_uid;
		head->flags |= msg_head_t::flag_compress; // 压缩标记

		int32_t retcode = compress((Bytef*)&zlibBuf[PH_LEN], &destLen, (const Bytef *)unBuf, uzSize);
		if (retcode != Z_OK) {
		...
		head->len = destLen;
        if (destLen > MAX_TCP_DATA_SIZE) {
            log_warn("user[%lu] cmd[%s:%d] uzsize[%u] size[%lu] > MAX_TCP_DATA_SIZE[%u]", uid, CMD_DESC(cmd), cmd, uzSize, destLen, MAX_TCP_DATA_SIZE);
        }
...
static msg_buf_ptr wrap_msg(uint16_t cmd, const T_MSG& msg)
{
	...
	if (PH_LEN + body_sz <= MAX_TCP_PACKET_SIZE) {
		...
		head->len = destLen;
        if (destLen > MAX_TCP_DATA_SIZE) {
            log_warn("cmd[%s:%d] uzsize[%u] size[%lu] > MAX_TCP_DATA_SIZE", CMD_DESC(cmd), cmd, uzSize, destLen);
        }
...
static msg_buf_ptr re_wrap_buf_to_client(uint16_t cmd, const T_BUF& buf)
{
	...
	if (PH_LEN + body_sz <= PACKET_ZIP_MIN) {
		...
		int32_t retcode = compress((Bytef*)&zlibBuf[PH_LEN], &destLen, (const Bytef *)unBuf, body_sz);
		if (retcode != Z_OK) {
			log_error("cmd[%s:%d] compress error, reason:%d", CMD_DESC(cmd), cmd, retcode);
			return NULL;
		}
		...
		head->len = destLen;
		if (destLen > MAX_TCP_DATA_SIZE) {
			log_warn("cmd[%s:%d] uzsize=%u size=%lu > MAX_TCP_DATA_SIZE", CMD_DESC(cmd), cmd, body_sz, destLen);
		}
...
static msg_buf_ptr wrap_msg_to_client(uint16_t cmd, const T_MSG& msg)
{
	...
	if (PH_LEN + body_sz <= PACKET_ZIP_MIN) {
		...
		head->len = destLen;
		if (destLen > MAX_TCP_DATA_SIZE) log_warn("cmd[%s:%d] uzsize=%u size=%lu > MAX_TCP_DATA_SIZE", CMD_DESC(cmd), cmd, uzSize, destLen);
...
template<int i = 0>
static msg_buf_ptr wrap_buffer_to_client(uint16_t cmd, const char *buffer, const unsigned int nLen)
{
#if 1 // 暂时把服务器去客户端的消息的压缩去掉
	if (PH_LEN + nLen <= PACKET_ZIP_MIN) {
		msg_buf_ptr out(new msg_buf_t(PH_LEN + nLen));
		char* p = out->begin();
		msg_head_t* head = new (out->data()) msg_head_t();//placement new;
		head->cmd = cmd;
		head->len = nLen;
		head->flags = 0;
		bcopy(buffer, &p[PH_LEN], nLen);
		return out;
	} else {
		if (PH_LEN + nLen >= MAX_UZLIB_CHAR) {
			log_error("cmd[%s:%d] msg size[%u] >= MAX_UZLIB_CHAR", CMD_DESC(cmd), cmd, PH_LEN + nLen);
			return NULL;
		}

		uLong destLen = unzip_size(nLen);
		char zlibBuf[destLen + PH_LEN];
		msg_head_t* head = new (zlibBuf) msg_head_t();//placement new;
		head->cmd = cmd;
		head->flags = msg_head_t::flag_compress; // 压缩标记

		int32_t retcode = compress((Bytef*)&zlibBuf[PH_LEN], &destLen, (const Bytef *)buffer, nLen);
		if (retcode != Z_OK) {
			log_error("cmd[%s:%d] compress error, reason:%d", CMD_DESC(cmd), cmd, retcode);
			return NULL;
		}
		...
		head->len = destLen;
		if (destLen > MAX_TCP_DATA_SIZE) log_warn("cmd[%s:%d] uzsize=%u size=%lu > MAX_TCP_DATA_SIZE", CMD_DESC(cmd), cmd, nLen, destLen);
...
static msg_buf_ptr re_wrap_msg_s(uint16_t cmd, const T_BUF& buf)
{
	...
	if (PH_LEN + body_sz <= MAX_TCP_PACKET_SIZE) {
		...
		int32_t retcode = compress((Bytef*)&zlibBuf[PH_LEN], &destLen, (const Bytef *)buf->data(), uzSize);
		if (retcode != Z_OK) {
			log_error("cmd[%s:%d] compress error, reason:%d", CMD_DESC(cmd), cmd, retcode);
			return NULL;
		}
