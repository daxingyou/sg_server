...
void trade_manager_t::event_reset_on_time(const boost::system::error_code& ec)
{
	...
	else
	{
		...
		do 
		{
			...
			if (-1 == rand)
			{
				log_error("trade_event_type_popular min[%d]  max[%d]",0, popular_list.size() - 1);
				break;
			}
			if (rand >= (int32_t)popular_list.size())
			{
				log_error("rand[%d] popular_list.size()[%d]",rand, popular_list.size());
				break;
			}
	...
	else
	{
		...
		do
		{
			...
			if (-1 == rand)
			{
				log_error("trade_event_type_black min[%d]  max[%d]", 0, black_list.size() - 1);
				break;
			}
			if (rand >= (int32_t)black_list.size())
			{
				log_error("rand[%d] black_list.size()[%d]", rand, black_list.size());
				break;
			}
...
uint32_t trade_manager_t::troop_trade_view_notify(const uint64_t role_uid,const uint32_t trade_id)
{
	...
	do 
	{
		...
		if (NULL == p_troop)
		{
			log_error("NULL == p_troop troop_id[%d]",troop_id);
			break;
		}
