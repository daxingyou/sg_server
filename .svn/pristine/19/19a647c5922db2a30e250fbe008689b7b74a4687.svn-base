...
bool dungeon_scene_t::is_can_restart()
{
	...
	if (is_have_fight()) {
		// 有人在战斗就不能重新开启
		log_error("dungeon[%u] state[%u] error", m_state_controler.get_state_type());
		return false;
	}
...
void dungeon_scene_t::on_role_enter(role_ptr p_role)
{
	...
	switch (m_dugenon_create.m_create_type) {
		...
		case proto::server::dungeon_create_team: {
			...
			if (p_troop == NULL) {
				log_error("dungeon[%u] creater is team[%u], is already not exist", m_dungeon_id, m_dugenon_create.m_id);
				return;
			}
...
void dungeon_scene_t::notify_dungeon_schedule(role_ptr p_role /*= NULL*/)
{
	...
	for (std::vector<schedule_monster_ptr>::const_iterator citr = m_schedule_monster_vec.begin();
		...
		if (p_schedule_monster != NULL && !p_schedule_monster->is_complete_schedule()) {
	...
	if (p_role != NULL) {
		...
		send_msg_to_scene(op_cmd::gc_dungeon_schedule_data_notify, notify);
	}
}

void dungeon_scene_t::on_other_kill_me(role_ptr p_role, object_position_ptr p_enemy)
{
	if (p_role == NULL) {
		log_error("p_role null error");
		return;
...
void dungeon_scene_t::start_settlement()
{
	...
	if (m_dungeon_result == proto::common::dungeon_result_success) {	// 对于通关的处理，每个人都不一样
		...
		for (const auto& uid : m_roles) {
			...
			achieve_common_t::notify_progress_state(uid, proto::common::ACHIEVE_DUNGEON_TOTAL_PASS);
		}
	} else { // 失败的话都是一样的处理
...
void dungeon_scene_t::add_event_done(uint32_t id)
{
	if (!is_event_done(id)) {
		...
		DUNGEON_LOG("dungeon[%lu:%u] add event[%u]", m_dungeon_id, m_map_tid, id);
	}
}
...
{
	if (is_event_done(id)) {
		log_error("dungeon[%lu:%u] event[%u] is done", m_dungeon_id, m_map_tid, id);
		return;
	}
