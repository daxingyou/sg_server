...
public:
	 ...
	 * \brief 王城战状态
	 */
	static uint32_t get_king_war_state();
...
public:
	...
	*/
	...
	// 请求国家大势
	static void world_trend_info(const global_user_ptr& p_user, proto::common::country_king_war_info* info);

	// 获得报名信息
	static void get_king_war_sign_up_info(uint64_t family_id, proto::common::king_war_country_sign_up_info* info);

	// 获得对战信息
	static void get_king_war_pre_info(uint64_t uid, proto::common::king_war_country_pre_info* info);

	// 上线的时候检测是否是参战家族
	static void user_online_king_war_check(const global_user_ptr& p_user, uint64_t family_uid);

	// 清楚所有关于我的王城战邀请信息
	static void clear_king_war_invite_info(const global_user_ptr& p_user);

	// 获得当前国家的国王数量
	static uint32_t get_king_family_num();
...
protected:
	...
	static proto::server::global_data_king_war_data m_global_king_war_data;
