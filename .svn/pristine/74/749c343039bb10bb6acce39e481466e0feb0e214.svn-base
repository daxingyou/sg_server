...
{
    if (op_cmd::ac_pingpong_reply != cmd)
    {
...
bool robot_msg_handler_t::handle_gc_combat_start_loading_ntf(robot_ptr robot, const network::msg_buf_ptr& msg_buf)
{
	...
	//log_warn("robot_name[%s],fight_uid[%s]", robot->get_nickname().c_str(), msg.combat().fight_uid().c_str());
...
bool robot_msg_handler_t::handle_gc_move_notify(robot_ptr robot, const network::msg_buf_ptr& msg_buf)
{
	...
	// 自己的移动，别人的抛掉
	uint64_t obj_id = msg.object_id();
	if (robot->get_uid() == obj_id)
	{
		robot->add_recv_one();
	}
...
{
	PRE_S2C_MSG(proto::client::gc_batch_move_notify);

	for (int32_t i = 0; i < msg.move_list_size(); ++i) {
		if (msg.move_list(i).object_id() == robot->get_uid()) {
			robot->add_recv_one();
		}
	}
