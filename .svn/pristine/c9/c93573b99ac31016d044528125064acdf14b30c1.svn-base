...
std::vector<Dawn::triple<uint32_t, uint32_t, uint32_t>> equip_manager_t::s_add_remake_info;

extern common::id_allocator_t common::g_id_allocator;

item_ptr equip_manager_t::get_equip_by_uid(object_base_ptr p_role, uint64_t uid)
{
    ...
    if (!p_item->is_equip())
    {
		log_error("p_item[%lu] is not equip!", uid);
		return item_ptr();
    }
    return p_item;
}
...
bool equip_manager_t::make_equip(role_ptr p_role, item_ptr p_equip)
{
	...
	if (!p_equip->is_equip())
	{
		log_error("p_item[%lu:%d] is not equip!", p_equip->get_uid(), p_equip->get_tid());
		return false;
	}
	...
	//生成基础属性
	EQUIP_LOG("role[%lu] equip[%lu:%d] make_attr", p_role->get_uid(), p_equip->get_uid(), p_equip->get_tid());
	std::map<uint32_t, uint32_t> attr_map;
	make_attr(p_equip_config, attr_map);
	p_equip->set_attr(attr_map);
	//生成附加属性
	EQUIP_LOG("role[%lu] equip[%lu:%d] make_exattr", p_role->get_uid(), p_equip->get_uid(), p_equip->get_tid());
	...
	p_equip->set_remain_remake_count(get_equip_remake_count());
	p_equip->calc_score();
...
uint32_t equip_manager_t::exchange_equip(role_ptr p_role, uint32_t type, uint32_t exchange_id, proto::common::role_change_data* p_data)
{
	...
	if (p_role->get_level() < p_exchange_conf->need_level())
	{
		log_error("role[%lu] exchange_id[%d] level[%d] not enough, need[%d]", p_role->get_uid(), exchange_id, p_role->get_level(), p_exchange_conf->need_level());
	...
	if (money_manager_t::get_money(p_role, proto::common::MONEY_TYPE_ENERGY) < p_exchange_conf->copper())
	{
		log_error("role[%lu] exchange_id[%d] energy not enough", p_role->get_uid(), exchange_id);
	...
	money_manager_t::use_money(p_role, proto::common::MONEY_TYPE_ENERGY, p_exchange_conf->copper(), log_enum::source_type_equip_exchange, exchange_id, true, p_data);
	...
	else
	{
		...
		{
			//增加幸运值
			uint32_t add_lucky = item_t::get_equip_make_star_constant();
...
uint32_t equip_manager_t::equip_on(role_ptr p_role, uint64_t equip_uid, uint64_t hero_uid, proto::common::role_change_data* p_data)
{
	...
	if (p_equip->get_hero_uid() > 0)
	{
		equip_off(p_role, equip_uid, p_equip->get_hero_uid(), p_data);
	}
	...
	//{
	//	log_error("p_hero[%lu] profession[%d] invalid, p_equip_conf[%d]", hero_uid, profession, p_equip->get_tid());
	//	return errcode_enum::notice_talent_tips5;
	//}
	uint32_t pos = p_equip_conf->equip_type();
	uint64_t exist_equip_uid = p_hero->get_equip(pos);
	EQUIP_LOG("role[%lu] hero[%d] exist_equip_uid[%lu]", p_role->get_uid(), p_hero->get_tid(), exist_equip_uid);
	if (exist_equip_uid > 0)
	{
		...
		EQUIP_LOG("role[%lu] hero[%d] equip_off[%lu]", p_role->get_uid(), p_hero->get_tid(), exist_equip_uid);
	...
	{
		p_equip_plan_mgr->check_hero_plan(hero_uid);
	}
	...
	EQUIP_LOG("role[%lu] equip_on[%lu:%d] hero[%d]", p_role->get_uid(), equip_uid, p_equip->get_tid(), p_hero->get_tid());
...
uint32_t equip_manager_t::equip_off(role_ptr p_role, uint64_t equip_uid, uint64_t hero_uid, proto::common::role_change_data* p_data)
{
	...
	if (p_equip->get_hero_uid() != hero_uid)
	{
		log_error("p_role[%lu] p_equip[%lu] not equip on hero[%lu]", p_role->get_uid(), equip_uid, hero_uid);
		return errcode_enum::notice_unknown;
	}
	auto p_equip_conf = GET_CONF(Equip, p_equip->get_tid());
	if (NULL == p_equip_conf)
	...
	{
		log_error("role[%lu] hero[%d] exist_equip_uid[%lu] != equip_uid[%lu]", p_role->get_uid(), p_hero->get_tid(), exist_equip_uid, equip_uid);
		return errcode_enum::notice_army_not_exist;
	}
	p_equip->set_hero_uid(0);
	p_equip->save_self(p_role->get_uid());
	...
	proto::common::hero_single* p_hero_single = p_hero_data->add_hero_list();
	p_hero->peek_data(p_hero_single);
	EQUIP_LOG("role[%lu] hero[%d] equip_off[%lu:%d] success", p_role->get_uid(), p_hero->get_tid(), equip_uid, p_equip->get_tid());
...
uint32_t equip_manager_t::remake_attr(role_ptr p_role, uint64_t equip_uid, uint32_t gold_flag, proto::common::role_change_data* p_data)
{
	...
	proto::common::personal_info* p_info = p_data->mutable_per_info();
	p_role->get_personal_info(p_info);
	EQUIP_LOG("role[%lu] remake_attr[%lu:%d] success", p_role->get_uid(), equip_uid, p_equip->get_tid());
...
uint32_t equip_manager_t::remake_exattr(role_ptr p_role, uint64_t equip_uid, const std::vector<uint32_t>& lock_vec, proto::common::role_change_data* p_data)
{
	...
	if (0 == p_equip->get_remain_remake_count())
	{
		log_error("role[%lu] p_equip[%lu] remain_remake_count not enough", p_role->get_uid(), equip_uid);
		return errcode_enum::notice_unknown;
	}
	auto p_equip_conf = GET_CONF(Equip, p_equip->get_tid());
	if (NULL == p_equip_conf)
	...
	{
		log_error("role[%lu] lock_vec_size[%d] invalid, >=[%d]", p_role->get_uid(), (int32_t)lock_vec.size(), p_equip_exattr_make_config->count());
		return errcode_enum::notice_unknown;
	}
	...
	{
		log_error("EquipExattrRemake[%d] FindCnf not found", (int32_t)lock_vec.size());
		return errcode_enum::notice_unknown;
	}
	...
	{
		log_error("p_role[%lu] equip_remake_exattr item is not enough!!", p_role->get_uid());
		return errcode_enum::notice_item_not_enough;
	}
	...
	{
		log_error("p_role[%lu] equip_remake_exattr copper[%d] not enough", p_role->get_uid(), cost_money);
		return errcode_enum::notice_copper_money_not_enough;
	}
	...
	p_equip->set_remain_remake_count(p_equip->get_remain_remake_count() - 1);
	...
	proto::common::personal_info* p_info = p_data->mutable_per_info();
	p_role->get_personal_info(p_info);
	EQUIP_LOG("role[%lu] remake_exattr[%lu:%d] success", p_role->get_uid(), equip_uid, p_equip->get_tid());
...
uint32_t equip_manager_t::replace_exattr(role_ptr p_role, uint64_t equip_uid, proto::common::role_change_data* p_data)
{
	...
	proto::common::item_single* p_item_single = p_item_data->add_item_list();
	p_equip->peek_data(p_item_single);
	EQUIP_LOG("role[%lu] replace_exattr[%lu:%d] success", p_role->get_uid(), equip_uid, p_equip->get_tid());
	return errcode_enum::error_ok;
}
...
uint32_t equip_manager_t::equip_smelt(role_ptr p_role, const std::vector<uint64_t>& equip_list, proto::common::role_change_data* p_data)
{
	...
	EQUIP_LOG("role[%lu] equip_smelt num[%d]", p_role->get_uid(), (int32_t)equip_list.size());
	...
	EQUIP_LOG("role[%lu] equip_smelt num[%d] success", p_role->get_uid(), (int32_t)equip_list.size());
	return errcode_enum::error_ok;
}
...
uint32_t equip_manager_t::equip_smelt_box(role_ptr p_role, uint32_t box_id, proto::common::role_change_data* p_data)
{
	...
	{
		log_error("role[%lu] equip_smelt_value[%d] not enough need[%d]", p_role->get_uid(), p_role->get_equip_smelt_value(), box_id);
		return errcode_enum::notice_unknown;
	}
	const std::vector<uint32_t>& equip_smelt_box_vec = p_role->get_equip_smelt_box_vec();
...
uint32_t equip_manager_t::equip_strengthen(role_ptr p_role, uint64_t equip_uid, uint32_t count, proto::common::role_change_data* p_data)
{
	...
	EQUIP_LOG("role[%lu] equip_strengthen[%lu:%d] count[%d]", p_role->get_uid(), equip_uid, p_equip->get_tid(), count);
	...
	if (real_count > 0)
	{
		...
		EQUIP_LOG("role[%lu] equip_strengthen[%lu:%d] real_count[%d] strengthen_level from[%d] to [%d]", 
...
uint32_t equip_manager_t::equip_soul(role_ptr p_role, uint64_t equip_uid, uint32_t soul_id, proto::common::role_change_data* p_data)
{
	if (NULL == p_role)
	{
		log_error("NULL == p_role");
	...
	Equip* p_equip_conf = GET_CONF(Equip, p_equip->get_tid());
	if (NULL == p_equip_conf)
	{
		log_error("NULL == p_equip_conf Equip[%d]", p_equip->get_tid());
		return errcode_enum::notice_item_null;
	}
	Item* p_equip_item_conf = GET_CONF(Item, p_equip->get_tid());
	if (NULL == p_equip_item_conf)
	{
		log_error("NULL == p_equip_item_conf Item[%d]", p_equip->get_tid());
		return errcode_enum::notice_item_null;
	}
	item_ptr p_soul = item_manager_t::get_item_by_tid(p_role, soul_id);
	if (NULL == p_soul)
	{
		log_error("role[%lu] soul[%d] not found", p_role->get_uid(), soul_id);
		return errcode_enum::notice_item_null;
	}
	EquipSoul* p_soul_conf = GET_CONF(EquipSoul, soul_id);
	if (NULL == p_soul_conf)
	{
		log_error("NULL == p_soul_conf EquipSoul[%d]", soul_id);
		return errcode_enum::notice_item_null;
	}
	Item* p_soul_item_conf = GET_CONF(Item, soul_id);
	if (NULL == p_soul_item_conf)
	{
		log_error("NULL == p_soul_item_conf Item[%d]", soul_id);
		return errcode_enum::notice_item_null;
	}
	if (p_soul_conf->equip_pos() != p_equip_conf->equip_type())
	{
		log_error("equip soul not match");
		return errcode_enum::notice_item_null;
	}
	EquipSpecial* p_special_conf = GET_CONF(EquipSpecial, p_soul_conf->special());
	if (NULL == p_special_conf)
	{
		log_error("NULL == p_special_conf EquipSpecial[%d]", p_soul_conf->special());
		return errcode_enum::notice_item_null;
	}
	if (p_soul_item_conf->quality() > p_equip_item_conf->quality())
	{
		log_error("quality not in range");
		return errcode_enum::zhuling_error_1;
	}
	proto::common::MONEY_TYPE money_type = (proto::common::MONEY_TYPE)p_soul_conf->money().first;
	uint32_t money_count = p_soul_conf->money().second;
	if (money_manager_t::get_money(p_role, money_type) < money_count)
	{
		log_error("role[%lu] soul[%d] money not enough", p_role->get_uid(), soul_id);
		return errcode_enum::notice_copper_money_not_enough;
	}
	if (errcode_enum::error_ok != item_manager_t::remove_item(p_role, soul_id, 1, log_enum::source_type_equip_soul, p_equip->get_tid(), p_data))
	{
		log_error("role[%lu] soul[%d] not exist", p_role->get_uid(), soul_id);
		return errcode_enum::notice_item_null;
	}
	money_manager_t::use_money(p_role, money_type, money_count, log_enum::source_type_equip_soul, p_equip->get_tid(), true, p_data);
	if (!random_util_t::selectByPercent(p_soul_conf->rate()))
	{
		return errcode_enum::zhuling_error_2;
	}
	uint32_t old_special = p_equip->get_special();
	p_equip->set_special(p_soul_conf->special());
	proto::common::item_data* p_item_data = p_data->mutable_item();
	proto::common::item_single* p_item_single = p_item_data->add_item_list();
	p_equip->peek_data(p_item_single);
	log_wrapper_t::send_equip_log(p_role->get_uid(), p_role->get_level(), p_equip, log_enum::opt_type_set, log_enum::source_type_equip_soul, 0);
	//auto p_task_mgr = p_role->get_task_mgr();
	//if (NULL != p_task_mgr)
	//{
	//	uint32_t equip_count = item_manager_t::get_equip_strengthen_count(p_role, 5);
	...
	//	p_task_mgr->try_change_task_state(TASK_TARGET_TYPE_EQUIP_STRENGTHEN_10, *p_data, 1, equip_count, false);
	//}
	EQUIP_LOG("role[%lu] equip_soul[%lu:%d] special from[%d] to [%d]",
		p_role->get_uid(), equip_uid, p_equip->get_tid(), old_special, p_equip->get_special());

	return errcode_enum::error_ok;
}

uint32_t equip_manager_t::equip_add_remake_count(role_ptr p_role, uint64_t equip_uid, proto::common::role_change_data* p_data)
{
	if (NULL == p_role)
	{
		log_error("NULL == p_role");
	...
	uint32_t add_remake_count = p_equip->get_add_remake_count();
	if (add_remake_count >= s_add_remake_info.size())
	{
		log_error("role[%lu] p_equip[%lu] add_remake_count max", p_role->get_uid(), equip_uid);
		return errcode_enum::notice_unknown;
	}
	uint32_t cost_item = s_add_remake_info[add_remake_count].first;
	uint32_t cost_num = s_add_remake_info[add_remake_count].second;
	uint32_t add_num = s_add_remake_info[add_remake_count].third;
	if (errcode_enum::error_ok != item_manager_t::remove_item(p_role, cost_item, cost_num, log_enum::source_type_equip_add_remake_count, p_equip->get_tid(), p_data))
	{
		log_error("role[%lu] p_equip[%lu] add_remake_count item not enough", p_role->get_uid(), equip_uid);
		return errcode_enum::notice_item_not_enough;
	}
	uint32_t old_remain_remake_count = p_equip->get_remain_remake_count();
	p_equip->set_remain_remake_count(old_remain_remake_count + add_num);
	p_equip->set_add_remake_count(add_remake_count + 1);
	proto::common::item_data* p_item_data = p_data->mutable_item();
	proto::common::item_single* p_item_single = p_item_data->add_item_list();
	p_equip->peek_data(p_item_single);
	//log_wrapper_t::send_equip_log(p_role->get_uid(), p_role->get_level(), p_equip, log_enum::opt_type_set, log_enum::source_type_equip_soul, 0);
	
	EQUIP_LOG("role[%lu] equip_add_remake_count[%lu:%d] remain_remake_count from[%d] to [%d], add_remake_count from[%d] to [%d]",
		p_role->get_uid(), equip_uid, p_equip->get_tid(), old_remain_remake_count, p_equip->get_remain_remake_count(), add_remake_count, p_equip->get_add_remake_count());
...
void equip_manager_t::make_attr(Equip* p_equip_conf, std::map<uint32_t, uint32_t>& attr_map)
{
	...
	attr_map.insert(std::make_pair(attrtype_enum::extra_atk, p_equip_conf->base_atk()));
	//EQUIP_LOG("atk[%d]", p_equip_conf->base_atk());
	attr_map.insert(std::make_pair(attrtype_enum::extra_def, p_equip_conf->base_def()));
	//EQUIP_LOG("def[%d]", p_equip_conf->base_def());
	attr_map.insert(std::make_pair(attrtype_enum::extra_hp, p_equip_conf->base_hp()));
...
uint32_t equip_manager_t::equip_strengthen_single(role_ptr p_role, item_ptr p_equip, proto::common::role_change_data* p_data)
{
	...
	if (strengthen_level >= p_role->get_level())
	{
		log_error("role[%lu] equip[%lu:%d] strengthen_level[%d] cannot pass role_level[%d]", p_role->get_uid(), p_equip->get_uid(),
			p_equip->get_tid(), strengthen_level, p_role->get_level());
		return errcode_enum::error_level_not_enough;
	}
	...
	if (!item_manager_t::has_items(p_role, item_id_counts))
	{
		log_error("p_role[%lu] equip_strengthen[%lu:%d] material not enough", p_role->get_uid(), p_equip->get_uid(), p_equip->get_tid());
		return errcode_enum::notice_item_not_enough;
	}
	...
	{
		log_error("p_role[%lu] equip_strengthen[%lu:%d] copper[%d] not enough", p_role->get_uid(), p_equip->get_uid(), p_equip->get_tid(), cost_money);
		return errcode_enum::notice_copper_money_not_enough;
}	
	...
	achieve_common_t::notify_progress_state(p_role->get_uid(), proto::common::ACHIEVE_STRENGTHEN_EQUIP_NUMBER, 0);
	return errcode_enum::error_ok;
}
...
uint32_t equip_manager_t::get_equip_remake_count()
{
	auto equip_remake_count = GET_CONF(Comprehensive, comprehensive_common::equip_change_max);
	uint32_t val = GET_COMPREHENSIVE_VALUE_1(equip_remake_count);
	return val;
}
...
void equip_manager_t::make_add_remake_info()
{
	auto p_add_remake_info = GET_CONF(Comprehensive, comprehensive_common::equip_change_add_item);
	if (NULL == p_add_remake_info)
	{
		log_error("NULL == p_add_remake_info make_add_remake_info");
		return;
	}
	if (!p_add_remake_info->parameter1().empty())
	{
		Dawn::parseTupleContainer<Dawn::triple<uint32_t, uint32_t, uint32_t>>(s_add_remake_info, p_add_remake_info->parameter1().c_str(), ":$");
	}
}
