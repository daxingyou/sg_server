...
#define ENTER_SCENE_DELAY_MSEC 500
...
public:
	...
	void set_login_state(const login_state_em login_state);
...
public:
	...
	bool move_arrive(float x, float z, float dist = 0.2f);
	void task_done();
	float get_currX();
	float get_currY();
...
public:
	...
	uint64_t get_monster_by_tid(uint32_t monster_tid);
...
private:
	...
	//-------------------世界boss-------------------
public:
	void request_boss_info(uint32_t id);
	void reply_boss_info(const proto::client::ec_field_boss_info_notify& ntf);
	uint32_t get_boss_monster_id() const { return m_filed_boss_data.monster_tid(); }
	uint32_t get_boss_map_id() const { return m_filed_boss_data.map_id(); }
	uint32_t get_boss_pos_x() const { return m_filed_boss_data.pos_x(); }
	uint32_t get_boss_pos_y() const { return m_filed_boss_data.pos_y(); }
	bool attack_field_boss(uint32_t monster_id);

	//召唤世界boss相关
	void set_field_boss_exist_flag(char flag);
	char get_field_boss_exist_flag();
	void clear_field_boss_exist_flag();
	uint32_t random_field_boss_id();

private:
	uint32_t m_boss_id = 0;
	proto::common::filed_boss_one_state m_filed_boss_data;
	char m_field_boss_exist_flag = -1;//-1未接受到反馈，0有boss，1没有boss
...
public:
	...
	bool is_arena_matched();

private:
	bool m_is_arena_matched;
	//-----------------------------------------------

	//-------------------商城购买--------------------
public:
	void buy_month_card(uint32_t id);

private:
	//-----------------------------------------------

public:
	uint64_t m_login_time_msec;

protected:
	uint64_t m_last_login_state_time;
	std::map<uint32_t, uint64_t> m_login_state_cost;
};
