...
void center_msg_handler_t::add_logs(uint16_t cmd, const std::string& msg_buf)
{
	...
	case op_cmd::eo_redbag_log_notify:
		...
		break;
	case op_cmd::eo_offline_arena_rank_log_notify:
	{
		PRE_S2LOG_MSG(proto::log::eo_offline_arena_rank_log_notify);
		offline_arena_rank_log_ptr new_log(new offline_arena_rank_log_t(msg.role_uid(), msg.role_name(), msg.role_level(), msg.op_type(),
			msg.old_class(), msg.old_rank(), msg.new_class(), msg.new_rank(), msg.log_time()));
		log.m_offline_arena_rank_log_mgr.add_log(new_log);
	}
	break;
    default:
        {
