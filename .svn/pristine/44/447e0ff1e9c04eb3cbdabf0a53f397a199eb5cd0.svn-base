...
uint32_t sevenday_login_t::get_sevenday_list_prize(uint32_t index, proto::client::gc_get_7d_login_prize_reply &reply)
{
	...
	drop_manager_t::drop(p_role, p_conf->drop_id(), log_enum::source_type_bussiness_seven_day_login, proto::common::drop_sys_type_sevenday_login, index, reply.mutable_rcd());
...
void sevenday_login_t::save_data()
{
	...
	peek_data(p_data);
	role_unify_save::add_task(m_role_uid, m_key, p_data);
}
...
//七日登录只能用是否领取所有奖励来判断是否活动中
bool sevenday_login_t::is_active()
{
	return ( is_get_all_prize() == false );
}

bool sevenday_login_t::is_get_all_prize()
{
	bool ret_flag = true;
	std::map<uint32_t, SevendayLogin*> sevenday_conf_map;
	GET_ALL_CONF(SevendayLogin, sevenday_conf_map);
	for (auto it : sevenday_conf_map)
	{
		if (get_prize_status(it.first) != 2)
		{
			ret_flag = false;
			break;
		}
...
bool sevenday_login_t::is_end_show_time()
{
	return true;
}
