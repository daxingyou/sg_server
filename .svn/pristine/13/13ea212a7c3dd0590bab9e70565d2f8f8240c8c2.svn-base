...
void item_t::load_data(const proto::common::item_single& single)
{
	...
	if (is_equip() && single.has_equip_info())
	{
		...
		{
			log_error("NULL == p_equip_conf Equip[%d]", m_tid);
			return;
		}
		m_attrs.insert(std::make_pair(attrtype_enum::extra_atk, p_equip_conf->base_atk()));
		m_attrs.insert(std::make_pair(attrtype_enum::extra_def, p_equip_conf->base_def()));
		m_attrs.insert(std::make_pair(attrtype_enum::extra_hp, p_equip_conf->base_hp()));
		...
		m_remain_remake_count = es.remain_remake_count();
		m_add_remake_count = es.add_remake_count();
		calc_score();
		calc_remake_score();
	}
	check_rarity();
...
void item_t::peek_data(proto::common::item_single* p_single)
{
	...
	if (p_single)
	{
		...
		if (is_equip())
		{
			...
			p_equip->set_remain_remake_count(m_remain_remake_count);
			p_equip->set_add_remake_count(m_add_remake_count);
		}
	}
}

void item_t::save_self(uint64_t role_uid)
{
    proto::common::item_single single;
	peek_data(&single);
	role_unify_save::add_task(role_uid, m_key, single, true);
}
