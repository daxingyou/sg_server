...
bool time_item_t::do_init(uint32_t start, uint32_t last, uint32_t interval, uint32_t time /*= 0*/)
{
	...
	if ((_start + _last) <= cur) {
		...
		if (_time < tt) {
			_start = time;	//mod by hy, 重启后，已经结束的时间仍然保留原始记录
			_time = 0;		//并且将_time 设置为0
		} else {
			_start += _interval * tt;
			_time -= tt;
		}
...
void time_item_t::timer(uint32_t cur)
{
	...
	if ((_start + _last) <= cur) {
		...
		//mod by hy, 如果活动结束了，就保留上次的时间。
		if (!_time) _start = old_time;
	}
...
uint32_t time_day_t::get_time_by_unique_id(uint32_t unique_id)
{
	uint32_t interval_time = _hour * 3600 + _min * 60 + _sec;

	return unique_id * DAY_SECOND + interval_time + time_util_t::get_timezone();
}
...
uint32_t time_week_t::get_time_by_unique_id(uint32_t unique_id)
{
	uint32_t interval_time = (_day - 1) * DAY_SECOND + _hour * 3600 + _min * 60 + _sec;

	return unique_id * WEEK_SECOND + interval_time + 4 * DAY_SECOND + time_util_t::get_timezone();
}
