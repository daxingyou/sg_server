...
void virtual_notice_manager_t::random_virtual_notice(proto::common::sys_notice_data* data)
{
	...
	int32_t random_id = random_util_t::randBetween(1, VirtualNoticeTableManager::getInstancePtr()->getSize());
    if (random_id < 0)
    {
        log_error("random_id[%d] error", VirtualNoticeTableManager::getInstancePtr()->getSize());
        return;
    }
