...
public:
	achieve_log_t(uint64_t role_uid, uint32_t achieve_id, uint32_t old_level,
		uint32_t up_level, uint32_t new_level,uint32_t old_count,uint32_t new_count,
		uint32_t old_achieve_point, uint32_t add_achieve_point, uint32_t new_achieve_point, 
		uint32_t old_achieve_level, uint32_t up_achieve_level, 	uint32_t new_achieve_level,
		uint32_t old_reward_get_level,uint32_t new_reward_get_level, uint32_t source_type, 
		uint32_t source_param,uint32_t log_time,const std::string& sourec_type_str)
		: m_role_uid(role_uid)
	...
	void update_to_db(const Connection_T& conn)
	{
		...
		PreparedStatement_setInt(pre, 7, m_new_count);
		PreparedStatement_setInt(pre, 8, m_old_achieve_point);
		PreparedStatement_setInt(pre, 9, m_add_achieve_point);
		PreparedStatement_setInt(pre, 10, m_new_achieve_point);
		PreparedStatement_setInt(pre, 11, m_old_achieve_level);
		PreparedStatement_setInt(pre, 12, m_up_achieve_level);
		PreparedStatement_setInt(pre, 13, m_new_achieve_level);
		PreparedStatement_setInt(pre, 14, m_old_reward_get_level);
		PreparedStatement_setInt(pre, 15, m_new_reward_get_level);
		PreparedStatement_setInt(pre, 16, m_source_type);
		PreparedStatement_setInt(pre, 17, m_source_param);
		PreparedStatement_setInt(pre, 18, m_log_time);
		PreparedStatement_setString(pre, 19, m_sourec_type_str.c_str());
