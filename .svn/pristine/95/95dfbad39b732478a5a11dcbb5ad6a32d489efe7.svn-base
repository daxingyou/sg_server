...
//---------------------------------------------------------------------------------------
std::size_t log_manager_t::m_thread_num;
std::size_t log_manager_t::m_thread_step = 0;
...
bool log_manager_t::init(uint32_t thread_num, uint32_t update_to_db_time)
{
...
{
    m_ios_list[m_thread_step++ % m_thread_num]->post(boost::bind(&game_msg_handler_t::handle_server_msg, s,uid, cmd, buf));
}

void log_manager_t::post_center_message(const tcp_socket_ptr& s, uint16_t cmd, const msg_buf_ptr& buf, uint64_t uid)
{
    m_ios_list[m_thread_step++ % m_thread_num]->post(boost::bind(&center_msg_handler_t::handle_server_msg, s,uid, cmd, buf));
}

void log_manager_t::post_account_message(const tcp_socket_ptr& s, uint16_t cmd, const msg_buf_ptr& buf, uint64_t uid)
{
    m_ios_list[m_thread_step++ % m_thread_num]->post(boost::bind(&account_msg_handler_t::handle_server_msg, s, cmd, buf));
}