...
{
    return `ps -ef | grep -w $1 | grep -w $2 | grep -v "grep" | wc -l `
}
function is_server_running()
{
    ...
    elif [ "$1" = "account" -o "$1" = "glog" -o "$1" = "area" -o "$1" = "video" ];then
...
function update_global_server_xml()
{
    ...
    sh set_xml_value.sh   server.start_time $start_time $TMP_XML_DST
    ...
    sh set_xml_value.sh   server.url.login_list_url $login_list_url $TMP_XML_DST
    sh set_xml_value.sh   server.url.sdk_login_url $sdk_login_url $TMP_XML_DST
    ...
    sh set_xml_value.sh   server.account_server.mysql.db    $account_db_name      $TMP_XML_DST
    sh set_xml_value.sh   server.account_server.mysql.user  $account_db_user    $TMP_XML_DST
    sh set_xml_value.sh   server.account_server.mysql.pwd   $account_db_pwd     $TMP_XML_DST
    ...
    sh set_xml_value.sh   server.area_server.mysql.db         	$area_db_name $TMP_XML_DST
    sh set_xml_value.sh   server.area_server.redis.host         $area_redis_ip  $TMP_XML_DST
    sh set_xml_value.sh   server.area_server.redis.pwd         	$area_redis_pwd  $TMP_XML_DST
    ...
    sh set_xml_value.sh   server.glog_server.mysql.db  		    $glog_db_name    		$TMP_XML_DST
    sh set_xml_value.sh   server.glog_server.mysql.user  		$glog_db_user    		$TMP_XML_DST
    sh set_xml_value.sh   server.glog_server.mysql.pwd   		$glog_db_pwd     		$TMP_XML_DST
    
    #修改video_server节点
    sh set_xml_value.sh   server.video_server.listen.ip             $video_server_listen_ip $TMP_XML_DST
    sh set_xml_value.sh   server.video_server.mysql.host            $video_db_ip            $TMP_XML_DST
    sh set_xml_value.sh   server.video_server.mysql.db         	    $video_db_name          $TMP_XML_DST
    sh set_xml_value.sh   server.video_server.redis.host            $video_redis_ip         $TMP_XML_DST
    sh set_xml_value.sh   server.video_server.redis.pwd         	$video_redis_pwd        $TMP_XML_DST
    sh set_xml_value.sh   server.video_server.cross_arena.db        $video_redis_db_1       $TMP_XML_DST
...
function update_server_xml()
{
    ...
    sh set_xml_value.sh  server.url.cdkey_url                       $cdkey_url                      $TMP_XML_DST 
    ...
    sh set_xml_value.sh  server.login_server.mysql.db               $game_db_name                    $TMP_XML_DST 
    ...
    sh set_xml_value.sh  server.db_server.mysql.db                  $game_db_name                    $TMP_XML_DST 
    sh set_xml_value.sh  server.db_server.mysql.user                $game_db_user                   $TMP_XML_DST 
    sh set_xml_value.sh  server.db_server.mysql.pwd                	$game_db_pwd                    $TMP_XML_DST
    sh set_xml_value.sh  server.db_server.redis.host                $game_redis_ip                  $TMP_XML_DST 
    sh set_xml_value.sh  server.db_server.redis.pwd                	$game_redis_pwd                 $TMP_XML_DST 
    ...
    sh set_xml_value.sh  server.center_server.mysql.db              $game_db_name                       $TMP_XML_DST 
    sh set_xml_value.sh  server.center_server.redis.host            $game_redis_ip                      $TMP_XML_DST 
    sh set_xml_value.sh  server.center_server.redis.pwd             $game_redis_pwd                     $TMP_XML_DST 
    ...
    sh set_xml_value.sh  server.chat_server.connect.video.ip        $chat_connect_to_video_ip       $TMP_XML_DST 
    sh set_xml_value.sh  server.chat_server.connect.video.port      $chat_connect_to_video_port     $TMP_XML_DST 
    ...
    sh set_xml_value.sh  server.log_server.mysql.db             $log_db_name                    $TMP_XML_DST 
	sh set_xml_value.sh  server.log_server.mysql.user           $log_db_user                   	$TMP_XML_DST 
    sh set_xml_value.sh  server.log_server.mysql.pwd            $log_db_pwd                    	$TMP_XML_DST

    # gm
    sh set_xml_value.sh  server.gm_server.listen.gmtool.ip          $server_ip                  $TMP_XML_DST 
    sh set_xml_value.sh  server.gm_server.listen.gmtool.out_ip      $server_out_ip              $TMP_XML_DST 
    sh set_xml_value.sh  server.gm_server.mysql.host                $gm_db_ip                   $TMP_XML_DST 
    sh set_xml_value.sh  server.gm_server.mysql.db                  $gm_db_name                 $TMP_XML_DST 
	sh set_xml_value.sh  server.gm_server.mysql.user                $gm_db_user                 $TMP_XML_DST 
    sh set_xml_value.sh  server.gm_server.mysql.pwd                 $gm_db_pwd                  $TMP_XML_DST
    sh set_xml_value.sh  server.gm_server.redis.host                $game_redis_ip              $TMP_XML_DST 
    sh set_xml_value.sh  server.gm_server.redis.port                $game_redis_port            $TMP_XML_DST 
	sh set_xml_value.sh  server.gm_server.redis.pwd                 $game_redis_pwd             $TMP_XML_DST 
    sh set_xml_value.sh  server.gm_server.redis.db                  $game_redis_db              $TMP_XML_DST
    ...
    # gm
    local out_port=$( sh get_xml_value.sh  server.gm_server.listen.port  $TMP_XML_DST )
    sh set_xml_value.sh  server.gm_server.listen.out_port $out_port  $TMP_XML_DST
...
function update_sql()
{
    ...
    deassign    config.py   con_accountdb   [\"$cnf_account_db_ip\",\"$cnf_account_db_user\",\"$cnf_account_db_pwd\",\"$cnf_account_db_name\",\"sg_account.sql\",\"$server_id\"\] 0
    deassign    config.py   con_areadb      [\"$cnf_area_db_ip\",\"$cnf_area_db_user\",\"$cnf_area_db_pwd\",\"$cnf_area_db_name\",\"sg_area.sql\",\"$server_id\"\] 0
    
    deassign    config.py   accountdb    [\"$account_db_ip\",\"$account_db_user\",\"$account_db_pwd\",\"$account_db_name\",\"sg_account.sql\",\"$server_id\"\] 0
    deassign    config.py   areadb       [\"$area_db_ip\",\"$area_db_user\",\"$area_db_pwd\",\"$area_db_name\",\"sg_area.sql\"] 0
    deassign    config.py   global_redis [\"$area_redis_ip\",\"$area_redis_port\",\"$area_redis_pwd\",\"$area_redis_db\"] 0
    deassign    config.py   glogdb       [\"$glog_db_ip\",\"$glog_db_user\",\"$glog_db_pwd\",\"$glog_db_name\",\"sg_glog.sql\"] 0
    deassign    config.py   videodb      [\"$video_db_ip\",\"$video_db_user\",\"$video_db_pwd\",\"$video_db_name\",\"sg_video.sql\"\]   0
    
    deassign    config.py   gamedb       [\"$game_db_ip\",\"$game_db_user\",\"$game_db_pwd\",\"$game_db_name\",\"sg_game.sql\"]   0
    deassign    config.py   game_redis   [\"$game_redis_ip\",\"$game_redis_port\",\"$game_redis_pwd\",\"$game_redis_db\"] 0
    deassign    config.py   logdb        [\"$log_db_ip\",\"$log_db_user\",\"$log_db_pwd\",\"$log_db_name\",\"sg_log.sql\"\]   0
    
    deassign    config.py   bbsdb        [\"$bbs_db_ip\",\"$bbs_db_user\",\"$bbs_db_pwd\",\"$bbs_db_name\",\"sg_bbs.sql\"\]   0
    deassign    config.py   updb        [\"$up_db_ip\",\"$up_db_user\",\"$up_db_pwd\",\"$up_db_name\",\"sg_up.sql\"\]   0
    deassign    config.py   ssdb        [\"$ss_db_ip\",\"$ss_db_user\",\"$ss_db_pwd\",\"$ss_db_name\",\"sg_statistics.sql\"\]   0
    deassign    config.py   gmdb        [\"$gm_db_ip\",\"$gm_db_user\",\"$gm_db_pwd\",\"$gm_db_name\",\"sg_gm.sql\"\]   0

    if [ "$1" = "account" ]; then
        echo -e "\033[1;36m<sg_account>\033[0m"
        echo -e "\033[1;36m   mysql_ip: $account_db_ip\033[0m"
        echo -e "\033[1;36m   mysql_db: $account_db_name\033[0m"
        echo -e "\033[1;36m<sg_glog>\033[0m"
        echo -e "\033[1;36m   mysql_ip: $glog_db_ip\033[0m"
        echo -e "\033[1;36m   mysql_db: $glog_db_name\033[0m"
        
    elif [ "$1" = "area" ]; then
        echo -e "\033[1;36m<sg_area>\033[0m"
        echo -e "\033[1;36m   mysql_ip: $area_db_ip\033[0m"
        echo -e "\033[1;36m   mysql_db: $area_db_name\033[0m"
        echo -e "\033[1;36m   redis_ip: $area_redis_ip\033[0m"
        echo -e "\033[1;36m   redis_db: $area_redis_db\033[0m"
        echo -e "\033[1;36m<sg_bbs>\033[0m"
        echo -e "\033[1;36m   mysql_ip: $bbs_db_ip\033[0m"
        echo -e "\033[1;36m   mysql_db: $bbs_db_name\033[0m"
        ...
        echo -e "\033[1;36m<sg_video>\033[0m"
        echo -e "\033[1;36m   mysql_ip: $video_db_ip\033[0m"
        echo -e "\033[1;36m   mysql_db: $video_db_name\033[0m"
        
    elif [ "$1" = "game" ]; then
        echo -e "\033[1;36m<sg_game>\033[0m"
        echo -e "\033[1;36m   account:  $cnf_account_db_ip[$cnf_account_db_name]: server_id($server_id)\033[0m"
        echo -e "\033[1;36m   area:     $cnf_area_db_ip[$cnf_area_db_name]: server_id($server_id)\033[0m"
        echo -e "\033[1;36m   mysql_ip: $game_db_ip\033[0m"
        echo -e "\033[1;36m   mysql_db: $game_db_name\033[0m"
        echo -e "\033[1;36m   redis_ip: $game_redis_ip\033[0m"
        echo -e "\033[1;36m   redis_db: $game_redis_db\033[0m"
        echo -e "\033[1;36m<sg_log>\033[0m"
        echo -e "\033[1;36m   mysql_ip: $log_db_ip\033[0m"
        echo -e "\033[1;36m   mysql_db: $log_db_name\033[0m"
        ...
        echo -e "\033[1;36m   mysql_ip: $up_db_ip\033[0m"
        echo -e "\033[1;36m   mysql_db: $up_db_name\033[0m"
    
    elif [ "$1" = "ss" ]; then
        echo -e "\033[1;36m   mysql_ip: $ss_db_ip\033[0m"
        echo -e "\033[1;36m   mysql_db: $ss_db_name\033[0m"
        echo -e "\033[1;36m   sg_glog:  $glog_db_name\033[0m"

    elif [ "$1" = "gm" ]; then
        echo -e "\033[1;36m   mysql_ip: $gm_db_ip\033[0m"
        echo -e "\033[1;36m   mysql_db: $gm_db_name\033[0m"
    ...
    if [ "$2" = "d" ]; then
        ...
        do
            ...
            if [ "$answer" = Yes ];then
                if [ "$1" = "account" ]; then
                    python database.py d account glog
                elif [ "$1" = "area" ]; then
                    python database.py d area bbs
                elif [ "$1" = "glog" ]; then
                    python database.py d glog
                elif [ "$1" = "video" ]; then
                    python database.py d video    
                elif [ "$1" = "game" ]; then
        ...
        if [ "$1" = "account" ]; then
            ...
            python database.py i video
            python database.py u video
            ...
            sed -i 's/wai_trunk_sg_glog/'$glog_db_name'/g' sg_statistics.sql
            cat sg_statistics.sql | grep $glog_db_name
            python database.py i ss
            python database.py u ss

        elif [ "$1" = "gm" ]; then
            python database.py i gm
            python database.py u gm
        fi
:<<!
        while [ true ]
        do
            ...
            if [ "$answer" = Yes ];then
                if [ "$1" = "account" ]; then
                    python database.py r account glog
                elif [ "$1" = "area" ]; then
                    python database.py r area
                elif [ "$1" = "glog" ]; then
                    python database.py r glog
                elif [ "$1" = "video" ]; then
                    python database.py r video    
                elif [ "$1" = "game" ]; then
...
###################################################################################################
echo "[`date '+%Y-%m-%d %H:%M:%S'`] sh update.sh $1 $2 $3 $4" >> update.log
is_make=false
if [ "$1" = "xml" ];then
    is_server_running 'all' 'account' 'glog'
    if [ $? -ne 0 ];then
        exit
    fi
    echo "============================================================================="
