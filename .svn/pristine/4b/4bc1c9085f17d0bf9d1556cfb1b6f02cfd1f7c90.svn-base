...
bool role_msg_handle_t::init_msg_handler()
{
	...
	gate_msg_handler_t::m_gate_msg_handle_from_client.register_func(op_cmd::cg_handbook_award_request, handle_cg_handbook_award_request);
...
bool role_msg_handle_t::handle_cg_handbook_award_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::client::cg_handbook_award_request);
	role_ptr p_role = role_manager_t::find_role(uid);
	if (p_role == NULL)
	{
		log_error("p_role is null [%lu]", uid);
	...
	proto::client::gc_handbook_award_reply reply;
	uint32_t reply_code = p_role->handbook_award(msg.handbook_id(), reply);
	reply.set_reply_code(reply_code);
	p_role->send_msg_to_client(op_cmd::gc_handbook_award_reply, reply);
	return true;
}
