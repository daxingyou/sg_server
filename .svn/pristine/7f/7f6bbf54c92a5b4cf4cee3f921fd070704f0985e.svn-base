...
bool account_server_t::load_global_server_xml()
{
    try
    {
		...
		env::xml->web_server_url = pt.get<std::string>("server.web_server_url");
...
bool account_server_t::load_channel_xml()
{
    try
    {
        ...
        for (uint32_t i = 0; i < env::xml->srv_group.channel_id_list.size(); ++i)
        {
            ...
            p_channel->m_game_name = pt.get<std::string>(path + ".game_name");
            p_channel->m_login_url = pt.get<std::string>(path + ".login_url");
            p_channel->m_login_sk = pt.get<std::string>(path + ".login_sk");
            p_channel->m_recharge_sk = pt.get<std::string>(path + ".recharge_sk");
