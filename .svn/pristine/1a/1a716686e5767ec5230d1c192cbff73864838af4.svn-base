...
void arena_manager_t::leave_arena(const role_ptr& p_role)
{
	...
	do
	{
		...
		scene_manager_t::role_jump_scene(p_role, proto::server::user_change_scene, last_map_tid, p_role->get_role_scene_info().get_last_pos_x(), p_role->get_role_scene_info().get_last_pos_y(), p_role->get_role_scene_info().get_last_dir());
		return;
	} while (0);
...
void arena_manager_t::deal_arena_rank_battle_result(role_ptr p_role, proto::server::rg_arena_battle_result_notify& msg)
{
	...
	if (msg.is_win())
	{
		...
		money_manager_t::add_money(p_role, proto::common::MONEY_TYPE_HONOR, p_arena_class_conf->win_honor(),
			log_enum::source_type_arena_battle_win, 0, true, msg_reply.mutable_rcd());
		...
		achieve_common_t::notify_progress_state(p_role->get_uid(),proto::common::ACHIEVE_ARENA_BATTLE_TOTAL_WIN_NUMBER);
	}
	else
	{
