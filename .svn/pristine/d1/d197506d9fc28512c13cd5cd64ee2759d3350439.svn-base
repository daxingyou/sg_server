...
void offline_arena_manager_t::change_rank(offline_arena_ptr p_arena, uint32_t cur_class, uint32_t rank)
{
	if (NULL != p_arena)
	{
		...
		OFFLINE_ARENA_LOG("role[%lu] occupy class[%d] rank[%d]", p_arena->get_uid(), cur_class, rank);
	}
	else
	{
...
void offline_arena_manager_t::peek_data(uint64_t role_uid, proto::client::ec_offline_arena_data_reply& reply)
{
	...
	for (auto sub_vec : p_match_conf->enemy())
	{
		if (sub_vec.size() < 2)
		{
			log_error("p_match_conf[%d] enemy invalid");
			continue;
		}
		uint32_t rank = 0;
...
uint32_t offline_arena_manager_t::fight(uint64_t self_uid, uint32_t self_rank, uint32_t enemy_type, uint64_t enemy_id, uint32_t enemy_rank)
{
	...
	else if (enemy_type == proto::common::SCENEOBJECT_MONSTER)
	{
		...
		{
			log_error("role[%lu] fight robot failed, robot[%d] rank[%d] not match", self_uid, robot_id, enemy_rank);
			return errcode_enum::notice_unknown;
		}
		//再查找对应排名是否已经存在玩家
...
void offline_arena_manager_t::fight_result(uint64_t role_uid, uint32_t is_win)
{
	...
	do 
	{
		...
		if (enemy_type == proto::common::SCENEOBJECT_USER)
		{
			...
			p_self_record->set_enemy_plugin(global_user_data_mgr_t::get_user_hero_plugin(enemy_id));
			...
			p_enemy_record->set_enemy_plugin(global_user_data_mgr_t::get_user_hero_plugin(role_uid));
		...
		else if (enemy_type == proto::common::SCENEOBJECT_MONSTER)
		{
			...
			OfflineRobotInfo *p_robot_info = GET_CONF(OfflineRobotInfo, hash_util_t::hash_to_uint32(cur_class, p_arena->get_cur_rank()));
			if (NULL == p_robot_info)
			{
				log_error("p_robot_info NULL cur_class[%d] rank[%d]", cur_class, p_arena->get_cur_rank());
				reply.set_reply_code(errcode_enum::notice_unknown);
				break;
			}
		...
		{
			log_error("role[%lu] fight failed, enemy_type[%d] invalid", enemy_type);
			reply.set_reply_code(errcode_enum::notice_unknown);
		}
		break;
	} while (true);
...
void offline_arena_manager_t::form_notify(uint64_t role_uid, const proto::common::mirror_fight_data& form_data)
{
	if (m_offline_arena_map.find(role_uid) == m_offline_arena_map.end())
	{
		log_error("role[%lu] not in offline_arena", role_uid);
		return;
	}
	redis_client_t::set_protobuf("offline_arena", "fight_data", role_uid, form_data);
	OFFLINE_ARENA_LOG("role[%lu] form_notify ", role_uid);
}
...
void offline_arena_manager_t::peek_general_data(uint32_t cur_class, uint32_t rank, proto::common::offline_arena_general* p_single)
{
	...
	p_single->set_fighting(global_user_data_mgr_t::get_user_fighting_value(role_uid));
	p_single->set_plugin(global_user_data_mgr_t::get_user_hero_plugin(role_uid));
