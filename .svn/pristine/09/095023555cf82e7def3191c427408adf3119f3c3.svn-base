...
enum shop_effect
{
	shop_effect_hp = 1,		//回复血量（不能复活）
	shop_effect_cd = 2,		//清除cd
	shop_effect_all = 3,	//回复状态（血量CD全部回复并且可以复活）
};
...
public:
	...
	//购买
	uint32_t shop_request(uint32_t shop_id, uint64_t hero_uid, uint32_t buy_style, proto::client::gc_expedition_shop_reply& reply);
	//领取奖励
...
private:
	...
	uint32_t m_glory = 0;
	uint32_t m_shop_count[shop_effect_all] = {0};
