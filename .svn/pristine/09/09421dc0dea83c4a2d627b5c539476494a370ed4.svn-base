...
bool chat_msg_handler_t::handle_ce_personal_chat_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	...
	{
		log_error("role[%lu] msg length[%d] error!", uid, msg.chat_info().length());
	...
	{
		reply_msg.set_reply_code(errcode_enum::friend_chat_black_other);
		p_src_user->send_msg_to_client(op_cmd::ec_personal_chat_reply, reply_msg);
		return true;
	}
	...
	{
		reply_msg.set_reply_code(errcode_enum::friend_chat_black);
		p_src_user->send_msg_to_client(op_cmd::ec_personal_chat_reply, reply_msg);
		return true;
	}
