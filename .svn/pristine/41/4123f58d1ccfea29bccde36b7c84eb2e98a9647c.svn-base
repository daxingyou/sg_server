...
void achieve_t::save_self(uint64_t role_uid, bool is_new )
{
...
void achieve_t::split_achieve_finish_info(const std::string &str, std::vector<proto::common::achieve_finish_info_single>& list)
{
	...
	if (strvec.empty())
	{
		log_error("strvec.empty() m_role_uid[%lu] achieve_id[%d] achieve_level str[%s]",m_role_uid,m_tid,m_level,str.c_str());
		return;
	}
