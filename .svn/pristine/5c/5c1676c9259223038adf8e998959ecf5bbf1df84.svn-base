...
uint32_t pk_manager_t::do_pk(role_ptr p_role, role_ptr p_target, proto::common::role_change_data* p_data)
{
	...
	{
		log_error("p_target[%lu] is more than[%d] too far", p_target->get_uid(), get_pk_distance());
		return errcode_enum::notice_fight_target_too_farawary;
	}
	if (is_safe(p_target))
	{
		log_error("p_target[%lu] is safe", p_target->get_uid());
		return errcode_enum::pk_cue_1;
	}
	if (is_protected(p_role))
	{
		log_error("p_role[%lu] level[%d] is protected", p_role->get_uid(), p_role->get_level());
	...
	case proto::common::PK_PEACE:
		{
			scene_ptr p_scene = scene_manager_t::find_scene(p_role->get_scene_id());
			if (!p_scene) {
			...
			if (p_scene->is_king_war_city()) {
				king_war_scene_ptr p_king_war_scene = p_scene->get_king_war_scene_ptr();
				if (p_king_war_scene && p_king_war_scene->get_king_war_state() >= proto::common::king_war_state_prepare) {
					log_error("role[%lu] scene king war start, can't pk", p_role->get_uid());
					return errcode_enum::king_war_error_pk_fight;
				}
			}
			...
			if (pk_count == 0)
			{
				log_error("p_role[%lu] pk_count is limit", p_role->get_uid());
			...
			{
				log_error("p_role[%lu] level[%d] p_target[%lu] level[%d] delta_level too large", p_role->get_uid(), p_role->get_level(), p_target->get_uid(), p_target->get_level());
				return errcode_enum::pk_cue_3;
			}
			if (is_protected(p_target))
			{
				log_error("p_target[%lu] level[%d] is protected", p_target->get_uid(), p_target->get_level());
				return errcode_enum::pk_cue_4;
			}
			if (p_target->get_pk_value() == 0)
			{
				...
				{
					log_error("p_role[%lu] pk item not enough", p_role->get_uid());
	...
	case proto::common::PK_FAMILY:
		{
			...
			{
				log_error("p_role[%lu] and  p_target[%lu] same family", p_role->get_uid(), p_target->get_uid());
				return errcode_enum::notice_unknown;
			}
	...
	case proto::common::PK_CAMP:
		{
			if (p_role->get_pk_mode_ex() == p_target->get_pk_mode_ex())
			{
				log_error("p_role[%lu] and p_target[%lu] same camp", p_role->get_uid(), p_target->get_uid());
				return errcode_enum::pk_king_war_help_family;
			}
		}
		break;
	case proto::common::PK_COUNTRY:
		{
			...
			{
				log_error("p_role[%lu] and  p_target[%lu] same country", p_role->get_uid(), p_target->get_uid());
				return errcode_enum::notice_unknown;
			}
		}
		break;
...
bool pk_manager_t::attacker_pk_result(uint64_t self_uid, uint64_t enemy_uid, bool is_win, proto::common::role_change_data* p_data)
{
	...
	if (NULL == p_self)
	{
		log_error("NULL == p_self[%lu]", self_uid);
		return false;
	}
	...
	case proto::common::PK_PEACE:
		{
			if (is_win)
			{
				PK_LOG("attacker_pk_result win self[%lu] enemy[%lu]", self_uid, enemy_uid);
			...
			{
				PK_LOG("attacker_pk_result lose self[%lu] enemy[%lu]", self_uid, enemy_uid);
	...
	case proto::common::PK_PEACE:
		{
			...
			{
				//胜利
				PK_LOG("defenser_pk_result win self[%lu] enemy[%lu]", self_uid, enemy_uid);
			}
			else
			{
				PK_LOG("defenser_pk_result lose self[%lu] enemy[%lu]", self_uid, enemy_uid);
				//失败
				uint32_t self_pk_value = p_self->get_calc_pk_value();
...
uint32_t pk_manager_t::do_challenge(role_ptr p_role, uint64_t target_uid)
{
	...
	{
		log_error("role[%lu] gold not enough", p_role->get_uid());
