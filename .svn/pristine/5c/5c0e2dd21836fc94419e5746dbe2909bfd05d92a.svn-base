...
void robot_manager_t::on_robot_enter_success(uint64_t robot_id, uint64_t uid, uint64_t login_start_time)
{
    ...
    log_info("-----------The success enter game robots[%lu] num: %d/%d----------", uid, cur_enter_nums, env::robot_config->number);
...
bool robot_manager_t::init_one_robot(uint32_t uid)
{
	...
	if (NULL == p_robot) {
		log_error("create robot[%lu] failed!", uid);
		return false;
	}
...
bool robot_manager_t::start_up_robot(const robot_ptr& p_robot)
{
	...
	//log_debug("robot[%lu] connect to account_server[%s:%d]...", robot->get_robot_id(), env::robot_config->account_server_ip.c_str(), env::robot_config->account_server_port);
