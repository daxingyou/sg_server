...
{
#ifdef _PLATFORM_X64
	static const int          FormatCheckCode = 0x04378A69; ///< I|VVIVIIVVBBIBIVIIVIIIIIII
#else
	static const int          FormatCheckCode = 0x0BD61479; ///< I|vvIvIIvvBBIBIvIIvIIIIIII
#endif
	...
	uint32_t                  buff;                         ///< 增益奖励
	uint32_t                  mail;                         ///< 邮件发放
	uint32_t                  one_key_cost_num;             ///< 一键完成消耗道具数量
	uint32_t                  task_buff;                    ///< 增益发放
	uint32_t                  office_exp;                   ///< 奖励逆命师经验
};
#pragma pack()
...
struct Task : public Entry
{
	...
	uint32_t                  one_key_cost_num()            { return t_one_key_cost_num; }
	uint32_t                  task_buff() { return t_task_buff; }
	uint32_t                  office_exp() { return t_office_exp; }
...
private:
	...
	uint32_t                  t_one_key_cost_num;           ///< 一键完成消耗道具数量
	uint32_t                  t_task_buff;                  ///< 增益发放
	uint32_t                  t_office_exp;                 ///< 奖励逆命师经验
...
public:
	...
	//只返回第一个
	uint32_t get_target_cond(uint32_t type)
	{
		for (auto ptr : target_type_id_cnt)
		{
			if (std::get<0>(ptr) == type )
			{
...
inline void Task::fill( const TaskBase& base )
{
	...
	this->t_one_key_cost_num = base.one_key_cost_num;
	this->t_task_buff = base.task_buff;
	this->t_office_exp = base.office_exp;
};
