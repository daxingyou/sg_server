...
object_position_t::object_position_t()
{
	
}

object_position_t::object_position_t(uint64_t unique_id) : m_object_id(unique_id)
{
}
...
void object_position_t::init(uint64_t unique_id)
{
	m_object_id = unique_id;
}

// 是否在地图的某个点附近
bool object_position_t::is_nearby(uint32_t map_id, float x, float y, float r)
{
	if (m_map_tid == map_id && 
...
// 是在某个点附近
bool object_position_t::is_nearby_point(std::pair<float, float> point, float r)
{
	return (point.first - m_current_x) * (point.first - m_current_x) + (point.second - m_current_y) * (point.second - m_current_y) <= r * r;
}

// 是在某个点附近
bool object_position_t::is_nearby_point(std::pair<uint32_t, uint32_t> point, uint32_t r)
{
	return (point.first - m_current_x) * (point.first - m_current_x) + (point.second - m_current_y) * (point.second - m_current_y) <= r * r;
}

// 是否是附近的生物
bool object_position_t::is_nearby_object(object_position_ptr p_target, float radius)
{
	if (NULL == p_target)
	{
...
// 获取附近的生物信息（可以只获取玩家）
bool object_position_t::get_nearby_objects(std::vector<object_id_type>& objects_nearby, bool role_only/* = false*/, float distance/* = 0*/)
{
...
// 设置对象场景属性
void object_position_t::get_object_info(proto::common::object_common_info* object_info)
{
	if (NULL == object_info)
	{
...
void object_position_t::get_object_pos_info(proto::client::gc_move_notify* pos_info)
{
	if (NULL == pos_info)
	{
		log_error("pos_info null error");
		return;
	}

	pos_info->set_object_id(m_object_id);
	pos_info->set_object_type(uint32_t(get_object_type()));
    pos_info->set_speed(m_real_speed);
    pos_info->set_pos_x(m_current_x);
    pos_info->set_pos_y(m_current_y);
	pos_info->set_direction(m_direction);
    //pos_info->set_time_stamp(m_time_stamp);
}

void object_position_t::set_object_born(uint64_t father /*= 0*/)
{
	on_object_born();
}

void object_position_t::on_object_born()
{
	// 设置
	set_object_state(proto::common::object_state_normal);
}

void object_position_t::set_object_die(uint64_t obj_killer)
{
	on_object_die();
}

void object_position_t::on_object_die()
{
	// 状态设置为死亡
	set_object_state(proto::common::object_state_death);
}

void object_position_t::set_object_revive(uint64_t obj_god /*= 0*/)
{
	on_object_revive();
}

void object_position_t::on_object_revive()
{
	// 设置
	set_object_state(proto::common::object_state_normal);
}

void object_position_t::send_msg_to_nine(uint16_t cmd, const std::string& cmd_data)
{
	const scene_ptr& p_scene = scene_manager_t::find_scene(m_scene_id);
	if (NULL == p_scene)
	{
...
void object_position_t::leave_scene()
{
