#ifndef __LOG_MONEY_LOG_HPP__
#define __LOG_MONEY_LOG_HPP__
...
{
public:
	money_log_t(uint64_t role_uid, uint32_t role_level, uint32_t old_num, uint32_t up_num, uint32_t new_num,
	...
	void update_to_db(const Connection_T& conn)
	{
		...
		PreparedStatement_setInt(pre,	 6, m_money_type);
		...
		PreparedStatement_execute(pre);
			...
			CATCH(SQLException)
		{
			log_error("[wys] user[%lu] insert money log[%d] to db error: %s", m_role_uid, m_source_type, Exception_frame.message);
		}
		FINALLY
		{
		}
...
private:
	...
	uint32_t            m_up_num;
	uint32_t            m_new_num;
	uint32_t            m_money_type;
	uint32_t            m_op_type;
	uint32_t            m_source_type;
	uint32_t            m_source_param;
	uint32_t            m_log_time;
};
typedef boost::shared_ptr<money_log_t> money_log_ptr;

///////////////////////////////////////////////////////////////////////////////////////////////////////
class money_log_manager_t
{
public:
	void add_log(money_log_ptr log_ptr)
	{
...
private:
	typedef std::vector<money_log_ptr> money_log_vec_type;
	money_log_vec_type m_logs_list;
};
