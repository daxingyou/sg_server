...
bool server_info_manager_t::load_login_server_list_from_web(std::string &server_list_type, std::string  json_data)
{
	...
	{
		log_error("parse json[%s] failed, error[%s]", json_data.c_str(), e.what());
		return false;
	}
...
bool server_info_manager_t::get_login_server_info_requrest(std::string &server_list_type) 
{
	...
	sprintf(url, "%s/login_version.php?server_list_type=%s", env::xml->url.login_list_url.c_str(), server_list_type.c_str());
...
bool server_info_manager_t::get_commpatible_info_request(std::string &server_list_type)
{
	char url[512];
	memset(url, 0, sizeof(url));
	sprintf(url, "%s/compatile_version.php?server_list_type=%s", env::xml->url.login_list_url.c_str(), server_list_type.c_str());
	log_info("get url:=%s", url);
	web_curl_wapper_ptr p_curl(new web_curl_wapper_t());
	if (!p_curl->init(url))
	{
...
bool server_info_manager_t::load_server_info() 
{
	...
	log_info("m_channel_list:%d", (int32_t)channel_manager_t::m_channel_list.size());
	return true;
}
...
bool server_info_manager_t::is_commpatible_version(const std::string &server_list_type, std::string version)
{
	...
	{
		log_error("check compatiable_version mark_type[%s], version[%s], version_map_size[%lu]", server_list_type.c_str(), version.c_str() , m_compatible_map.size() );
		return false;
	}
	...
	{
		log_error("can't find mark_type[%s] compatible_version[%s]", server_list_type.c_str(), version.c_str());
		return false;
	}
