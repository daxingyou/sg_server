...
uint32_t scene_manager_t::m_seq = 0;
...
bool scene_manager_t::init()
{
	...
	for (std::map<uint32_t, MapTable*>::iterator itr = confs.begin(); 
		...
		if (p_maptable->main_game().first == env::server->get_id()) { // 当前服务器是这个地图主场景所在
			is_this_game = true;
		} else if (p_maptable->get_diversion_game(env::server->get_id()).first != 0) { // 当前服务器有这个地图分流场景所在
			is_this_game = true;
		}

		map_ptr pMap(new map_t(itr->first, (proto::server::em_map_type)p_maptable->scene_type()));
		if (!pMap) {
			log_error("pMap null error");
			return false;
		}
		...
		if (is_this_game && p_maptable->scene_type() == proto::server::map_type_static) {
			if (p_maptable->main_game().first == env::server->get_id()) { // 如果有主场景在这里，注册
				...
				if (p_scene != NULL) {
					p_scene->set_diversion(false, p_maptable->main_game().second);
				} else {
					log_error("scene[%u] map[%u] load error", itr->first, itr->first);
					return false;
				}
			}

			std::pair<uint32_t, uint32_t> diversion_scene = p_maptable->get_diversion_game(env::server->get_id());
			if (diversion_scene.first != 0) {
				for (uint32_t i = 0; i < diversion_scene.first; ++i) {
					scene_ptr p_scene = create_scene(itr->first, get_static_scene_id(itr->first, i + 1), (proto::server::em_scene_type)p_maptable->get_scene_attr_type(), p_maptable->get_scene_attr_type_param());
					if (p_scene != NULL) {
						p_scene->set_diversion(true, diversion_scene.second);
					} else {
						log_error("scene[%u] map[%u] load error", itr->first, itr->first);
						return false;
					}
...
uint32_t scene_manager_t::get_pk_jump_time(uint32_t mode)
{
	uint32_t mode_id = 0;
	switch (mode) {
...
uint32_t scene_manager_t::role_jump_scene(role_ptr p_role, proto::server::scene_change_type change_type, uint64_t scene_id, float des_x/* = -1*/, float des_y /*= -1*/, float des_dir /*= 0.f*/, bool is_teleport /*= true*/)
{
	...
	if (proto::server::user_change_scene == change_type && p_scene != NULL) {
		uint32_t scene_check = p_scene->scene_enter_check(p_role);
		if (scene_check != common::errcode_enum::error_ok) {
			SCENE_LOG("role[%lu] scene[%lu] check error:%u", p_role->get_uid(), scene_id, scene_check);
			return scene_check;
		}
	}
	...
	if (p_role->get_troop_id() > 0)
	{
		...
		if (NULL != p_troop)
		{
			if (p_role->is_leader())
			{
				...
				if (proto::server::user_change_scene == change_type && p_scene != NULL) {
					uint32_t scene_check = p_troop->member_scene_check(p_scene);
					if (scene_check != common::errcode_enum::error_ok) {
						SCENE_LOG("role[%lu] scene[%lu] troop check error:%u", p_role->get_uid(), scene_id, scene_check);
						return scene_check;
					}
				}
				...
				case jump_result_success:// 成功队员做相同处理
					{
						// 队员跳
						p_troop->follow_jump_scene(change_type, p_role->get_scene_id(), p_role->get_current_x(), p_role->get_current_y(), p_role->get_direction());
					}
					break;
				case jump_result_cross_server:
					{
						// 队员跳
...
uint32_t scene_manager_t::role_fast_move(role_ptr p_role, uint32_t move_type, float des_x, float des_y, float des_dir)
{
	...
	p_scene->on_move_s(p_role, des_x, des_y, des_dir, 0, 0);
		...
		case proto::server::center_change_scene:
			{
				...
				return jump_result_cross_server;
			}
			break;
		default:
			{
...
uint64_t scene_manager_t::get_new_dungeon_id(uint32_t map_tid)
{
	uint64_t id = 0;
	...
	id |= ((uint64_t)(0xF & env::server->get_id()) << 52);
	id |= ((uint64_t)(0xF & proto::server::map_type_dynamic) << 48);
	id |= ((uint64_t)(0xFFFFFFFF & m_seq) << 16);
	id |= ((uint64_t)(0xFFFF) & map_tid);

	return id;
}

uint64_t scene_manager_t::get_static_scene_id(uint32_t map_tid, uint32_t index)
{
	uint64_t id = 0;
	id |= ((uint64_t)(0xF & env::server->get_id()) << 52);
	id |= ((uint64_t)(0xF & proto::server::map_type_static) << 48);
	id |= ((uint64_t)(0xFFFFFFFF & index) << 16);
	id |= ((uint64_t)(0xFFFF) & map_tid);
...
void scene_manager_t::reg_game_scene_to_center(proto::server::ge_game_scene_reg_request* scene_reg_request)
{
		...
		citr != m_scenes.end(); ++citr)
	{
		...
		if (info != NULL)
		{
			...
			info->set_num(p_scene_ptr->get_scene_max_num());
			info->set_is_diversion(p_scene_ptr->is_diversion());
		}
	}
}
