...
bool role_data_manager::save_role_data_to_redis(uint64_t uid, const proto::common::role_data& ud)
{
	...
	redis_client_t::set_string("role", "name", uid, ud.name());
	...
	redis_client_t::set_uint32("role", "function_code", uid, ud.function_code());
...
bool role_data_manager::load_role_data_from_redis(uint64_t uid, proto::common::role_data* ud)
{
	...
	ud->set_function_code(redis_client_t::get_uint32("role", "function_code", uid));
...
{
    if (redis_client_t::is_key_exist("role", uid))
    {
...
bool role_data_manager::query_role_data(uint64_t uid, uint32_t gate_id)
{
	// 不存在正常返回
	if (!redis_client_t::is_key_exist("role", uid))
	{
	...
	log_info("role[%lu] center not find, but db is exist!!!!!!!!!!!!!", uid);

	// 对于走到这里说明db创建了人物但是center没创建，把任务信息发给center

	proto::server::de_create_role_reply ret;
	ret.set_uid(uid);
	ret.set_reply_code(common::errcode_enum::error_ok);
	ret.set_gate_id(gate_id);
	ret.set_name(redis_client_t::get_string("role", "name", uid));
	ret.set_create_time(redis_client_t::get_uint32("role", "create_time", uid));
	ret.set_role_type(redis_client_t::get_uint32("role", "role_type", uid));
...
uint32_t role_data_manager::check_nickname_valid(const std::string& name)
{
	...
	redis_client_t::get_keys_by_field_condition<uint64_t, std::string, std::vector<uint64_t>>("role", "name", name, uids);
	if (uids.size() >= 1)
	{
        ROLE_LOG("nickname[%s] is repeated!", name.c_str());
		return errcode_enum::notice_name_repeat;
...
bool role_data_manager::save_role_ex_to_redis(uint64_t uid, const proto::common::role_data& ud)
{
	...
	redis_client_t::set_uint32("role_ex", "recharge_count", uid, ud.recharge_data_info().recharge_flag());
	redis_client_t::set_uint32("role_ex", "total_pay", uid, ud.recharge_data_info().total_cny());
	redis_client_t::set_uint32("role_ex", "vip_level", uid, ud.recharge_data_info().vip_level());
	redis_client_t::set_uint32("role_ex", "recharge_reward_flag", uid, ud.recharge_data_info().recharge_reward_flag());
...
bool role_data_manager::load_role_ex_from_redis(uint64_t uid, proto::common::role_data* ud)
{
	...
	proto::common::recharge_data* p_recharge_info = ud->mutable_recharge_data_info();
	p_recharge_info->set_recharge_flag(redis_client_t::get_uint32("role_ex", "recharge_count", uid));
	p_recharge_info->set_total_cny(redis_client_t::get_uint32("role_ex", "total_pay", uid));
	p_recharge_info->set_vip_level(redis_client_t::get_uint32("role_ex", "vip_level", uid));
	p_recharge_info->set_recharge_reward_flag(redis_client_t::get_uint32("role_ex", "recharge_reward_flag", uid));
