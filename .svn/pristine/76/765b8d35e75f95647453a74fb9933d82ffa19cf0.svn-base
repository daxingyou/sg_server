...
public:
	...
	// 进入场景
	bool on_enter_s(object_position_ptr p_object, float x, float y, float dir = 0.f);
	// 离开场景
	void on_leave_s(object_id_type object_id);
	void on_leave_s(object_position_ptr p_object);
	// 移动同步
	bool on_move_s(const object_position_ptr& p_object, float to_x, float to_y, float direction, float time_stamp);
	// 得到生物附近生物，复杂
	...
	// 通知移动消息
	void notify_nearbies_move_s(object_position_ptr p_object, const nine_vector& nines);

	// 生物9屏更新
	bool object_into_vision(object_position_ptr p_object, const nine_vector& nines);
	// 人物视野更新
	bool role_into_vision(role_ptr p_role, const nine_vector& nines);
	// 生物离开9屏
	bool object_out_of_vision(object_position_ptr p_object, const nine_vector& nines);
	// 人物视野更新
...
public:
	...
	// 改变场景中人物的pk模式
	void scene_change_pk_mode(role_ptr p_role, proto::common::EM_PK_MODE mode);
