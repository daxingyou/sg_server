...
void lifeforce_manager_t::level_up_lifestar_level(uint64_t role_uid)
{
	...
	uint32_t need_item_id = 0;
	uint32_t need_item_quantity = 0;
	bool is_need_item = true;
	...
	do 
	{
		...
		if (p_config->need_item().size() != 2)
		{
			is_need_item = false;
		}
		else
		{
			need_item_id = p_config->need_item()[0];
			need_item_quantity = p_config->need_item()[1];
			is_need_item = true;
		}
		
		if (is_need_item == true)
		{
			const uint32_t item_quantity_owned = item_manager_t::get_item_count(p_role, need_item_id);
			if (item_quantity_owned < need_item_quantity)
			{
				log_error("role[%lu] up level lifestar need item not enough cur item/need item/item_id[%u/%u/%u]", role_uid, item_quantity_owned, need_item_quantity, need_item_id);
				recode = errcode_enum::notice_lifeforce_not_enough;
				break;
			}
			//扣除道具
			if (errcode_enum::error_ok != item_manager_t::remove_item(p_role, need_item_id, need_item_quantity, log_enum::source_type_lifeforce_up_lifestar_level, 0, p_data))
			{ 
				log_error("role[%lu] up lifestar level[%d] deduct item fail", role_uid, old_lifestar_level);
				break;
			}
		}
	...
	if (recode == errcode_enum::error_ok)
	{
		...
		log_wrapper_t::send_lifeforce_log(role_uid, old_lifestate_level, old_lifestar_level, 0, 1, new_lifestate_level,
			new_lifestar_level, log_enum::source_type_lifeforce_up_lifestar_level, 0);
		...
		do
		{
			...
			{
				log_error("NULL == p_cur_level_config role[%lu] cur state level[%d] not exist config", role_uid, old_lifestate_level + 1);
				break;
			}

			if (new_lifestar_level == p_cur_level_config->piece_count())
			{
...
void lifeforce_manager_t::level_up_lifelabel_level(uint64_t role_uid)
{
	...
	do 
	{
		...
		LIFEFORCE_LOG("level_cond size[%d] item_cond size[%d]", (int32_t)level_cond.size(), (int32_t)item_cond.size());
