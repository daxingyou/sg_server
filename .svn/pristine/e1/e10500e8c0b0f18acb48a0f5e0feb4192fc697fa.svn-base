...
uint32_t dungeon_manager_t::m_dungeon_size = 0;
...
void dungeon_manager_t::update(const boost::system::error_code& ec)
{
		...
		itr != m_dungeon_group[m_group_step].end();)
	{
		...
		if (p_scene == NULL || p_scene->get_scene_state() == SCENE_STATE_UNLOADED)
		{
			...
			if (m_dungeon_size > 0) m_dungeon_size--;
		}
		else
		{
...
void dungeon_manager_t::add_dungeon_id(uint64_t id)
{
	...
	m_dungeon_size++;
	if (m_dungeon_size == 1) { // 当有一个副本时候说明从无到有，启动定时器
		// 每250毫秒执行一次 一个轮回刚好是1秒
        if (NULL != m_group_timer)
        {
            m_group_timer->expires_from_now(boost::posix_time::millisec(DUNGEON_PRE_TIME));
...
void dungeon_manager_t::create_dungeon(const proto::server::dungeon_create_data& data, proto::server::ge_create_dugeon_reply& reply)
{
	...
	dungeon_scene_ptr p_dungeon_scene = scene_manager_t::create_dungeon_scene(p_dungeon_table->map_tid(), scene_manager_t::get_new_dungeon_id(p_dungeon_table->map_tid()), data.dungeon_id(), p_map_table->get_scene_attr_type_param());
...
{
	family_war_scene_ptr p_family_war = scene_manager_t::create_family_war_scene(map_tid, scene_manager_t::get_new_dungeon_id(map_tid));
...
bool dungeon_manager_t::check_fight_times(role_ptr p_role, uint32_t dungeon_id)
{
	if (NULL == p_role)
	{
		log_error("NULL == p_role");
	...
	DungeonTable* p_conf = GET_CONF(DungeonTable, dungeon_id);
	if (NULL == p_conf)
	{
		log_error("DungeonTable NULL == p_conf[%d]", dungeon_id);
		return false;
	}
	DungeonType* p_type_conf = GET_CONF(DungeonType, p_conf->base_id());
	if (NULL == p_type_conf)
	{
		log_error("DungeonType NULL == p_conf[%d]", p_conf->base_id());
		return false;
	}
	if (p_type_conf->fight_times() > 0)
	{
		dungeon_times_data_ptr p_dungeon_times_data = p_role->get_dungeon_data().get_dungeon_times_data(p_conf->base_id());
		if (NULL != p_dungeon_times_data && 0 == p_dungeon_times_data->remain_fight_time)
		{
			log_error("role[%lu] dungeon_id[%d] fight_times not enough", p_role->get_uid(), dungeon_id);
			return false;
		}
	}
	return true;
...
uint32_t dungeon_manager_t::buy_times(role_ptr p_role, uint32_t dungeon_type, proto::common::role_dungeon_times_data* p_data, proto::common::role_change_data* rcd)
{
	if (NULL == p_role)
	{
		log_error("NULL == p_role");
	...
	DungeonType* p_conf = GET_CONF(DungeonType, dungeon_type);
	if (NULL == p_conf)
	{
		log_error("DungeonType NULL == p_conf[%d]", dungeon_type);
		return errcode_enum::notice_unknown;
	}
	dungeon_times_data_ptr p_dungeon_times_data = p_role->get_dungeon_data().get_dungeon_times_data(dungeon_type);
	if (NULL == p_dungeon_times_data)
	{
		p_dungeon_times_data = p_role->get_dungeon_data().add_dungeon_times_data(dungeon_type);
	}
	if (NULL == p_dungeon_times_data) {
		log_error("DungeonType NULL == p_conf[%d]", dungeon_type);
		return errcode_enum::notice_unknown;
	}
	if (p_dungeon_times_data->remain_fight_time >= p_conf->fight_times())
	{
		log_error("role[%lu] DungeonType[%d] fight_times full", p_role->get_uid(), dungeon_type);
		return errcode_enum::dungeon_buy_number_error2;
	}
	if (p_dungeon_times_data->today_buy_time >= p_conf->buy_times())
	{
		log_error("role[%lu] DungeonType[%d] buy_times full", p_role->get_uid(), dungeon_type);
		return errcode_enum::dungeon_buy_number_error1;
	}
	if (p_dungeon_times_data->today_buy_time >= p_conf->buy_cost().size())
	{
		log_error("role[%lu] DungeonType[%d] buy_times full", p_role->get_uid(), dungeon_type);
		return errcode_enum::notice_unknown;
	}
	uint32_t cost = p_conf->buy_cost().at(p_dungeon_times_data->today_buy_time);
	if (!money_manager_t::use_money(p_role, proto::common::MONEY_TYPE_YUANBAO, cost, log_enum::source_type_dungeon_buy, dungeon_type))
	{
		log_error("role[%lu] DungeonType[%d] buy_times yuanbao not enough", p_role->get_uid(), dungeon_type);
		return errcode_enum::notice_gold_money_not_enough;
	}
	p_dungeon_times_data->today_buy_time += 1;
	p_dungeon_times_data->remain_fight_time += 1;
	p_data->set_dungeon_type(dungeon_type);
	p_data->set_remain_fight_time(p_dungeon_times_data->remain_fight_time);
	p_data->set_today_buy_time(p_dungeon_times_data->today_buy_time);
	proto::common::personal_info* p_info = rcd->mutable_per_info();
	p_role->get_personal_info(p_info);
	return errcode_enum::error_ok;
}

void dungeon_manager_t::ask_dungeon_times(const role_ptr& p_role)
{
	if (NULL == p_role) {
	...
	uint32_t dungeon_tid = p_role->get_role_scene_info().get_dungeon_id();
	DungeonTable* p_dungeon_table = GET_CONF(DungeonTable, dungeon_tid);
	if (NULL == p_dungeon_table) {
		log_error("role[%lu] dungeon_table id[%u] not find", p_role->get_uid(), dungeon_tid);
		return;
	}

	DungeonType* p_conf = GET_CONF(DungeonType, p_dungeon_table->base_id());
	if (NULL == p_conf) {
		log_error("role[%lu] DungeonType NULL == p_conf[%d]", p_role->get_uid(), p_dungeon_table->base_id());
		return;
	}

	dungeon_times_data_ptr p_dungeon_times_data = p_role->get_dungeon_data().get_dungeon_times_data(p_dungeon_table->base_id());
	if (NULL == p_dungeon_times_data) {
		p_dungeon_times_data = p_role->get_dungeon_data().add_dungeon_times_data(p_dungeon_table->base_id());
	}
	...
	proto::client::gc_dungeon_times_reply reply;
	reply.set_vigour(p_role->get_vigour());
	proto::common::role_dungeon_times_data* p_data = reply.mutable_dungeon_times_data();
	if (p_data) {
		p_data->set_dungeon_type(p_dungeon_table->base_id());
		p_data->set_remain_fight_time(p_dungeon_times_data->remain_fight_time);
		p_data->set_today_buy_time(p_dungeon_times_data->today_buy_time);
	}
	p_role->send_msg_to_client(op_cmd::gc_dungeon_times_reply, reply);
}
...
bool dungeon_manager_t::check_dungeon(role_ptr p_role, uint32_t dungeon_id)
{
	...
	do
	{
		...
		case dungeon::team_type_team_2:
			{
				if (p_troop == NULL || p_troop->get_member_size() < 2)
				{
		...
		case dungeon::team_type_team_3:
			{
				if (p_troop == NULL || p_troop->get_member_size() < 3)
				{
		...
		for (const auto& uid : check_list)
		{
			...
			if (NULL != p_role_check)
			{
				...
				// 体力、挑战次数限制，放到副本结算的时候
// 				if (!check_fight_times(p_role_check, dungeon_id))
// 				{
// 					DUNGEON_LOG("role[%lu] dungeon[%u] fight_times error", p_role_check->get_uid(), dungeon_id);
// 					back_code = common::errcode_enum::dungeon_create_number_no_enough;
// 					error_uid = uid;
// 					dungeon_team_limit = false;
// 					break;
// 				}
