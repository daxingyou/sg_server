...
public:
	...
	// 占领的势力
	uint32_t get_occupy_side() const { return m_occupy_side; }

	// 占领的家族
	uint64_t get_occupy_family() const;
	uint64_t get_occupy_help_family() const;
...
public:
	...
	static void on_long_revive_timer_call_back(const boost::weak_ptr<long_pulse_t>& pw_long, const boost::system::error_code& ec);

	static void on_long_occupied_timer_call_back(const boost::weak_ptr<long_pulse_t>& pw_long, const boost::system::error_code& ec);
...
protected:
	...
	// 参战家族
	king_war_family m_war_family[king_war_side_max];
	// 龙脉区域
	uint32_t m_area_id = 0;
	// 占领方
	uint32_t m_occupy_side = king_war_side_none;
