...
public:
	...
	*/
	...
	//获取开始时间
	static uint32_t get_timer_start_time( uint32_t time_id);

	//获取结束时间
	static uint32_t get_timer_end_time( uint32_t time_id);

	// 获取一个时间的唯一值(根据周期计算)
	static uint32_t get_time_unique_id(uint32_t time_id, uint32_t check_time);

	// 获取一个时间通过唯一值(根据周期计算)
	static uint32_t get_time_by_unique_id(uint32_t time_id, uint32_t unique_id);
private:
