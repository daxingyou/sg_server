...
public:
	...
	// 获取武将
	static hero_ptr get_hero_by_unique_id(object_base_ptr p_role, uint64_t unique_id);
    //获取主角武将
	static hero_ptr get_role_hero(role_ptr p_role);
    //获取所有武将
	static void get_all_heroes(object_base_ptr p_role, std::vector<hero_ptr>& res);
	//获取所有武将(包含隐藏武将)
	static void get_all_heroes_include_hide(role_ptr p_role, std::vector<hero_ptr>& res);
	//获取所有武将(战力排行)
	static void get_all_heroes_by_fight_value(object_base_ptr p_role, std::vector<hero_ptr>& res);
	// 战力s排序
	...
	// 所有属性更新属性
	static void all_heros_update_attr(role_ptr p_role, proto::common::role_change_data* p_data);
public:
