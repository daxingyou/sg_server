...
uint32_t shop_user_data_t::get_presonal_limit_buy_count(uint32_t id)
{
	shop_user_ptr person_limit_data = get_presonal_limit_info(id);
	if (!person_limit_data) {
		return 0;
	}

	// 判断是否在刷新时间内
	if (common::time_util_t::now_time() <= person_limit_data->m_user_goods.last_refresh_time()) {
		return person_limit_data->m_user_goods.buy_count();
	}
