...
void arena_manager_t::init()
{
	...
	arena_score_rank_mgr_t::load_from_redis(reset_time_id);
...
void arena_manager_t::match_ai(arena_role_ptr p_role, uint32_t min_score, uint32_t score, uint32_t match_pool_interval_score)
{
	...
	do 
	{
		...
		{
			log_error("arena_manager_t find_target_pos error min[%d] max[%d]", 0, find_pool.m_arena_ai_match_pool.size());
			break;
		}
...
void arena_manager_t::match_role(arena_role_ptr p_role, uint32_t min_score, uint32_t match_max_num, uint32_t match_pool_interval_score)
{
	...
	for (int32_t i = down_pool_pos; i < up_pool_pos; ++i)
	{
		...
		if (find_target_pos == -1)
		{
			log_error("arena_manager_t find_target_pos error min[%d] max[%d]", 0, m_match_pools[i].m_arena_match_pool.size());
			continue;
		}
												   ...
												   const proto::common::hero_data &self_heros)
{
	...
	user_old_rank = arena_score_rank_mgr_t::get_rank(user->get_role_uid());
	...
	user_now_rank = arena_score_rank_mgr_t::update_arena_score_rank(user->get_role_uid(), user_new_score, user_new_level,
		user->get_role_server_id(), user->get_role_name(), user->get_role_server_name(),
...
uint32_t arena_manager_t::add_role(uint32_t pool_pos, arena_role_ptr p_role)
{
	...
	{
		log_error("arena pool_pos[%d] > m_match_pools_size[%d]", m_match_pools.size());
		return errcode_enum::notice_unknown;
	}
...
uint32_t arena_manager_t::remove_role(int32_t pool_pos, uint64_t role_uid)
{
	...
	{
		log_error("arena pool_pos[%d] error m_match_pools_size[%d]", pool_pos, m_match_pools.size());
		return errcode_enum::notice_unknown;
	}
...
// 	}
	...
	{
		log_error("arena create dynamic ai fail id[%d] score[%]", p_ai->get_tid(), p_ai->get_score());
