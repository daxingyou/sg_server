...
bool center_msg_handler_t::init_msg_handler()
{
	...
	ret &= m_center_msg_handle.register_func(op_cmd::eh_silence_role_notify, handle_eh_silence_role_notify);
	ret &= m_center_msg_handle.register_func(op_cmd::eh_change_name_notify, handle_eh_change_name_notify);
	ret &= m_center_msg_handle.register_func(op_cmd::eh_kick_all_user_notify, handle_eh_kick_all_user_notify);
...
bool center_msg_handler_t::handle_eh_silence_role_notify(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eh_silence_role_notify);

	user_ptr p_role = user_manager_t::get_user(uid);
	if (NULL == p_role)
	{
		log_error("NULL == p_role[%lu]", uid);
		return false;
	}
	p_role->set_silence_time(msg.silence_time());
	return true;
}

bool center_msg_handler_t::handle_eh_change_name_notify(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eh_change_name_notify);
...
bool center_msg_handler_t::handle_eh_kick_all_user_notify(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eh_kick_all_user_notify);

	user_manager_t::kick_all_users();

	family_manager_t::clear_all_family();

	country_manager_t::clear_all_country();

	log_error("chat server del all user, reason:%u", msg.reason());
