...
uint32_t world_cup_manager_t::play(uint64_t player_id, const proto::common::role_cross_data& rcd)
{
	...
	{
		log_error("player[%lu] already play", player_id);
		return errcode_enum::notice_unknown;
	}
	bool bfind = false;
	for (auto it : battle_map)
	...
	if (!bfind)
	{
		log_error("player[%lu] invalid", player_id);
		return errcode_enum::notice_unknown;
	}
...
void world_cup_manager_t::make_battle()
{
	...
	if (current_round == 0)
	{
		...
		{
			log_error("get_16th_final_player error, player_vec.size()[%d] != 32", (int32_t)player_vec.size());
			return;
		}
		////测试代码 随机添加一个在线参赛者
	...
	else
	{
		...
		for (auto it : battle_map)
		{
			...
			{
				WORLDCUP_LOG("p_battle[%lu] is not end, round[%d]", p_battle->get_uid(), current_round);
				continue;
			}
	...
	for (uint32_t i = 0; i < player_vec.size()/2; ++i)
	{
		...
		battle_map.insert(std::make_pair(uid, p_battle));
		WORLDCUP_LOG("make battle[%lu] player_a[%lu] player_b[%lu]", uid, p_battle->get_player_a_uid(), p_battle->get_player_b_uid());
		uint32_t rank_index_a = arena_score_rank_mgr_t::get_arena_rank(p_battle->get_player_a_uid());
		uint32_t rank_index_b = arena_score_rank_mgr_t::get_arena_rank(p_battle->get_player_b_uid());
		if (0 != rank_index_a && 0 != rank_index_b)
		{
			proto::server::re_world_cup_mail_notify ntf_a;
			ntf_a.set_player_name(redis_client_t::get_string("arena_score_rank", "name", p_battle->get_player_b_uid()));
			ntf_a.set_server_id(redis_client_t::get_uint32("arena_score_rank", "server_id", p_battle->get_player_a_uid()));
			...
			ntf_b.set_player_name(redis_client_t::get_string("arena_score_rank", "name", p_battle->get_player_a_uid()));
			ntf_b.set_server_id(redis_client_t::get_uint32("arena_score_rank", "server_id", p_battle->get_player_b_uid()));
	...
	WORLDCUP_LOG("battle_map size[%d]", (int32_t)battle_map.size());
	if (NULL == m_timer)
	{
...
void world_cup_manager_t::comming_quarter(const boost::system::error_code& ec)
{
	...
	for (auto it : battle_map)
	{
		...
		uint64_t player_a_uid = p_battle->get_player_a_uid();
		uint32_t rank_index_a = arena_score_rank_mgr_t::get_arena_rank(player_a_uid);
		if (0 != rank_index_a)
		{
			proto::server::re_world_cup_coming_quarter_notify ntf_a;
			ntf_a.set_server_id(redis_client_t::get_uint32("arena_score_rank", "server_id", player_a_uid));
		...
		uint64_t player_b_uid = p_battle->get_player_b_uid();
		uint32_t rank_index_b = arena_score_rank_mgr_t::get_arena_rank(player_b_uid);
		if (0 != rank_index_b)
		{
			proto::server::re_world_cup_coming_quarter_notify ntf_b;
			ntf_b.set_server_id(redis_client_t::get_uint32("arena_score_rank", "server_id", player_b_uid));
	...
	for (auto it : battle_map)
	{
		...
		if (is_a_exist)
		{
			...
			else
			{
				...
				p_battle->bet_result();
				WORLDCUP_LOG("make battle[%lu] player[%lu] win", p_battle->get_uid(), player_a_uid);
			}
		}
		else
		{
			if (is_b_exist)
			{
				...
				p_battle->set_state(proto::common::world_cup_battle_state_end);
				p_battle->bet_result();
				WORLDCUP_LOG("make battle[%lu] player[%lu] win", p_battle->get_uid(), player_b_uid);
			}
			else
			{
				//没有胜利者
				...
				p_battle->set_state(proto::common::world_cup_battle_state_end);
				p_battle->bet_result();
				WORLDCUP_LOG("make battle[%lu] no winner", p_battle->get_uid());
			}
		}
...
void world_cup_manager_t::battle_result(uint64_t winner_id, uint64_t loser_id)
{
	for (auto it : battle_map)
	{
		...
		if (player_a_uid == winner_id && player_b_uid == loser_id)
		{
			...
			WORLDCUP_LOG("battle[%lu] player[%lu] score[%d]", p_battle->get_uid(), player_a_uid, p_battle->get_player_a_score());
			...
			if (p_battle->get_player_a_score() >= winner_score)
			{
				...
				WORLDCUP_LOG("battle[%lu] player[%lu] win", p_battle->get_uid(), player_a_uid);
			}
			proto::server::re_world_cup_update_notify ntf;
			p_battle->peek_data(ntf.mutable_battle_info());
			env::server->send_msg_to_transfer(op_cmd::re_world_cup_update_notify, 0, ntf);
		...
		else if (player_a_uid == loser_id && player_b_uid == winner_id)
		{
			...
			WORLDCUP_LOG("battle[%lu] player[%lu] score[%d]", p_battle->get_uid(), player_b_uid, p_battle->get_player_b_score());
			uint32_t winner_score = get_winner_score();
			uint32_t time_remain = time_util_t::get_remain_time(m_interval_timer);
			if (p_battle->get_player_b_score() >= winner_score)
			{
				...
				p_battle->bet_result();
				time_remain = time_util_t::get_remain_time(m_timer);
				WORLDCUP_LOG("battle[%lu] player[%lu] win", p_battle->get_uid(), player_b_uid);
			}
			proto::server::re_world_cup_update_notify ntf;
			p_battle->peek_data(ntf.mutable_battle_info());
			env::server->send_msg_to_transfer(op_cmd::re_world_cup_update_notify, 0, ntf);
