...
void activity_manager_t::load_data(const proto::common::activities_data& activities_data)
{
	...
	{
		uint32_t liveness = activities_data.box_list(i).liveness();
		uint32_t state = activities_data.box_list(i).state();
		if (state == liveness_box_state_over)
		{
			m_box_vec.push_back(liveness);
		}
	}
...
void activity_manager_t::save_self()
{
	...
	role_unify_save::add_task(m_role_uid, m_key, activities_data);
}
...
bool activity_manager_t::check_open(uint32_t activity_id)
{
	...
	if (it_activity != m_activity_map.end())
	{
		...
		if (NULL != p_activity)
		{
			...
			add_count = p_activity->get_add_count();
		}
	}	
	
	if (p_activity_conf->times() > 0 && count >= p_activity_conf->times() + add_count)
	{
...
void activity_manager_t::activity_done(uint32_t activity_id)
{
	...
	//容错代码（当前值大于最大值）
	if (p_activity->get_count() >= p_activity_conf->times()+ p_activity->get_add_count())
	{
		return;
	}
	...
	if (0 != p_activity_conf->liveness())
	{
		sys_notice_manager_t::broadcast_system_msg_for_money(m_role_uid, proto::common::MONEY_TYPE_LIVENESS, p_activity_conf->liveness());
	}
...
bool activity_manager_t::is_finish_all_circle(uint32_t activity_id)
{
	...
	if (p_activity_conf->times() > 0 && count >= p_activity_conf->times() + p_activity->get_add_count())
		return false;

	return true;
}

bool activity_manager_t::add_activity_count(uint32_t activity_id, uint32_t count)
{
	auto p_activity_conf = GET_CONF(Activities, activity_id);
	if (NULL == p_activity_conf)
	{
	...
	uint32_t add_count = p_activity->get_add_count();
	if (add_count + count > p_activity_conf->add_times())
	{
		log_error("role[%lu] activity[%d] add_count not enough", m_role_uid, activity_id);
		return false;
	}
	p_activity->set_add_count(add_count + count);
	role_ptr p_role = role_manager_t::find_role(m_role_uid);
	if (NULL != p_role)
	{
		proto::client::gc_activity_done_notify ntf;
...
uint32_t activity_manager_t::get_activity_rest_count(uint32_t activity_id)
{
	auto p_activity_conf = GET_CONF(Activities, activity_id);
	if (NULL == p_activity_conf)
	{
	...
	uint32_t max_count = p_activity_conf->times();
	auto it_activity = m_activity_map.find(activity_id);
	if (it_activity != m_activity_map.end())
	{
		activity_ptr p_activity = it_activity->second;
		...
		{
			uint32_t count = p_activity->get_count();
			return max_count + p_activity->get_add_count() - count;
		}
	}
	return max_count;
}
