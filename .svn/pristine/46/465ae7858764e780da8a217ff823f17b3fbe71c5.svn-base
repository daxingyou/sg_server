...
bool achieve_data_manager::save_achieve_data_to_redis(uint64_t uid, const proto::common::achieve_data& data)
{
	redis_client_t::set_protobuf("achieve", "achieve_data", uid, data);
	return true;
}

bool achieve_data_manager::load_achieve_data_from_redis(uint64_t uid, proto::common::achieve_data* p_data)
{
	if (p_data == NULL)
	{
		log_error("load_achieve_data_from_redis [%lu] data == null", uid);
		return false;
	}
	proto::common::achieve_data data_info;
	redis_client_t::get_protobuf("achieve", "achieve_data", uid, data_info);
