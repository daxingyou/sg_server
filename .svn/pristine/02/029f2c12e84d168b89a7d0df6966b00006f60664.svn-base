...
bool admin_manager_t::del_admin(uint32_t admin_id)
{
    ...
    if (it == m_admin_map.end())
    {
        log_error("delete admin failed! admin[%u] doesn't exist!", admin_id);
        return false;
    }
    ...
    if (!admin)
    {
        log_error("delete admin failed! admin[%u] is NULL!", admin_id);
        return false;
    }
    ...
    log_debug("delete admin[%u] success!", admin_id);
