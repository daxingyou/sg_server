	...
	if (elite_list.empty()) {
		...
		do {
			...
			if (!is_break)
			{
				...
				PAGE_LOG("role[%lu] page elite open list add[%d]",role_uid,pass_elite_count + 1);
			}
		} while (false);
...
void page_manager_t::check_page_task_status(uint64_t role_uid,uint32_t task_id)
{
	...
	if (!p_task_mgr->is_done_or_can_commmit(task_id)){
		return;
	}
	...
	if (0 == section_id)
	{
		PAGE_LOG("task[%d] not belong to page task", task_id);
		return;
	}
	...
	{
		if (!p_task_mgr->is_done_or_can_commmit(it)){
...
uint32_t page_manager_t::get_page_list(uint64_t role_uid,proto::client::gc_section_reply& reply)
{
	...
	do 
	{
		...
		const std::set<uint32_t>& task_list = p_task_mgr->get_all_comp_task();
	
		for (auto& it : task_list)
		{
			Task* p_config = GET_CONF(Task, it);
			if (NULL == p_config) {
				log_error("NULL == p_config task_id[%d]", it);
				reply_code = errcode_enum::notice_config_error;
				break;
			}

			if (task_type::TASK_TYPE_TRUNK != p_config->type()){
				PAGE_LOG("task[%d] task type not trunk type[%d]", it, p_config->type());
				continue;
			}
	...
	for (auto& it : task_id_vec )
	{
		...
		PAGE_LOG("role[%lu] task_id[%d]", role_uid, it);
	}
...
uint32_t page_manager_t::get_page_pass_reward(uint32_t type,uint64_t role_uid,uint32_t id ,proto::client::gc_section_reward_reply& reply)
{
	...
	do 
	{
		...
		else if(2 == type)
		{
			...
			p_role->save_self();
			PAGE_LOG("role[%lu] page_elite_size[%d]",role_uid, (int32_t)page_elite_list.size());
		}
	} while (false);
...
uint32_t page_manager_t::quick_fight_elite_page(uint64_t role_uid, uint32_t id, uint32_t type, proto::client::gc_quick_fight_elite_page_reply &reply)
{
	...
	do {
		...
		//精英副本扫荡次数活跃度
		achieve_common_t::notify_progress_state(role_uid, proto::common::ACHIEVE_ELITE_PAGE_QUICK_FIGHT_NUMBER, 0, need_count);
	} while (0);
	return reply_code;
}
...
void page_manager_t::gm_finish_all_elite(uint64_t role_uid)
{
	role_ptr p_role = role_manager_t::find_role(role_uid);
	if (NULL == p_role)
	{
	...
	std::map<uint32_t, SectionElite*> confs;
	GET_ALL_CONF(SectionElite, confs);
	for (auto& it : confs)
	{
		SectionElite* p_config = it.second;
		if (NULL == p_config) {
			log_error("SectionElite null error");
			break;
		}
		check_page_elite_task_status(p_role->get_uid(), p_config->id(), p_config->fight_id());
	}
}
