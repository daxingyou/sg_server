...
void troop_t::process_event(uint32_t event_id, uint64_t member_id, proto::common::troop_data* td)
{
	if (!td) {
		log_error("troop_data null error");
		return;
	...
	switch (event_id) {
		case proto::common::troop_event_create:
			do_create_troop(member_id, td);
			break;
		case proto::common::troop_event_join:
			do_member_join(member_id, td);
			add_zhuagui_task(member_id, td);
			break;
		case proto::common::troop_event_quit:
			do_member_quit(member_id);
			add_zhuagui_task(member_id, NULL);
			break;
		...
		case proto::common::troop_event_zhuagui_task:
			do_member_zhuagui_task(member_id, td);
			break;
		default: {
...
void troop_t::do_create_troop(uint64_t member_id, proto::common::troop_data* td)
{
	if (!td) {
		log_error("troop_data null error");
		return;
...
void troop_t::do_change_leader(uint64_t member_id, proto::common::troop_data* td)
{
	if (!td) {
		log_error("troop_data null error");
		return;
...
void troop_t::do_member_join(uint64_t member_id, proto::common::troop_data* td)
{
	if (!td) {
		log_error("troop_data null error");
		return;
		...
		if (p_join_member->get_pk_mode() != proto::common::PK_PEACE) { // 玩家不能直接传送
			...
			if (p_leader != NULL) { // 同个服务器的处理
				...
				if (index >= TROOP_MEMBER_MAX_COUNT) {
					log_error("troop[%lu] member[%lu] size[%u] error", m_uid, member_id, m_troop_members.size());
					return;
				}
	...
	TROOP_LOG("role[%lu] join troop[%lu]", member_id, m_uid);
}

void troop_t::do_member_quit(uint64_t member_id)
{
...
void troop_t::do_member_back(uint64_t member_id)
{
...
void troop_t::do_troop_modify(uint64_t member_id, proto::common::troop_data* td)
{
	if (!td) {
		log_error("troop_data null error");
		return;
...
void troop_t::do_member_update(uint64_t member_id, proto::common::troop_data* td)
{
	if (!td) {
		log_error("troop_data null error");
		return;
...
void troop_t::do_member_online(uint64_t member_id, proto::common::troop_data* td)
{
	if (!td) {
		log_error("troop_data null error");
		return;
...
void troop_t::leader_move(const scene_ptr& p_scene, uint32_t speed, float x, float y, float direction)
{
	if (NULL == p_scene) {
	...
	if (speed > 10) {
		...
		m_moves.pop_back();
		m_moves.push_front(troop_move(x, y, direction));
	}

	for (uint32_t i = 1, index = 1; i < m_troop_members.size(); ++i) {
		if (NULL != m_troop_members[i] && m_troop_members[i]->get_state() == proto::common::troop_state_normal) {
			role_ptr p_role = role_manager_t::find_role(m_troop_members[i]->get_uid());
			if (NULL != p_role && index < TROOP_MEMBER_MAX_COUNT) {
				p_scene->on_move_s(p_role, m_moves[index].m_x, m_moves[index].m_y, m_moves[index].m_dir, 0);
				++index;
			}
		}
	}
	...
	for (uint32_t i = 1; i < m_troop_members.size(); ++i) {
		if (NULL != m_troop_members[i] && m_troop_members[i]->get_state() == proto::common::troop_state_normal) {
			...
			if (NULL != p_role) {
				...
				p_scene->on_move_s(p_role, x, y, dir, 0);
			}
		}
	}
}
...
void troop_t::start_team_check(uint32_t dungeon_id)
{
	...
	for (const auto& p_member : m_troop_members) {
		...
		// 队长默认确认
		if (p_member->get_uid() == m_leader_uid) {
...
bool troop_t::fill_member_hero_to_formation()
{
	...
	for (uint32_t i = 0; i < m_troop_members.size(); ++i)
	{
		p_member = m_troop_members[i];
		if (NULL == p_member)
		{
			log_error("user_troop fill_member_hero_to_formation p_member NULL");
			return false;
		}
		p_role = role_manager_t::find_role(p_member->get_uid());
		if (NULL == p_role)
		{
		...
		hero_manager_t::get_all_heroes_by_fight_value(p_role, result);


		hero_ptr p_hero = hero_ptr();
		// 队长必须上阵主公
		if (p_member->get_uid() == get_leader_id())
		{
			p_leader = p_role;
			// 加入主公
			hero_pos.insert(std::make_pair(++hero_fight_pos, p_role->get_main_hero_uid()));
			for (uint32_t i = 0; i < result.size(); ++i)
			{
				...
				if (p_hero->get_unique_id() == p_role->get_main_hero_uid())
					continue;
				hero_pos.insert(std::make_pair(++hero_fight_pos, p_hero->get_unique_id()));
				// 达到上阵上限
	...
	m_formation_mgr.update_formation(p_leader, m_formation_mgr.get_cur_formation_id(), proto::common::fight_type_none, hero_pos, 0);
	return true;
}

role_ptr troop_t::get_heros_role_by_hero_uid(uint64_t hero_id)
{
	for (uint32_t i = 0; i < m_troop_members.size(); ++i)
	{
		troop_member_ptr p_member = m_troop_members[i];
		if (NULL == p_member)
		{
			continue;
		}
		auto p_role = role_manager_t::find_role(p_member->get_uid());
		if (NULL != p_role)
		{
			if (hero_manager_t::has_hero(p_role, hero_id))
			{
				return p_role;
			}
		}
	...
	log_error("not find hero[%llu]'s role", hero_id);
	return role_ptr();
}
...
void troop_t::open_treasure(uint64_t role_uid)
{
	...
	for (auto p_member : m_troop_members)
	{
		if (NULL == p_member)
		{
		...
		{
			log_error("role[%lu] open_treasure failed, check_open failed", member_uid);
			return;
		}
	}
	//广播消息给队员
...
void troop_t::gather_treasure(uint64_t role_uid)
{
	...
	{
		log_error("role[%lu] gather_treasure failed, pos invalid", role_uid, m_treasure_id);
		return;
	}
...
void troop_t::do_member_zhuagui_task(uint64_t member_id, proto::common::troop_data* td)
{
	for (uint32_t i = 0; i < m_troop_members.size(); ++i)
	{
		const troop_member_ptr& p_member = m_troop_members[i];
...
void troop_t::add_zhuagui_task(uint64_t member_id, proto::common::troop_data* td)
{
	role_ptr p_role = role_manager_t::find_role( member_id );
	if (NULL == p_role)
	{
		return;
	}
...
void troop_t::clear_zhuagui_task() 
{
	for (uint32_t i = 0; i < m_troop_members.size(); ++i)
	{
		const troop_member_ptr& p_member = m_troop_members[i];
