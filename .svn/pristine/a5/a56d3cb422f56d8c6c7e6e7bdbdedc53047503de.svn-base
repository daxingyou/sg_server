...
void hero_manager_t::peek_all_datas(role_ptr p_role, proto::common::hero_data* p_data)
{
	...
	for (; it != it_end; ++it)
	{
		...
		if (NULL != p_hero)
		{
			...
			p_hero->peek_data(p_single, false);
			++send_num;
		}
	}
...
void hero_manager_t::create_hero(role_ptr p_role, const proto::common::hero_single& single)
{
	...
	hero_ptr p_hero(new hero_t(p_role->get_object_id_type()));
	...
	for (iter_conf = confs.begin(); iter_conf != confs.end(); ++iter_conf)
	{
		...
		if (hero_job == conf->id())
		{
			add_hero(
				p_role, 
				conf->initial_id(), 
				log_enum::source_type_role_create, 
				NULL, 
				0, 
				proto::common::hero_state_type_none, 
				is_notify);
		}
		else
		{
			add_hero(
				...
				NULL, 
				1, 
				proto::common::hero_state_type_lock, 
				is_notify);
		}
	}
}
...
void hero_manager_t::add_hero(
	...
	uint32_t state, 
	bool is_notify, 
	proto::common::luckydraw_award* la)
{
	if (NULL == p_role)
	{
		log_error("p_role is NULL!");
	...
	if (NULL == p_hero)
	{
		...
		p_hero = hero_ptr(new hero_t(hero_tid, unique_id, p_role->get_object_id_type()));
		if (type == 0)
		{
		...
		bool is_add_main_hero = false;
		std::map<uint32_t, HeroJobTransfer*> confs;
		GET_ALL_CONF(HeroJobTransfer, confs);
		for (auto& it : confs)
		{
			HeroJobTransfer* p_config = it.second;
			if (NULL == p_config)
				continue;
			if (p_config->initial_id() == hero_tid)
			{
				is_add_main_hero = true;
				break;
			}
		}
		if (!is_add_main_hero)
		{
			p_role->add_head_frame(proto::common::HEAD_FRAME_HERO, hero_tid);
			//成就完成立即通知
...
void hero_manager_t::get_all_heroes(object_base_ptr p_role, std::vector<hero_ptr>& res)
{
    if (p_role == NULL)
    {
	...
	hero_map* list = p_role->get_hero_list();
	if (NULL == list)
		return;
	hero_map::iterator iter = list->begin();
	for (; iter != list->end(); ++iter)
	{
		...
		if(p_hero->get_hero_state() == proto::common::hero_state_type_lock)
			continue;
		// 如果是主角英雄，并且不是当前的主角英雄
		if (p_hero->is_main_hero() && p_hero->get_tid() != p_role->get_main_hero_tid())
			continue;
...
void hero_manager_t::get_all_heroes_by_fight_value(object_base_ptr p_role, std::vector<hero_ptr>& res)
{
	...
	common::Sort::Qsort(res, 0, SAFE_SUB(res.size(),1), hero_manager_t::hero_fight_value_sort);
...
void hero_manager_t::remove_hero(role_ptr p_role, uint32_t hero_tid, uint32_t source_type, proto::common::role_change_data* p_data)
{
 	...
 	if (NULL != p_hero)
	{
		...
		if (NULL != p_data)
		{
			...
			if (p_hero_data)
			{
				p_hero->peek_data(p_hero_data->add_hero_list());
			}
		}
	}
}
...
hero_ptr hero_manager_t::get_role_hero(role_ptr p_role)
{
	...
	hero_ptr p_hero = p_role->get_hero_by_unique_id(p_role->get_main_hero_uid());
	if (NULL == p_hero)
	{
...
// 获取武将
hero_ptr hero_manager_t::get_hero_by_unique_id(object_base_ptr p_role, uint64_t unique_id)
{
	if (p_role == NULL)
	{
	...
	return p_role->get_hero_by_unique_id(unique_id);
}
...
bool hero_manager_t::has_hero(role_ptr p_role, uint64_t uid)
{
	...
	return NULL != p_role->get_hero_by_unique_id(uid);
}
...
void hero_manager_t::add_pve_exp(role_ptr p_role, uint64_t hero_uid, uint32_t exp, proto::client::gc_combat_result_ntf* p_data /*= NULL*/)
{
	...
	if (exp == 0)
	{
		log_error("role[%lu] can't add zero exp!", p_role->get_uid());
		return;
	}
...
uint32_t hero_manager_t::unlock_main_hero(role_ptr p_role, uint32_t hero_tid, proto::common::role_change_data* p_data)
{
	...
	p_role->add_head_frame(proto::common::HEAD_FRAME_UNLOCK_MAIN_HERO,hero_tid);
...
void hero_manager_t::change_role_main_hero(role_ptr p_role, uint32_t hero_tid)
{
	...
	{
		log_error("role[%lu] change_role_main_hero old_main_hero_uid[%lu] = change_hero_uid[%lu]", p_role->get_uid(), old_main_hero_uid, p_hero->get_unique_id());
		return;
	}
	hero_ptr p_old_hero = p_role->get_hero_by_unique_id(old_main_hero_uid);
	if (NULL == p_old_hero)
	{
...
void hero_manager_t::all_heros_update_attr(role_ptr p_role, proto::common::role_change_data* p_data)
{
	if (NULL == p_role)
	{
		log_error("NULL == p_role");
	...
	std::vector<hero_ptr> all_heroes;
	get_all_heroes(p_role, all_heroes);
	for (auto p_hero : all_heroes)
	{
		if (NULL == p_hero)
		{
