...
bool login_server_t::load_server_xml()
{
    try
    {
        ...
        //connect to account
        {
            std::string str_ip_list = pt.get<std::string>("server.login_server.connect.account.ip");
            ...
            std::string str_port_list = pt.get<std::string>("server.login_server.connect.account.port");
...
bool login_server_t::init_network()
{
    ...
    for (size_t index = 0; index < env::xml->connect_to_account_list.size(); ++index)
    {
        ...
        {
            log_error("init account[%s:%d] msg handler failed!", connector.ip.c_str(), connector.port);
            continue;
        }
    }
...
void login_server_t::on_add_account(const tcp_socket_ptr& s)
{
    ...
    // 同步login信息给account
    proto::server::ln_syn_login_info_notify ntf;
    ...
    send_msg_to_account(s, op_cmd::ln_syn_login_info_notify, ntf);
}
void login_server_t::on_del_account(const tcp_socket_ptr& s)
{
    account_manager_t::del_account(s);
}
void login_server_t::on_account_msg(const tcp_socket_ptr& s, uint64_t uid,uint16_t cmd, const msg_buf_ptr& msg_buf)
{
    msg_count_t::push_msg_recv_info(msg_buf, cmd, env::xml->is_log_msg_count);
    account_msg_handler_t::handle_server_msg(s, uid, cmd, msg_buf);
}

void login_server_t::stop_network()
{
	m_network.stop();
}
