...
public:
    ...
    void add_user(uint64_t uid)
    {
        ...
        //log_warn("add user[%lu] to the gate[%d] list, has %d users in the gate[%d]", uid, m_id, m_user_list.size(), m_id);
    }
    ...
    bool del_user(uint64_t uid)
    {
        ...
        //log_warn("delete user[%lu] from the gate[%d] list, left %d users in the gate[%d]", uid, m_id, m_user_list.size(), m_id);
        return true;
    }
...
private:
    ...
    std::string     m_ip = "";
    uint32_t        m_port = 0;
    std::vector<uint64_t> m_user_list;
