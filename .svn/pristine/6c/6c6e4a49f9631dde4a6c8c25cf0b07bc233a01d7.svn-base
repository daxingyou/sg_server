...
bool expedition_msg_handle_t::init_center_msg_handler()
{
	...
	bresult &= center_msg_handler_t::m_center_msg_handle.register_func(op_cmd::eg_expedition_refresh_stage_reply, handle_eg_expedition_refresh_stage_reply);
...
bool expedition_msg_handle_t::handle_cg_expedition_data_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	...
	{
		log_error("NULL == p_expedition[%lu]", uid);
		return false;
	}
...
bool expedition_msg_handle_t::handle_cg_expedition_shop_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	...
	uint32_t reply_code = p_expedition->shop_request(msg.buy_style(), reply);
	reply.set_reply_code(reply_code);
...
bool expedition_msg_handle_t::handle_eg_expedition_refresh_stage_reply(const network::msg_buf_ptr& msg_buf)
{
	PRE_S2S_MSG(proto::server::eg_expedition_refresh_stage_reply);
	...
	p_expedition->refresh_stage_reply(msg);
	return true;
}
