...
void game_t::reset()
{
	...
	{
		std::vector<uint64_t> temp(m_user_list);

		for (std::vector<uint64_t>::const_iterator citr = temp.begin();
			citr != temp.end(); ++citr)
		{
    ...
    for (size_t idx = 0; idx < m_game_list.size(); ++idx)
    {
        ...
        if (NULL != p_game)
        {
            ...
            if (NULL != s && p_game->get_socket() == s)
            {
                ...
                {
                    log_info("add game[%d], has %d games", game_id, get_game_size());
                }
                else if (game_server_type_cross == game_type)
                {
                    log_info("add cross[%d], has %d crosses", game_id, get_cross_size());
                }
                else
                {
...
void game_manager_t::del_game(const tcp_socket_ptr& s)
{
    ...
    for (; itr != itr_end; ++itr)
    {
        ...
        if (NULL != p_game && (NULL != s && p_game->get_socket() == s))
        {
            ...
            if (game_server_type_normal == game_type)
            {
                log_info("del game[%d], left %d games", game_id, get_game_size());
            }
            else if (game_server_type_cross == game_type)
            {
                log_info("del cross[%d], left %d crosses", game_id, get_cross_size());
            }
            else
            {
                log_error("game type[%d] error!", game_type);
...
const tcp_socket_ptr game_manager_t::get_game_socket_by_user_uid(uint64_t uid, uint32_t game_type)
{
    ...
    if (NULL == p_game)
    {
        log_error("NULL == p_game"); // 还未登陆场景服就是找不到gameid
