...
global_user_manager_t::nickname_uid_map_type global_user_manager_t::m_nickname_uid_map;
...
bool global_user_manager_t::add_global_user( global_user_ptr p_user)
{
	...
	m_nickname_uid_map.insert(nickname_uid_map_type::value_type(p_user->get_name(), p_user->get_uid()));
...
void global_user_manager_t::kick_all_global_users()
{
...
global_user_ptr global_user_manager_t::get_global_user_by_name(const std::string& name)
{
    //需要重新实现
    /*nickname_uid_map_type::iterator itr = m_nickname_uid_map.find(name);
    if (itr != m_nickname_uid_map.end())
    {
        uint64_t tmp_uid = itr->second;
        global_user_map_type::iterator itr_user = m_global_user_map.find(tmp_uid);
        if (itr_user != m_global_user_map.end())
        {
            return (itr_user->second);
        }
        else
        {
            COMMON_ASSERT(false);
...
void global_user_manager_t::reg_global_user(const network::tcp_socket_ptr& s, uint64_t uid)
{
	...
	else
	{
		if (global_user_data_mgr_t::is_user_key_exsit(uid))
		{
			...
			if (NULL != p_new_user && p_new_user->reg_user(gate_id) && global_user_manager_t::add_global_user(p_new_user))
			{
		...
		{
			// 对于没有注册过这个玩家，去db在查询下
			proto::server::ed_query_role_data_request ed_request;
			ed_request.set_uid(uid);
			ed_request.set_gate_id(gate_manager_t::get_gate_id_by_socket(s));
			env::server->send_msg_to_db(op_cmd::ed_query_role_data_request, uid, ed_request);
		}
	}
}
...
{
	global_user_data_mgr_t::create_new_global_user(uid, nickname, create_time, channel_id);
		...
		case common::UNREGUSER_RET_OFFLINE:
			{
				...
				env::server->send_msg_to_db(op_cmd::ed_role_leave_game_notify, uid, ntf_db);

				proto::server::gr_role_leave_game_notify ntf_area;
				env::server->send_msg_to_transfer(op_cmd::gr_role_leave_game_notify, uid, ntf_area);
