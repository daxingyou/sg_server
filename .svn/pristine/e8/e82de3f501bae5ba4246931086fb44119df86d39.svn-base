...
public:
	...
	// 获取家族战结果
	void peek_family_war_result(proto::server::family_war_result_data* p_result_data);
	...
	// 增加家族战积分，返回类型战斗是否结束
	bool add_family_score(family_war_side_type type, uint32_t score);
	...
	// 战斗结束通知
	void attack_long_end(uint64_t uid, uint64_t fight_id, const combat_ptr& p_combat, bool is_win);
	// 通知龙柱死亡
	void family_war_long_death(uint64_t fight_id);

	// 得到title
	std::pair<family_war_kill_title, uint32_t> get_kill_title(uint32_t kill_num);
	// 击杀获得的奖励
...
public:
	...
	struct family_war_platform_info
	{
		...
		uint32_t m_hurt = 0;
		uint64_t m_object_id = 0;
		family_war_side_type m_occupied_side = family_war_side_none;	// 占领家族
		proto::common::family_object_pos m_pos;
	};
	...
	struct family_war_role_info_t
	{
		...
		uint32_t m_kills = 0;
		std::pair<family_war_kill_title, uint32_t> m_kill_title;
		...
		void peek_hero_data(proto::client::gc_formations_hero_update_reply& notify);
	...
	typedef std::list<family_war_role_info_ptr> family_war_rank_list;
...
protected:
	...
	uint32_t m_next_update_rank = 0;// 排名最后更新时间
...
protected:
	...
	family_war_rank_list m_family_war_sort_list;
