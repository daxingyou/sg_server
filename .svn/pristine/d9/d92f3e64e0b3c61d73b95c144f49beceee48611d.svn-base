...
void transfer_manager_t::set_transfer(const tcp_socket_ptr& s, uint32_t transfer_id)
{
	...
	//同步聊天服務器信息给中转服务器
	proto::server::hf_syn_chat_info_notify req;
	req.set_server_group_id(env::xml->srv_group.group_id);
	req.set_server_name(env::xml->srv_group.group_name);
	env::server->send_msg_to_transfer(s, op_cmd::hf_syn_chat_info_notify, 0, req);
