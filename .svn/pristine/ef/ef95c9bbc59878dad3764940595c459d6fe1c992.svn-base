...
public:
	...
	static bool sort_dents_goods( uint64_t auid, uint64_t buid );
public:
	//获取出售（公示）商品清单
...
public:
	//获取道具配置
	static Goods* get_goods_conf(uint32_t item_tid);
	//添加查询映射
	static void add_search_goods( goods_ptr p_goods );
	//删除查询映射
	static void del_search_goods(goods_ptr p_goods );
	//预定某个道具	
	static uint32_t destine_goods( uint64_t role_uid, uint32_t search_id, proto::server::eg_destine_item_reply &reply );
	//解除预定某个道具
	static void undestine_goods( uint64_t role_uid, uint64_t goods_uid );
private:
	static goods_map m_goods_map;				//所有的商品 以goods_uid为KEY
	static goods_map_by_tid m_selling_goods_map_by_tid; //正在出售的 以item_tid为KEY的所有商品(道具出手后需要删除)
	static goods_map_by_tid m_selling_rarity_map_by_tid; //正在出售的 以item_tid为KEY的所有珍品装备(道具出售后需要删除)
	...
	static std::map<uint32_t, goods_vec> m_goods_search_map;//以查询id为归类的数据
};
