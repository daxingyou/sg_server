...
public:
	...
	static uint32_t check_user_buy_goods(role_ptr p_role, const proto::common::user_buy_goods& info, em_goods_check_type type, proto::common::role_change_data* p_data);

	// center验证不通过还钱
	static bool back_role_money(role_ptr p_role, const proto::common::user_buy_goods& info);

	// center验证给道具
	static uint32_t give_role_item(role_ptr p_role, const proto::common::user_buy_goods& info, proto::common::role_change_data* p_data);
	...
	static uint32_t goods_buy_request(role_ptr p_role, uint64_t goods_uid, uint32_t item_tid, uint32_t buy_count, uint32_t price);
	//处理center回复的购买返回消息
