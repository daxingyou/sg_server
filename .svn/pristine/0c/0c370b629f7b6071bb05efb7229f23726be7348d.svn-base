...
public:
	...
	//刷新国家悬赏任务
	uint32_t refresh_bounty(proto::common::role_change_data* p_data);
	//领取宝箱
	...
	//快速完成悬赏任务
	void quick_finish_task(uint32_t bounty_id, std::map<uint32_t,uint32_t>& drop_id_count, std::map<uint32_t, uint32_t>& coin_id_count);
private:
	...
	//生成指定星级的国家悬赏任务id
	static uint32_t make_bounty_id(uint32_t star, uint32_t level, const std::vector<uint32_t>& exclude_vec);
	//刷新消耗
	uint32_t get_refresh_cost();
public:
	static uint32_t get_quick_circle_id(uint32_t star, uint32_t level, uint32_t& bounty_id);
private:
	std::string m_key = "";
	...
	uint32_t m_accept_level = 0;		//接取等级（存下来是因为环任务需要按等级筛选，防止由于升级造成匹配不上）
