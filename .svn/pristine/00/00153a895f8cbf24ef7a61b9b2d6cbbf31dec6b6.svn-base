...
bool load_config()
{
	...
	if (NULL != OfflineArenaScoreManager::getInstancePtr() &&
		!OfflineArenaScoreManager::getInstancePtr()->loadBinary(env::xml->config_path))
	{
		return false;
	}
	else
	{
		PUSH_TO_CONFIG_LIST("OfflineArenaScoreManager");
	}
	...
	if (NULL != FriendAwardManager::getInstancePtr() &&
		!FriendAwardManager::getInstancePtr()->loadBinary(env::xml->config_path)) {
		return false;
	}
	else {
		PUSH_TO_CONFIG_LIST("FriendAwardManager");
	}
	...
	//生成反表
	general_event_manager_t::make_event_table();
	general_info_manager_t::make_conf();		//生成反表
...
void release_config()
{
	...
	OfflineArenaScoreManager::delInstance();
	REMOVE_FROM_CONFIG_LIST("OfflineArenaScoreManager");
	...
	FriendAwardManager::delInstance();
	REMOVE_FROM_CONFIG_LIST("FriendAwardManager");
