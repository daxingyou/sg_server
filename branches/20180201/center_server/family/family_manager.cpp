	...
	if (is_in_time)
	{
			...
			citr != m_against_data_map.end(); ++citr)
		{
			...
			if (NULL != p_data)
			{
				...
				{
					p_left_family->set_family_id(common::string_util_t::string_to_uint64(war_data.m_data.left_side().family_id()));
				...
				{
					p_right_family->set_family_id(common::string_util_t::string_to_uint64(war_data.m_data.right_side().family_id()));
		...
		const network::tcp_socket_ptr s = game_manager_t::get_game_socket_by_id(p_map_table->game_id());
		if (NULL == s)
		{
			log_error("game[%u] not open!!!!!!!!!!!!!", p_map_table->game_id());
...
{
	// 防止时间过小
	if (t < WEEK_SECOND * 4) return 0;

	return (t - time_util_t::get_timezone() - 4 * DAY_SECOND) / WEEK_SECOND;
}

uint32_t family_manager_t::get_war_time(uint32_t id)
{
	TimeTable* p_conf = GET_CONF(TimeTable, time_type_enum::time_family_war3);
	if (NULL == p_conf)
	{
		log_error("invalid time id[%u]", time_type_enum::time_family_war3);
		return 0;
	}

	if (p_conf->time_type() != time_type::time_type_every_weekday)
	{
		log_error("time[%u] type[%u] error", time_type_enum::time_family_war3, p_conf->time_type());
		return 0;
	}
	...
	{
		log_error("invalid time_type_every_weekday format error");
		return 0;
	}

	uint32_t interval_time = (start_time_vec[0] - 1) * DAY_SECOND + start_time_vec[1] * 3600 + start_time_vec[2] * 60 + start_time_vec[3];

	return id * WEEK_SECOND + 4 * DAY_SECOND + time_util_t::get_timezone() + interval_time;
}

void family_manager_t::update_against_list()
{
	m_against_list.Clear();
...
uint32_t family_manager_t::ask_enter_family_war_dungeon(const global_user_ptr& p_user)
{
	...
	against_data_map::const_iterator against_citr = m_against_data_map.find(citr->second);
	...
	{
		log_warn("user[%lu] family_war dungeon[%lu] scene_state[%u] error", p_user->get_uid(), against_citr->second.m_war_scene_id, p_family_dungeon->get_scene_state());
		return common::errcode_enum::family_err_war_not_in_time;
	}
...
void family_manager_t::family_war_early_result(const proto::server::family_war_result_data& data)
{
	...
	if (p_left_data)
	{
		...
		if (common::string_util_t::string_to_uint64(p_left_data->family_id()) == data.win_family_id())
		{
	...
	if (p_right_data)
	{
		...
		if (common::string_util_t::string_to_uint64(p_right_data->family_id()) == data.win_family_id())
		{
