...
bool scene_t::unload()
{
	...
	if (!m_roles.empty())
	{
		log_error("scene[%lu] map[%u] have other role num[%u]", m_scene_id, m_map_tid, m_roles.size());
	}
...
bool scene_t::on_enter_s(object_position_ptr p_object, float x, float y, float dir /*= 0.f*/)
{
	if (NULL == p_object)
	{
...
{
	object_position_ptr p_object = object_manager_t::find_object(object_id);
	if (NULL == p_object)
	{
...
void scene_t::on_leave_s(object_position_ptr p_object)
{
	if (NULL == p_object)
	{
		log_error("p_object error null");
...
bool scene_t::on_move_s(const object_position_ptr& p_object, float to_x, float to_y, float direction, float time_stamp)
{
	if (NULL == p_object)
	{
	...
	{
		//SCENE_LOG("onject[%lu] move too near from[%f, %f] to[%f, %f] time_stamp[%f] dir[%f]", object_id.first, p_object->get_current_x(), p_object->get_current_y(),
	...
	p_object->set_current_position(to_x, to_y, direction, time_stamp);
	...
	// SCENE_LOG("object[%lu] move to[%f, %f] time_stamp[%f] dir[%f]", object_id.first, to_x, to_y, time_stamp, direction);
	...
	for (const auto& key : nine_keys)
	{
		...
		if (ptr != NULL)
		{
			for (const auto& itr : *ptr)
			{
				...
				if (distance > 0)
				{
					object_position_ptr p_nearby = object_manager_t::find_object(itr);
					if (NULL == p_nearby)
						continue;
 ...
 */
bool scene_t::object_into_vision(object_position_ptr p_object, const nine_vector& nines)
{
	if (NULL == p_object)
	{
		log_error("p_object null error");
		...
		ncitr != nines.end(); ++ncitr)
	{
		...
		for (scene_grid::const_iterator citr = p_grid->begin();
			citr != p_grid->end() && nine_objs < env::xml->max_nine_objects; ++citr)
		{
			...
			else
			{
				if (object_id.second == proto::common::SCENEOBJECT_USER)
				{
					object_position_ptr p_near_object = object_manager_t::find_object(*citr);
...
*/
bool scene_t::object_out_of_vision(object_position_ptr p_object, const nine_vector& nines)
{
	if (NULL == p_object)
	{
		log_error("p_object null error");
...
void scene_t::notify_nearbies_move_s(object_position_ptr p_object, const nine_vector& nines)
{
	if (NULL == p_object)
	{
...
bool scene_t::init_object(monster_ptr p_monster, const monster_common::monster_define_t& define)
{
	...
	{
		log_error("monster[%u] enter scene[%lu] map[%u] error", p_monster->get_object_id(), m_scene_id, m_map_tid);
...
bool scene_t::revive_object(monster_ptr p_monster, float x, float y, float dir)
{
	...
	{
		log_error("monster[%u] revive error, scene[%lu] map[%u]", p_monster->get_object_id(), m_scene_id, m_map_tid);
		return false;
	}
...
void scene_t::scene_change_pk_mode(role_ptr p_role, proto::common::EM_PK_MODE mode)
{
	if (NULL == p_role)
	{
	...
	// 改变pk状态
	if (p_role->get_pk_mode() != mode && p_role->change_pk_mode(mode))
	{
		...
		//log_warn("role[%lu] pk_mode[%u]", p_role->get_object_id(), mode);
	}
}
...
void scene_t::on_other_kill_me(role_ptr p_role, object_position_ptr p_enemy)
{
	if (p_role == NULL)
	{
		log_error("p_role null error");
	...
	for (uint32_t i = 0; i < m_grid_num; ++i)
	{
		if (NULL != m_scene_objects[i])
		{
				...
				citr != m_scene_objects[i]->end(); ++citr)
			{
				...
				// 加入除了人以外的生物
				if (id_type.second != proto::common::SCENEOBJECT_USER)
				{
