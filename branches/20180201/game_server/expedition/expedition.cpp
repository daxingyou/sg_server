...
void expedition_t::load_data(const proto::common::expedition_data& ed)
{
	...
	for (int32_t i = 0; i < ed.buy_count_size(); i++)
	{
		m_shop_count[i] = ed.buy_count(i);
	}
...
void expedition_t::peek_data(proto::common::expedition_data* p_data)
{
	...
	for (uint32_t i = 0; i < shop_effect_all; ++i)
	{
		p_data->add_buy_count(m_shop_count[i]);
	}
...
void expedition_t::reset_shop_count()
{
	for (uint32_t i = 0; i < shop_effect_all; ++i)
	{
		auto p_conf = GET_CONF(ExpeditionShop, i + 1);
		if (NULL == p_conf)
		{
			log_error("NULL == p_conf[%d]", i+1);
			return;
		}
		m_shop_count[i] = p_conf->count();
	}
}
...
void expedition_t::save_self()
{
	...
	role_unify_save::add_task(m_key, expedition_data, false, false, m_owner);
}
...
uint32_t expedition_t::fight(uint32_t stage_id)
{
	...
	if (NULL == p_role)
	{
		log_error("NULL == p_role[%llu]", m_owner);
		return errcode_enum::notice_role_null;
	}
	if (!has_hero())
	{
		log_error("p_role[%llu] no hero", m_owner);
	...
	if (troop_id > 0)
	{
		...
		if (NULL == p_troop)
		{
			log_error("NULL == p_troop[%llu]", troop_id);
	...
	const proto::common::fight_save_data& esd, const std::vector<uint64_t>& helpers)
{
	...
	if (is_win)
	{
		...
		for (auto helper_uid : helpers)
		{
			...
			if (NULL == p_helper)
			{
				log_error("NULL == p_helper[%llu]", helper_uid);
				continue;
			}
			//帮手奖励
			...
			{
				log_error("NULL == p_expedition[%llu]", helper_uid);
				continue;
			}
...
uint32_t expedition_t::shop_request(uint32_t shop_id, uint64_t hero_uid, uint32_t buy_style, proto::client::gc_expedition_shop_reply& reply)
{
	role_ptr p_role = role_manager_t::find_role(m_owner);
	if (NULL == p_role)
	{
	...
	auto p_shop_conf = GET_CONF(ExpeditionShop, shop_id);
	if (NULL == p_shop_conf)
	{
		log_error("NULL == p_shop_conf[%d]", shop_id);
		return errcode_enum::notice_unknown;
	}
	if (0 == shop_id || shop_id > shop_effect_all)
	{
		log_error("shop_id[%d] invalid", shop_id);
		return errcode_enum::notice_unknown;
	}
	if (m_shop_count[shop_id - 1] == 0)
	{
		log_error("shop_id[%d] count is 0", shop_id);
		return errcode_enum::notice_unknown;
	}
	if (buy_style == 0)
	{
		if (!money_manager_t::use_money(p_role, proto::common::MONEY_TYPE_YUANBAO, p_shop_conf->money(), log_enum::source_type_expedition_shop, shop_id, true, reply.mutable_rcd()))
		{
	...
	{
		if (m_glory < p_shop_conf->glory())
		{
			log_error("role[%lu] glory not enough", m_owner);
			return errcode_enum::notice_expedition_not_enough_glory;
		}
		m_glory -= p_shop_conf->glory();
		notify_glory_to_center();
	...
	{
	case shop_effect_hp:
		reply_code = shop_hp(hero_uid);
		break;
	case shop_effect_cd:
		reply_code = shop_cd(hero_uid);
		break;
	case shop_effect_all:
		reply_code = shop_all(hero_uid);
		break;
	default:
		break;
	}
	if (reply_code == errcode_enum::error_ok)
	{
		...
		for (auto it : m_hero_map)
		{
			...
			proto::common::hero_save_data* p_single = reply.mutable_save_data()->add_hero_list();
			p_hero->peek_data(p_single);
		}
		reply.set_glory(m_glory);
		for (uint32_t i = 0; i < shop_effect_all; ++i)
		{
			reply.add_buy_count(m_shop_count[i]);
		}
		log_wrapper_t::send_expedition_log(m_owner, p_role->get_level(), log_enum::source_type_expedition_shop, shop_id);
	}
...
{
	if (m_pass_num >= m_fight_data.size())
	{
		log_error("role[%lu] stage[%d]", owner, m_tid);
		return errcode_enum::notice_unknown;
	}
	auto p_fight_data = m_fight_data[m_pass_num];
	if (NULL == p_fight_data)
	{
