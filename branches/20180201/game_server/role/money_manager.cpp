...
bool money_manager_t::add_money(role_ptr p_role, proto::common::MONEY_TYPE type, uint32_t count, uint32_t source_type, uint32_t source_param, bool notice , proto::common::role_change_data* p_data,bool is_notify)
{
	...
	switch (type) {
		case proto::common::MONEY_TYPE_PRESTIGE: {
			add_prestige(p_role, count, source_type, source_param, notice);
			break;
		}
		case proto::common::MONEY_TYPE_ACHIEVE_POINT: {
			add_achieve_point(p_role, count, source_type, source_param, notice, p_data);
			break;
		}
		case proto::common::MONEY_TYPE_HONOR: {
			add_honor(p_role, count, source_type, source_param, notice, p_data);
			break;
		}
		case proto::common::MONEY_TYPE_VIGOUR: {
	...
	if (is_notify) sys_notice_manager_t::broadcast_system_msg_for_money(p_role->get_uid(), type, count);
...
bool money_manager_t::use_money(role_ptr p_role, proto::common::MONEY_TYPE type, uint32_t count, uint32_t source_type, uint32_t source_param, bool notice /*= true*/, proto::common::role_change_data* p_data)
{
	...
	switch (type) {
		case proto::common::MONEY_TYPE_PRESTIGE: {
			use_prestige(p_role, count, source_type, source_param, notice);
			break;
		}
		case proto::common::MONEY_TYPE_VIGOUR: {
...
void money_manager_t::add_prestige(role_ptr p_role, uint32_t count, uint32_t source_type, uint32_t source_param, bool notice /*= true*/)
{
	if (NULL == p_role)
	{
		log_error("NULL == p_role");
...
bool money_manager_t::use_prestige(role_ptr p_role, uint32_t count, uint32_t source_type, uint32_t source_param, bool notice /*= true*/)
{
	if (NULL == p_role)
	{
		log_error("NULL == p_role");
...
void money_manager_t::add_achieve_point(role_ptr p_role, uint32_t count, uint32_t source_type, uint32_t source_param, bool notice, proto::common::role_change_data* p_data)
{
	if (NULL == p_role)
	{
		log_error("NULL == p_role");
	...
	custom_data.push_back(string_util_t::uint32_to_string(p_role->get_achieve_level()));
	rank_manager_t::update_rank(rank_common::achievement_rank_0, p_role->get_uid(), new_achieve_point, custom_data);
...
void money_manager_t::add_honor(role_ptr p_role, uint32_t count, uint32_t source_type, uint32_t source_param, bool notice /*= true*/, proto::common::role_change_data* p_data /*= NULL*/)
{
	if (NULL == p_role)
	{
		log_error("NULL == p_role");
...
uint32_t money_manager_t::game_recharge(const uint64_t role_uid, const uint32_t tid, proto::common::role_change_data* p_data /*= NULL*/)
{
	...
	do
	{
		...
		if (p_role->is_first_recharge())
		{
			add_val += p_config->gift_money();
		}
		p_role->add_recharge_count();
		add_money(p_role, proto::common::MONEY_TYPE_LIFE_STONE, add_val, log_enum::source_type_recharge_success, tid, true, p_data);
		p_role->add_total_cny(p_config->CNY(),p_data);
		p_role->save_self(true);

		proto::common::recharge_data* p_recharge = p_data->mutable_recharge();
		p_recharge->set_recharge_flag(p_role->get_recharge_count());
		p_recharge->set_recharge_reward_flag(p_role->get_recharge_reward_flag());

		time_t t = common::time_util_t::now_time();
		struct tm *ptm = localtime(&t);
		char timestamp[128];
		strftime(timestamp, 128, "%Y-%m-%d", ptm);
		uint64_t order_id = common::g_id_allocator.gen_new_id(common::ID_ALLOCATOR_TYPE_RECHARGE);
		std::string order_id_str = string_util_t::uint64_to_string(order_id);
		log_wrapper_t::send_recharge_log(role_uid, order_id_str, tid, p_config->CNY(), 0, timestamp);
