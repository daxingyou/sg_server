...
public:
	static uint32_t game_recharge(const uint64_t role_uid, const uint32_t tid, proto::common::role_change_data* p_data);

private:
	//声望
	static void		add_prestige(role_ptr p_role, uint32_t count, uint32_t source_type, uint32_t source_param, bool notice = true);
	static bool		use_prestige(role_ptr p_role, uint32_t count, uint32_t source_type, uint32_t source_param, bool notice = true);
	static uint32_t get_prestige_level(role_ptr p_role);
	//成就积分
	static void		add_achieve_point(role_ptr p_role, uint32_t count, uint32_t source_type, uint32_t source_param, bool notice = true, proto::common::role_change_data* p_data = NULL);

	//荣誉
	static void		add_honor(role_ptr p_role, uint32_t count, uint32_t source_type, uint32_t source_param, bool notice = true, proto::common::role_change_data* p_data = NULL);
