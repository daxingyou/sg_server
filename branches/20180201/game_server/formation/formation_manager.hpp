...
public:
	...
	// 检查武将上阵要求
	bool check_hero_up_form(uint32_t fight_type, hero_ptr p_hero);
	// 是否是有效的布阵
	virtual bool is_validate_formation(role_ptr p_role, formation_ptr p_formation, uint32_t fight_type, const std::map<uint32_t, uint64_t> &hero_pos, uint32_t tactic_id);
	// 添加新的布阵
	virtual bool init_formation();
	// 更新布阵
	virtual bool update_formation(role_ptr p_role, uint32_t form_index, uint32_t fight_type, const std::map<uint32_t, uint64_t> &hero_pos, uint32_t tactic_id);
	// 更新当前阵容
