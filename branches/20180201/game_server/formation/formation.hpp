...
public:
	...
	// 更新布阵
	void update_formation(const std::map<uint32_t, uint64_t> &pos_hero, uint32_t tactic_id, bool is_new = false, bool is_save = true);
...
public:
    ...
    const std::map<uint32_t, uint64_t>& get_pos_hero() {return m_pos_hero;}
...
protected:
	...
	uint64_t m_owner = 0;
	uint32_t m_index = 0;
	std::map<uint32_t, uint64_t> m_pos_hero;
