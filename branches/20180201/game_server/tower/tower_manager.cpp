...
void tower_manager_t::fight_result(role_ptr p_role, bool iswin, uint64_t trigger_uid, proto::common::role_change_data* rcd)
{
	...
	if (rcd == NULL)
	{
		log_error("role[%u] NULL == rcd", p_role->get_uid());
		return;
	}
	tower_ptr p_tower = p_role->get_tower();
	if (NULL == p_tower)
	...
	if (0 == trigger_uid)
	{
		...
		if (record_tower < current_tower)
		{
			...
			//排行榜
			uint32_t now_day = time_util_t::get_today();
			std::vector<std::string> custom_data;
			custom_data.push_back(string_util_t::uint32_to_string(p_role->get_fighting()));
			custom_data.push_back(string_util_t::uint32_to_string(now_day - p_tower->get_first_day()));
			rank_manager_t::update_rank(rank_common::towers_rank_0, p_role->get_uid(), current_tower, custom_data);
