...
void log_wrapper_t::send_luck_gift_log(uint64_t src_role_uid, uint64_t grab_role_uid, uint64_t luck_gift_uid, uint32_t old_num,
									   uint32_t up_num, uint32_t new_num)
{
	proto::log::go_luck_gift_log_notify msg;
	msg.set_src_role_uid(src_role_uid);
	msg.set_grab_role_uid(grab_role_uid);
	msg.set_luck_gift_uid(luck_gift_uid);
	msg.set_old_num(old_num);
	msg.set_up_num(up_num);
	msg.set_new_num(new_num);
	...
	uint32_t now_time = common::time_util_t::now_time();
	msg.set_log_time(now_time);
	write_new_log(op_cmd::go_luck_gift_log_notify, msg);
}
...
void log_wrapper_t::send_task_log(uint64_t role_uid, uint32_t task_id, uint32_t task_state, const std::string& progress, 
...
void log_wrapper_t::send_recharge_log(uint64_t role_uid, const std::string& order_id,uint32_t recharge_tid,
									uint32_t recharge_rmb, uint32_t channel_id,const std::string& date_str)
{
