...
void task_t::save_self(uint64_t role_uid, bool is_new)
{
	proto::common::task_state state;
	peek_data(&state);
	role_unify_save::add_task(m_key, state, is_new, false, role_uid);
}
...
void task_t::change_state(task_state now_state)
{
	m_state = now_state;
