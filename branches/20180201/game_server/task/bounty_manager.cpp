...
void bounty_manager_t::save_self()
{
	...
	role_unify_save::add_task(m_key, country_bounty_data, false, false, m_role_uid);
}
...
bool bounty_manager_t::accept_bounty(uint32_t bounty_id)
{
	...
	p_task_mgr->active_circle_task(TASK_TYPE_BOUNTY, p_bounty_conf->circle_id(), ACCEPT_TASK_BY_SYSTEM, rcd);
...
bool bounty_manager_t::refresh_bounty()
{
	role_ptr p_role = role_manager_t::find_role(m_role_uid);
	if (NULL == p_role)
	{
	...
	{
		log_error("role[%lu] refresh_count not enough", m_role_uid);
		return false;
	}
...
uint32_t bounty_manager_t::rand_task_id(uint32_t level)
{
	std::vector<uint32_t> rand_bounty;
	...
	uint32_t bounty_id = rand_bounty[0];
...
void bounty_manager_t::quick_finish_task(uint32_t tid, proto::common::role_change_data *rcd)
{
	role_ptr p_role = role_manager_t::find_role(m_role_uid);
	if (NULL == p_role)
	{
	...
	auto p_bounty_conf = GET_CONF(Bounty, tid);
	if (NULL == p_bounty_conf)
	{
		log_error("user:[%lu] p_bounty_conf is null", m_role_uid);
		return;
	}

	TASK_LOG("role[%lu] bounty_finish[%d]", m_role_uid, tid);
	...
	drop_manager_t::drop(p_role, drop_id, log_enum::source_type_bounty_finish, proto::common::drop_sys_type_bounty, tid, rcd );
	money_manager_t::add_money(p_role, proto::common::MONEY_TYPE_PRESTIGE, p_bounty_conf->prestige_award(), log_enum::source_type_bounty_finish, tid, rcd);
	center_msg_handler_t::sync_data_to_center(m_role_uid, proto::server::data_sync_type_country_money, p_bounty_conf->country_award(), true);
