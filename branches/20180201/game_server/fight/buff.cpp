...
buff_t::buff_t(uint32_t id, BuffTable* conf, const uint64_t attacker, uint32_t skill, const uint32_t layer, uint32_t hero_round)
{
	m_skill = skill;
	if (NULL != conf)
	{
...
bool buff_t::need_remove_when_dead()
{
	...
	return (conf->remove_type() != buff_remove_type_dead_not_remove);
}
