...
bool combat_t::init_fight_obj(proto::common::obj_data* od)
{
	...
	for (int i = 0; i < od->obj_size(); ++i)
	{
		...
		if (!obj->init(os))
		{
			log_error("combat_t::init_fight_obj fail %lu", os->object_uid().c_str());
			continue;
		}
...
bool combat_t::loading_end(uint64_t uid)
{
	...
	if (obj->get_state() != proto::common::combat_obj_state_loading)
	{
...
bool combat_t::reset_single_camp_form_data(uint32_t camp)
{
	...
	for (auto objpair : m_objs)
	{
		...
		// 非玩家不处理
		if (objpair.second->get_type() != proto::common::SCENEOBJECT_USER ||
			objpair.second->get_camp() != camp)
			continue;

		p_role = role_manager_t::find_role(objpair.second->get_uid());
		if (NULL == p_role)
			continue;
		...
		case proto::common::combat_camp_1:
			{
				fight_manager_t::fill_hero_data(proto::common::combat_camp_1, CAMP_SELF_UNIQUE_OFFSET, p_role, form,
					get_fight_type(), m_fight_data.mutable_self()->mutable_heroes(),
		...
		case proto::common::combat_camp_2:
			{
				fight_manager_t::fill_hero_data(proto::common::combat_camp_2, CAMP_ENEMY_UNIQUE_OFFSET, p_role, form,
					get_fight_type(), m_fight_data.mutable_enemy()->mutable_heroes(),
...
void combat_t::fight_start_event()
{
	...
	for (it = m_heros.begin(); it != m_heros.end(); ++it)
	{
		...
		p_hero->on_event(event_on_fight_begin);
	}
}
...
bool combat_t::round_start()
{
	...
	if (m_hero_att_round == 1)
	{
		...
		fight_hero_ptr p_hero = m_alive_hero_list[0];
		if (NULL == p_hero)
		{
			log_error("combat_t p_fight_hero NULL");
			return false;
		}
		if (p_hero->get_speed() == 0)
		{
		...
		m_max_att_speed = p_hero->get_att_speed() * max_att_speed_rate;
	}
	...
	m_p_att_fight_hero->on_round_start_imp();
...
void combat_t::find_cur_att_hero()
{
	...
	if(!b_find_first_att_hero)
	{
		...
		for (uint32_t i = 0; i < m_alive_hero_list.size(); ++i)
		{
			...
			uint32_t speed = p_fight_hero->get_speed();
			if (speed == 0)
			{
			...
			speed_dec = m_max_att_speed - speed;
			speed_remain = (speed_dec % speed) > 0 ? 1 : 0;
			temp_add_speed_count = speed_dec / speed + speed_remain;
		...
		{
			log_error("combat_t final_add_speed_count = 0 hero_tid[%d] hero_uid[%d] att_speed[%d] order_att_speed[%d]",
...
bool combat_t::prepare_start()
{
	...
	if (m_p_att_fight_hero->is_in_control() || 
...
//		return false;
//	}
//	m_timer.start(deploy_finish_ani_time, this, &combat_t::on_disposition_ready_timer, env::server->get_ios());
//	m_end_time = time_util_t::now_time() + deploy_finish_ani_time;
	play_start();
	return true;
}
...
bool combat_t::play_start()
{
	...
	// 预留给客户端的时间 服务器时间不能太及时 客户端一卡就懵逼了
	uint32_t play_time = ceil(get_fight_play_time(m_hero_att_round)) + fight_fault_tolerant_time;
...
void combat_t::add_first_att_hero_list(fight_hero_ptr p_fight_hero)
{
	...
	if (p_fight_hero->get_att_more_speed() > 0)
	{
		...
		{
			log_error("add_first_att_hero_list order_pos[%d] >= order_list[%d]", order_pos, m_first_att_hero_list.size());
			return;
		}
		// 自己
...
void combat_t::remove_first_att_hero_list(fight_hero_ptr p_fight_hero)
{
	...
	if ((uint32_t)order_pos >= m_first_att_hero_list.size())
	{
		log_error("remove_first_att_hero_list order_pos[%d] >= order_size[%d]", order_pos, m_first_att_hero_list.size());
		return;
	}
...
void combat_t::check_hide_event(combat_event_step_type type)
{
	...
	for (; iter != m_hide_heros.end(); ++iter)
	{
		...
		if (NULL == conf)
		{
			log_error("combat_t conf NULL tid[%]", p_hero->get_tid());
			continue;
		}
...
//	data->set_round(m_round);
	...
	if (p_hero != NULL)
	{
		...
		p_hero->peek_fight_skill_data(data->mutable_skill_data());
	}
...
bool combat_t::check_loading_finish()
{
	for (auto objpair : m_objs)
	{
		if ((NULL != objpair.second) && (objpair.second->get_state() != proto::common::combat_obj_state_loading_end))
		{
			return false;
		}
	}
...
// 	}
	...
	for (const proto::common::combat_form_single& action : data.forms())
	{
...
// 		}
		...
		uint32_t cd = attacker->get_skill_cd(action.skill());
		if (cd > 0)
		{
			log_error("disposition skill cd error %u %u", action.skill(), cd);
			continue;
		}
		...
		{
			log_error("disposition attacker[%lu %d] speed[%d] error %u %d", attacker->get_uid(), attacker->get_tid(), attacker->get_speed());
			continue;
		}
...
bool combat_t::check_disposition_finish()
{
	for (auto objpair : m_objs)
	{
		if (NULL != objpair.second && objpair.second->get_state() != proto::common::combat_obj_state_prepare_end)
		{
			return false;
		}
	}
...
bool combat_t::check_play_finish()
{
	for (auto objpair : m_objs)
	{
		if (NULL != objpair.second && objpair.second->get_state() != proto::common::combat_obj_state_play_end)
		{
			return false;
		}
	}
...
const pos_hero* combat_t::get_skill_target_camp_pos_hero(fight_hero_ptr attaker, SkillEffectTable* skill_conf)
{
	...
	if (skill_conf->target_type() == skill_target_alive_friend
		|| skill_conf->target_type() == skill_target_alive_teammate
		|| skill_conf->target_type() == skill_target_friend
		|| skill_conf->target_type() == skill_target_teammate
		|| skill_conf->target_type() == skill_target_dead_teammate
		|| skill_conf->target_type() == skill_target_dead_friend )
	{
		camp_heros = get_camp_pos_hero(attaker->get_camp());
	}
	else if (skill_conf->target_type() == skill_target_alive_enemy
		|| skill_conf->target_type() == skill_target_dead_enemy
		|| skill_conf->target_type() == skill_target_enemy)
	{
...
bool combat_t::check_skill_target_type(SkillEffectTable* skill_conf, fight_hero_ptr attacker, fight_hero_ptr target, bool disposition)
{
	...
	// 阵营
	if (target_type == skill_target_friend || 
...
fight_hero_ptr combat_t::find_default_target(fight_hero_ptr attacker, SkillEffectTable* skill_conf)
{
	...
	{
		uint32_t random_value = random_util_t::randMin(0, result.size());
		return result[random_value];
	}
	else
	{
		uint32_t random_value = random_util_t::randBetween(1, all_skill_target_ai_rate);
		...
		for (uint32_t i = 0; i < skill_target_ai_rate_list.size(); ++i)
		{
			...
			if (random_value <= compare_value)
			{
				...
				{
					log_error("find_default_target find_pos[%d] >= skill_ai_list_size[%d]", i, result.size());
					return 0;
				}
			}
		}
	...
	fight_hero_ptr target )
{
	...
	default:
	{
		if (target != NULL)
		{
			if (check_skill_target_type(skill_conf, attaker, target))
			{
	...
	fight_hero_ptr select)
{
	...
	return find_target_by_skill_conf(attaker, skill_conf, targets, spurting_targets, target);
}
...
bool combat_t::find_spurting_target( fight_hero_ptr attaker, SkillEffectTable* skill_conf, 
										std::vector<fight_hero_ptr>& targets, 
										std::vector<fight_hero_ptr>& spurting_heroes, fight_hero_ptr select, uint32_t num)
{
	if (check_skill_target_type(skill_conf, attaker, select))
	{
		targets.push_back(select);
	...
	fight_hero_ptr select)
{
	...
	if (random_pos == -1)
	{
		log_error("find_min_hp_target rand error min[%d] max[%d]", 0, temp_targets.size());
		return false;
	}
...
fight_hero_ptr combat_t::select_confused_target(fight_hero_ptr attaker)
{
	...
	if (rand_num == -1)
	{
		log_error("select_confused_target rand error min[%d] max[%d]", 0, heros.size());
		return NULL;
	}
...
bool combat_t::request_escape(uint64_t uid)
{
	...
	for (auto pair : m_objs)
	{
		...
		if(NULL == p_obj)
			continue;
		if (p_obj->get_type() != proto::common::SCENEOBJECT_USER || p_obj->get_camp() != p_escape_obj->get_camp())
			continue;
		p_role = role_manager_t::find_role(p_obj->get_uid());
		if (NULL == p_role)
		{
		...
		if (p_obj->is_escape() || p_role->get_online_state() == common::ROLE_ONLINE_STATE_OFFLINE || p_role->get_online_state() == common::ROLE_ONLINE_STATE_LOGOUT)
			++escape_num;
		++all_num;
	}
	// 多人组队情况下 2个人以上逃跑才能结束战斗
	// 不可结束战斗
	if (all_num - escape_num >= 2)
	{
// 		proto::client::gc_combat_escape_ntf reply;
// 		reply.set_role_uid(string_util_t::uint64_to_string(uid));
// 		send_msg_to_camp(p_escape_obj->get_camp(), op_cmd::gc_combat_escape_ntf, reply);
		return true;
	}
	// 结束战斗
...
{
	auto fight_two_speed_conf = GET_CONF(Comprehensive, comprehensive_common::fight_two_speed_rate);
...
uint32_t combat_t::get_hero_injured(uint64_t hero_uid)
{
	...
	return hero->get_count_data().one_round_injured;
}
...
fight_hero_ptr combat_t::select_min_hp_target(uint32_t camp)
{
	std::vector<fight_hero_ptr> hero_list;
	get_camp_alive_heros(hero_list, camp);
...
bool combat_t::can_watch(uint32_t watch_num)
{
	...
	uint32_t max_normal_combat_watch_num = GET_COMPREHENSIVE_VALUE_1(conf);
