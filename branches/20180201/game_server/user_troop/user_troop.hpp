...
public:
	...
	void process_event(uint32_t event_id, uint64_t member_id, proto::common::troop_data* td);
	...
	// 各个事件的处理
public:
	void do_create_troop(uint64_t member_id, proto::common::troop_data* td);
	void do_change_leader(uint64_t member_id, proto::common::troop_data* td);
	void do_member_join(uint64_t member_id, proto::common::troop_data* td);
	void do_member_quit(uint64_t member_id);
	void do_member_kick(uint64_t member_id);
	void do_member_leave(uint64_t member_id);
	void do_member_back(uint64_t member_id);
	void do_troop_modify(uint64_t member_id, proto::common::troop_data* td);
	void do_member_update(uint64_t member_id, proto::common::troop_data* td);
	void do_member_online(uint64_t member_id, proto::common::troop_data* td);
	void do_troop_dissolve(uint64_t member_id);
	void do_member_transmission(uint64_t member_id);
	void do_member_zhuagui_task(uint64_t member_id, proto::common::troop_data* td);

	void add_zhuagui_task(uint64_t member_id, proto::common::troop_data* td);
	void clear_zhuagui_task();
...
public:
	// 添加队长移动位置
	void leader_move(const scene_ptr& p_scene, uint32_t speed, float x, float y, float direction);
...
public:
	...
	//根据武将ID找到拥有者
	role_ptr get_heros_role_by_hero_uid(uint64_t hero_id);
...
private:
	struct troop_move
	{
		...
		troop_move() {};
		troop_move(float x, float y, float dir) : m_x(x), m_y(y), m_dir(dir) {};
	};
