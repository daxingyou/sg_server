...
void game_troop_mgr_t::troop_event(uint64_t uid, uint32_t event_id, uint64_t member_id, proto::common::troop_data* td)
{
	if (event_id == proto::common::troop_event_create) {
		...
		if (!res.second) {
			log_error("user[%lu] create new troop insert error, it is exist", member_id);
			return;
		}

		p_troop->process_event(event_id, member_id, td);
	} else {
		troop_map::iterator itr = m_troop_map.find(uid);
		if (itr != m_troop_map.end() && itr->second != NULL) {
			itr->second->process_event(event_id, member_id, td);
		}
