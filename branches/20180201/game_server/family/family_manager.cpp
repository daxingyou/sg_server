...
void family_manager_t::family_war_end()
{
		...
		citr != m_family_war_info_map.end(); ++citr)
	{
		...
		proto::server::family_war_result_data* data = notify.add_datas();
		if (data)
		{
			data->set_id(citr->first);
			data->set_win_family_id(p_family_war_scene->get_win_family());
			data->set_left_family_score(p_family_war_scene->get_family_score(family_war_side_left));
			data->set_right_family_score(p_family_war_scene->get_family_score(family_war_side_right));
		}
	}
...
uint32_t family_manager_t::ask_formations_hero_up(const role_ptr& p_role, const proto::client::cg_formations_hero_up_request& request)
{
	if (NULL == p_role)
	{
		log_error("NULL == p_role");
	...
	{
		back_code = p_role_info->role_hero_up(p_role, common::string_util_t::string_to_uint64(request.hero_uid(i)));
...
uint32_t family_manager_t::ask_formations_hero_down(const role_ptr& p_role, const proto::client::cg_formations_hero_down_request& request)
{
	if (NULL == p_role)
	{
		log_error("NULL == p_role");
	...
	for (int32_t i = 0; i < request.hero_uid_size(); ++i)
	{
		back_code = p_role_info->role_hero_down(p_role, common::string_util_t::string_to_uint64(request.hero_uid(i)));
...
void family_manager_t::family_war_early_end(uint32_t war_id, uint64_t win_family_id, uint32_t left_score, uint32_t right_score)
{
	proto::server::ge_family_war_early_result_notify notify;
	proto::server::family_war_result_data* data = notify.mutable_data();
	if (data)
	{
		data->set_id(war_id);
		data->set_win_family_id(win_family_id);
		data->set_left_family_score(left_score);
		data->set_right_family_score(right_score);
	}
	env::server->send_msg_to_center(op_cmd::ge_family_war_early_result_notify, 0, notify);

	family_war_info_map::iterator itr = m_family_war_info_map.find(war_id);
...
uint32_t family_manager_t::ask_family_prayer_request(const role_ptr& p_role, const proto::client::cg_family_prayer_request& request)
{
	...
	if (request.has_hero_uid() && !request.hero_uid().empty()) {
		hero_ptr p_hero = hero_manager_t::get_hero_by_unique_id(p_role, common::string_util_t::string_to_uint64(request.hero_uid()));
		if (NULL == p_hero) {
...
uint32_t family_manager_t::family_shop_buy_request(const role_ptr& p_role, uint32_t index, uint32_t shop_id, uint32_t num)
{
	...
	if (num == 0) {
		log_error("role[%lu] family shop buy count == 0", p_role->get_uid(), num);
