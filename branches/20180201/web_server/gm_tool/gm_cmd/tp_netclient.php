...
class TP_NetClient_TCP extends TP_NetClient
{
	...
	// 带probuff的send
	function probuffsend($g_header,$g_admin)
	{
		$g_sz_header = $g_header->SerializeToString();
		$g_sz_admin = $g_admin->SerializeToString();
		$g_sz_magic = 0;
		
		$g_head_len = strlen($g_sz_header);
		$g_admin_len = strlen($g_sz_admin);
		$msglen = $g_admin_len+$g_head_len+4+4+1;

		$g_head_len_ntohl=$this->net_htonl($g_head_len);
		$g_msg_len_ntohl=$this->net_htonl($msglen);
		$g_magic = $this->net_htonc($g_sz_magic);

		$g_sz_total = $g_msg_len_ntohl . $g_head_len_ntohl . $g_magic . $g_sz_header . $g_sz_admin;
