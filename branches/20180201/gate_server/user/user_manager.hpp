...
public:
    ...
    static void send_to_all_users(uint16_t cmd, const T_MSG& msg)
    {
        ...
        for (; itr != itr_end; ++itr)
        {
            if (NULL != itr->second && itr->second->get_game_state() == common::ROLE_GAME_STATE_PLAY)
            {
...
public:
	...
	// 有socket不一定有user对象
	// 有user对象也不一定有socket
    static void clear_all_users_and_socket(bool is_notify_login, common::KICK_ROLE_REASON reason);
	...
	static void alive_timeout_callback(const boost::system::error_code& ec, uint64_t uid);
...
public:
    static list_t<network::tcp_socket_wptr>::type m_client_socket_list;
