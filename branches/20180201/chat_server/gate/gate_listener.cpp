...
{
    env::server->get_ios().post(boost::bind(&chat_server_t::on_add_gate, env::server, s));
}
...
void gate_listener_t::on_new_msg(const tcp_socket_ptr& s, const message_t& msg)
{
	if (msg.has_uid())
	{
		env::server->get_ios().post(boost::bind(&chat_server_t::on_gate_msg, env::server, s, msg.get_uid(), msg.head.cmd, msg.body));
	}
	else
	{
		env::server->get_ios().post(boost::bind(&chat_server_t::on_gate_msg, env::server, s, 0, msg.head.cmd, msg.body));
	}
...
{
    env::server->get_ios().post(boost::bind(&chat_server_t::on_del_gate, env::server, s));
}
