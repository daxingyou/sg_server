...
public:
    ...
    */
    ...
    void send_msg_to_transfer(uint16_t cmd, uint64_t uid, const T_MSG& msg)
    {
        ...
        int32_t transfer_server_pos = env::xml->srv_group.group_id % m_transfer_connector_list.size();
        transfer_connector_t &transfer_connector = m_transfer_connector_list[transfer_server_pos];
		//transfer_connector.send_msg_to_transfer(send_buf);
		post_network_send_msg(transfer_connector.get_socket(), send_buf);
        uint32_t transfer_id = transfer_manager_t::get_transfer_id_by_socket(transfer_connector.get_socket());
        log_trace("send uid[%lu] cmd[%s:%d] to transfer[%d]", uid, CMD_DESC(cmd), cmd, transfer_id);
    }
    ...
    void send_msg_to_transfer(const network::tcp_socket_ptr& s, uint16_t cmd, uint64_t uid, const T_MSG& msg)
    {
        ...
        post_network_send_msg(s, send_buf);
        log_trace("send uid[%lu] cmd[%s:%d] to transfer[%d]", uid, CMD_DESC(cmd), cmd, transfer_manager_t::get_transfer_id_by_socket(s));
	}
	...
	void send_msg_from_game_to_transfer(uint16_t cmd, uint64_t uid, const T_BUF& msg_buf)
	{
		...
		post_network_send_msg(transfer_connector.get_socket(), send_buf);
		uint32_t transfer_id = transfer_manager_t::get_transfer_id_by_socket(transfer_connector.get_socket());
		log_trace("send role[%lu] cmd[%s:%d] from game to transfer[%d]", uid, CMD_DESC(cmd), cmd, transfer_id);
	}
