...
void world_cup_battle_t::peek_data(proto::common::world_cup_battle* p_battle_single)
{
	...
	arena_rank_info_ptr p_role_a = arena_score_rank_mgr_t::find_user_arena_info(m_player_a_uid);
	if (NULL != p_role_a)
	{
		p_player_a->set_uid(string_util_t::uint64_to_string(m_player_a_uid));
		p_player_a->set_name(p_role_a->get_name());
	...
	arena_rank_info_ptr p_role_b = arena_score_rank_mgr_t::find_user_arena_info(m_player_b_uid);
	if (NULL != p_role_b)
	{
		p_player_b->set_uid(string_util_t::uint64_to_string(m_player_b_uid));
		p_player_b->set_name(p_role_b->get_name());
