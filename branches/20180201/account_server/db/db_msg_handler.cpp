...
bool db_msg_handler_t::handle_dn_create_role_request(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	...
	TRY
	{
		do
		{
			...
			{
				uint64_t role_num = ResultSet_getLLong(result, 1);
			...
			{
				channel_id = ResultSet_getLLong(account_sql, 1);
			}
...
bool db_msg_handler_t::handle_dn_role_data_update_notify(const network::tcp_socket_ptr& s, const network::msg_buf_ptr& msg_buf)
{
	...
	TRY
	{
		do
		{
			PreparedStatement_T pre = Connection_prepareStatement(conn, "UPDATE `role` SET `level` = ?, `plugin` = ? WHERE `uid` = ?;");
			if (pre)
			{
				PreparedStatement_setInt(pre,		1, level);
				PreparedStatement_setInt(pre,		2, plugin);
				PreparedStatement_setLLong(pre,		3, uid);
				PreparedStatement_execute(pre);
			}
			else
			{
    ...
    ROLE_LOG("role[%lu] update data[level=%d plugin=%d] success!", uid, level, plugin);
