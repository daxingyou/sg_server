...
bool curl_warpper_t::init(const std::string& url)
{
    ...
    if (CURLE_OK != curl_code)
    {
        log_error("curl_easy_setopt CURLOPT_URL[%s], error[%d:%s]", m_curl, curl_code, curl_easy_strerror(curl_code));
        return false;
    }
...
std::string curl_warpper_t::post_http_request(const std::string& post_fields)
{
    ...
    do 
    {
        ...
        if (CURLE_OK != curl_code)
        {
            log_error("curl_easy_setopt CURLOPT_POSTFIELDSIZE[%d], error[%d:%s]", post_fields.size(), curl_code, curl_easy_strerror(curl_code));
            break;
        }
...
std::string curl_warpper_t::parse_respone_data(const std::string& result)
{
    ...
    {
        log_error("parse json[%s] failed, error[%s] result[%s]", result.c_str(), e.what());
    }
