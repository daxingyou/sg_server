...
{
    env::server->get_ios().post(boost::bind(&account_server_t::on_add_log, env::server, s));
}
...
{
    env::server->get_ios().post(boost::bind(&account_server_t::on_del_log, env::server, s));
}
